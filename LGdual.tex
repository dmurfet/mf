...
TODO: Ragnar Buchweitz, who else? 
...
$$ 
where $(-)^\vee$ denotes taking the dual module and dual map.
\end{definition}
...
\begin{theorem}
The becategory $\LG$ has adjoints. Explicit expressions for the evaluation and coevaluation maps are given by~\eqref{TODO}. 
\end{theorem}
...
\begin{proof}
The first identity follows immediately from the explicit expression for the evaluation in terms of the supertrace. To prove the second and third identity we only have to apply the Zorro move twice: 
$$
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=.4cm]
\fill (3,0.5) circle (2.5pt) node[right] {{\small $\varphi^\dual$}};
...
\begin{proof}
(i): In the proof of the explicit formula for $\mathcal D_l(X)$ in Proposition~\ref{prop:defectaction} we could also have used the alternative representations for $\eval_X$ and $\widetilde\coev_X$ in terms of $d_{X^\dual}$. Then the assertion immediately follows. 

(ii) \& (iii): These relations follow from Proposition~\ref{prop:monoidalproperty} and a straightforward calculation with string diagrams, see e.\,g.~the proof of~\cite[Lemma~3.1]{cr1006.5609}. 
\end{proof}
