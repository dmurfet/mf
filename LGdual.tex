...
\begin{align}
\At_2(X \otimes Y)^m( e_i \otimes f_j ) &= \sum_{p+q = m} \At_2(X)^p(e_i) \times \Atlarrow_2(Y)^q(f_j)\,,\label{eq:atshufat}\\
\At_1(X \otimes Y)^m( e_i \otimes f_j ) &= \sum_{p+q = m} (-1)^{pq} \At_1(X)^p(e_i) \times \Atlarrow_1(Y)^q(f_j)\label{eq:atshufat2}
\end{align}
...
\]
But considering \eqref{eq:powerofatiyah} it is straightforward to check that the right-hand side of~\eqref{eq:atshufat} is a sum over the same collection of terms, and the signs match, so that \eqref{eq:atshufat} holds. The situation for \eqref{eq:atshufat2} and $\At_1$ is similar. The only significant difference is that $s$ contributes an additional sign of parity $\binom{p+1}{2}$ to $\At_1(X)^p$, of parity $\binom{q+1}{2}$ to $\At_1(Y)^q$ and of parity $\binom{m+1}{2}$ to $\At_1(X \otimes Y)^m$ (compared to the signs on the $\At_2$ counterparts). The difference is compensated by the sign $(-1)^{pq}$ present in \eqref{eq:atshufat2}.
\end{proof}
...
\begin{gather}
\widetilde\coev_X(\gamma) = \sum_{l \ge 0} \varepsilon \left( \gamma \wedge  (-1)^l \Psi \Atlarrow_2(X^\vee)^l( \iota_X ) \right)\,, \label{eq:coev10}\\
\iota_X = \sum_{j} (-1)^{|e_j|} e_j^* \otimes e_j \in X^\dual \otimes_{k[z]} X \nonumber
\end{gather}
and
\begin{gather}
\coev_X( \gamma ) = \sum_{l \ge 0} \varepsilon \left( \gamma \wedge (-1)^l \Psi \Atlarrow_2(X)^l( \iota'_X ) \right)\,,\label{eq:coev102}\\
...
\begin{definition}\label{def:coeval} We define $\widetilde\coev_X: \Delta_W \lra X^\dual \otimes_S X$ to be the morphism in the homotopy category $\HMF(\Re, \widetilde{W})$ whose cohomology class maps to $1_X$ under the quasi-isomorphism \eqref{eq:isogivescoev}.
\end{definition}
...
\[
\xymatrix@C+1pc{
X^\vee \otimes_{S} X \ar[rr]^-{\Atlarrow_2(X^\vee)^m} & & \reprod_*(\Bar) \otimes_{\Re} ( X^\vee \otimes_{S} X ) \ar[r]^-{\Psi} & \Delta_W \otimes_{\Re} (X^\vee \otimes_{S} X)
}
\]
...
\end{equation}
Applying the swap isomorphism has the effect of exchanging $\At_2$ for $\Atlarrow_2$ in this formula, so that the cohomology class in $\Delta_W' \otimes_{\Re} (X^\vee \otimes_S X)$ mapping to $1_X$ is $\sum_{m \ge 0} (-1)^m \Psi \Atlarrow_2(X^{\vee})^m( \iota_X )$.
...
There is an alternative presentation which we will need. On $( X^\dual \otimes_R X) \otimes_{\Re } \Bar$ consider
\[
...
We conclude this section by sketching how one defines, and derives an explicit formula for, the other coevaluation. To begin with, one constructs a canonical homotopy equivalence
...
The partial derivatives $f_i = \partial_{x_i} W$ act null-homotopically on $X$ and we let $\lambda_i \in \Hom_{R \otimes_k S}(X,X)$ denote a degree-one map with $[d_X, \lambda_i] = f_i \cdot 1_X$. The construction will turn out to be independent of this choice up to homotopy, although this is not obvious. We also set $\Lambda = \lambda_1 \ldots \lambda_n$. For example, taking $\lambda_i = \partial_{x_i} d_X$ would do, but it will be important to allow other choices.
...
X \otimes_R X^{\vee}[n] \lto \left( X \otimes_R X^{\vee}[n] \right) \otimes_R \bar{R} \cong \bar{X} \otimes_R \bar{X}^{\vee}[n]
\ee
where we write $\bar{R} = R/(f_1,\ldots,f_n)$ and $\bar{X} = X \otimes_R \bar{R}$. Then we compose with
\be\label{eq:defneval0_0}
\xymatrix@C+2.5pc{
\bar{X} \otimes_R X^{\vee}[n] \cong \bar{X}[n] \otimes_R X^{\vee} \ar[r]^-{(-1)^n \Lambda \otimes 1} & \bar{X} \otimes_R X^{\vee}
}
\ee
which is closed because $\Lambda$ is a closed map $X[n] \lto X$ modulo the $f_i$. Finally we compose with
...
\[
\Res_{R \otimes_k S^e/S^e} \left[ \frac{ (-) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]: \bar{R} \otimes_K S \lto S\,.
\]
We note that the first map in \eqref{eq:defneval0_0} and the second in \eqref{eq:defneval0_1} are as defined in Section \ref{section:canonicalmaps}.

\begin{definition} $\eval_0$ is the composite of \eqref{eq:defneval0_00}, \eqref{eq:defneval0_0} and \eqref{eq:defneval0_1}. We write $\eval'_0$ for \eqref{eq:defneval0_1}.
\end{definition}

It is immediate that

\begin{lemma}\label{lemma:morphismevalzero} $\eval_0$ is a morphism of linear factorisations of $\widetilde{V}$ over $\Se $, and for $\eta \in X, \nu \in X^\dual$
\begin{align}
...
\xymatrix{
X \otimes_R X^\dual \ar[dr]_-{\eval_0} \ar@{.>}[rr]^-{\widetilde\eval_X} & & \Delta_V \ar[dl]^-{\pi_\Delta} \\
& S
}\,.
\ee
...
\[
\widetilde\eval_X: X \otimes_R X^\dual \lto \Delta_V
\]
defined for homogeneous $\eta \in X$ and $\nu \in X^\dagger$ by
\be\label{eq:constructevalmap0}
\widetilde\eval_X( \eta \otimes \nu ) = \sum_{m \ge 0} (-1)^{m+n+n|\eta|} \Res_{\Se  \otimes_k R/\Se } \left[ \frac{ \Psi \big( \lAt_2(X)^m \Lambda ( \eta ), \nu \big) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]
\ee
\end{proposition}

To be clear, the term in the numerator of the residue is the image of $\Lambda(\eta) \otimes \nu$ under the map
\[
\xymatrix@C+1pc{
X \otimes_R X^{\vee} \ar[r]^-{\lAt_2(X)^m} & \Bar \otimes_{S^e}(X \otimes_R X^{\vee} ) \ar[r]^-{1 \otimes (-,-)} & \Bar \otimes_{S^e} ( S^e \otimes_k R ) \cong \Bar \otimes_k R \ar[r]^-{\Psi \otimes 1} & \Delta_V \otimes_k R
}
\]
where $\lAt_2(X) = [d, d_X]$. The residue integrates out the $R$, leaving us with an element of $\Delta_V$.
