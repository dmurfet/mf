...
\ee
Unfortunately we do not know how to make sense of the right hand side, let alone prove it. We hope to return to this via a more thorough study of associative Atiyah classes. But let us ignore this problem for a moment and see where our daring gets us: we calculate that for $\varphi \in \End(X) \otimes_{R_1} \Bar$, there are homotopies of functionals of the variable $\varphi$
\begin{align*}
\dlangle \str \big( \lambda_1 \cdots \lambda_n \circ \At (\varphi) \big) \drangle &\simeq -\dlangle \str \big( \At(\lambda_1 \cdots \lambda_n) \circ \At^{n-1}(\varphi) \big) \drangle\\
&\simeq - \sum_{i=1}^n (-1)^{i+1}\dlangle \str \big( \lambda_1 \cdots \At(\lambda_i) \cdots \lambda_n \circ \At^{n-1}(\varphi) \big) \drangle\\
&\simeq -\sum_{i=1}^n (-1)^{i+1} \dlangle df_i \str \big( \lambda_1 \cdots \widehat{\lambda}_i \cdots \lambda_n \circ \varphi \big) \drangle\,.
\end{align*}
We will not make sense of these intermediate steps, but the initial and final terms are well-defined and we prove that they are homotopic functionals of $\varphi$ as the $p = 0$ case of Lemma \ref{lemma:zorromain} below. In this way we may, by omitting the ill-defined intermediate steps, proceed ``using'' the skew-symmetry \eqref{eq:untrueformula} to prove the Zorro move. 

Taking $\varphi = \At^{n-1}(\alpha)$ we prove in Lemma \ref{lemma:zorromain} each of the following homotopies
\begin{align*}
...
&\simeq \cdots\\
&\simeq (-1)^n \sum_{\sigma \in S_n} (-1)^{|\sigma|} \dlangle df_{\sigma(1)} \cdots df_{\sigma(n)} \str( \alpha ) \big) \drangle\\
&= (-1)^n \str( \alpha )
\end{align*}
...
\ee
The empty sequence $\emptyset$ is the unique sequence of length zero. We write $\Lambda_{i_1,\ldots,i_p}$ for the product $\Lambda = \lambda_1 \ldots \lambda_n$ with the $\lambda_{i_1},\ldots,\lambda_{i_p}$ omitted, and $f_i = \partial_{x_i} W$ as before.
...
The idea is to use the nondegenerate pairing on the mapping complexes of the dg-category of matrix factorisations  of $V-W$. If~$k$ is a field this is the Kapustin-Li pairing of \cite{kl0305,hl0404} but in general we use the contents of Section \ref{section:dualityadjointop}. The precise statement (Theorem \ref{theorem:generalkl}) is that there is a homotopy equivalence of $\mathbb{Z}_2$-graded complexes over $k$
\be
