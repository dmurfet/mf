...
\end{align}
So for example $d( a e_i \otimes \omega ) = (-1)^{|e_i|} e_i \otimes d( a \omega )$ for $a \in R^e$.
...
There are similar formulas for $\At_{1}$. We call these \emph{associative} Atiyah classes to distinguish them from the more standard Atiyah classes defined using commutative differential forms, but since the latter do not appear in this paper (with the exception of Appendix \ref{section:dualityadjointop}) we will omit the qualifier ``associative''. Without conditions on $D^2$ there is no interpretation of $\At_{2}$ as a cohomology class, so we should perhaps refer to Atiyah \emph{operators}, but we persist with the above terminology.

To discuss the linearity we need to be clear that $X \otimes \Bar$ is made into a right $R^e$-module via the right action of $R^e$ on $\Bar$ by multiplication. It is then apparent from \eqref{eq:powerofatiyah} that both $\At_{2}(X)$ and $\At_{1}(X)$ are right $R^e$-linear. The subscripts on the Atiyah classes arise because we think of $d$ (resp. $s$) as differentiating in the $R_1$-directions (resp. $R_2$-directions).

\begin{remark}\label{remark:linearatother} Suppose that $S$ is a $k$-algebra and $X$ a $\mathbb{Z}_2$-graded free $S \otimes_k R^e$-module with $S \otimes_k R^e$-linear operator $D$. If we choose a homogeneous $S \otimes_k R^e$-basis for $X$ and extend $d,s$ to $S$-linear operators on $X \otimes \Bar$ via \eqref{eq:extendd},\eqref{eq:extends} then $[d, D]$ and $[s, D]$ are $S$-linear and right $R^e$-linear operators on $X \otimes \Bar$ and we use the same notation and terminology for these operators. All our remarks extend \emph{mutatis mutandis} to this kind of Atiyah class.
\end{remark}
...
\end{equation}
From this expression it is clear that $\lAt_{2}$ and $\lAt_{1}$ are both left $R^e$-linear, using the action of left multiplication on $\Bar$ in $\Bar \otimes X$.
...
\]
But considering \eqref{eq:powerofatiyah} it is straightforward to check that the right-hand side of~\eqref{eq:atshufat} is a sum over the same collection of terms, and it can be checked that the signs match.
\end{proof}
...
Let $k$ be a ring and $R$ a $k$-algebra, $\Omega R = \Omega_k R$ the differential graded algebra $(\Omega R, d, \cdot)$ of non-commutative differential forms explained in Section \ref{subsec:Bar}. A \emph{connection} on a $\mathbb{Z}_2$-graded $R$-module $X$ is a $k$-linear map
\[
\nabla: X \lto X \otimes_R \Omega^1 R\,,
\]
which sends $X^i$ into $X^i \otimes_R \Omega^1_R$ and satisfies the graded Leibniz rule for $x \in X, a \in R$
\[
\nabla( x a ) = \nabla( x ) a + (-1)^{|x|} x da\,.
\]
Notice that by left and right multiplication $\Omega R$ is an $R$-bimodule, and so $X \otimes_R \Omega^1 R$ is naturally an $R$-bimodule. Let us write $R^e = R_1 \otimes_k R_2$ with $R_i = R$ so that the right $R$-module structure on $X \otimes_R \Omega^1 R$ is the action of $R_2$. The next lemma is an exercise in the Leibniz rule.
...
\begin{remark} To relate this to the earlier construction, observe that if $X$ is a free $R^e$-module with basis $\{ e_i \}_{i \in I}$ then $\Omega R$ can be identified with $\Bar$ and the operator $d: X \lto X \otimes \Bar_1 = X \otimes_R \Omega^1 R$ of \eqref{eq:extendd} is a connection.
\end{remark}
...
In the notation of Section \ref{section:atiyahclasses} we can describe the homotopy inverses of $\pi$ using Atiyah classes. Note that $X$ is a free $S \otimes_k R^e$-module so we are using the convention of Remark \ref{remark:linearatother}.
...
With these formulas in hand we return to the lifting problem discussed at the beginning of this section. We show that given $Y \in \hmf(R^e, \widetilde{W})$ composing with $\pi_{\Delta}$ induces a quasi-isomorphism
...
\begin{proposition}\label{prop:liftingresult} The map \eqref{eq:liftingresult0} is a homotopy equivalence and, in the above notation
...
\begin{definition}
The \emph{adjoint} of $X$ is $X^\dual = X^\vee[n] \in \hmf(k[x,z], W-V)$.
\end{definition}
...
\]
while simply stating the formulas for $\coev_X$ and $\eval_X$. To simplify the notation we set $R = k[x]$ and $S = k[z]$ so that $R^e = R_1 \otimes_k R_2$ with $R_i = R$ and $S^e = S_1 \otimes_k S_2$ with $S_i = S$. Then $\Delta_W$ and $X^\dual \otimes X$ are $R^e$-modules and $\widetilde\coev_X$ is $R^e$-linear, while $\Delta_V$ and $X \otimes X^\dual$ are $S^e$-modules and $\widetilde\eval_X$ is $S$-linear. The differentials on the matrix factorisations $\Delta_V, \Delta_W$ are given by \eqref{DeltaW}. We fix a homogeneous basis $\{ e_i \}_{i \in I}$ for $X$ with dual basis $\{ e_i^* \}_{i \in I}$.
...
\begin{equation}
\Hom_{R^e}( \Delta, X^\dual \otimes_{S} X ) \lto \Hom_{R \otimes_k S}(X,X)\,. \label{eq:isogivescoev}
\end{equation}
\end{proposition}
\begin{proof}
...
the operator
\[
\Atlarrow_{2}(X^\dual \otimes X) = [d, d_{X^\dual \otimes X}] = [d, d_{X^\dual} \otimes 1_X]
\]
...
In this section $\Delta$ denotes $\Delta_V$. The evaluation will be produced by writing down a natural morphism $\eval_0: X \otimes_R X^\dual \lto S$ and then lifting this via perturbation to the morphism $\widetilde\eval_X$ into $\Delta$. To begin with there is a canonical quotient (an $S^e$-linear morphism of linear factorisations of $\widetilde{V}$)
\be\label{eq:firstisoevalsec}
X \otimes_R (X^{\vee}[n]) \cong (X \otimes_R X^\vee)[n] \lto S \otimes_{S^e} (X \otimes_R X^{\vee})[n] \cong \Hom_{S \otimes_k R}(X,X)[n]\,.
\ee
sending $e_i \otimes e_j^*$ to $(-1)^{n|e_i|} e_i \circ e_j^*$.

The partial derivatives $f_i = \partial_{x_i} W$ act null-homotopically on $X$ and we let $\lambda_i \in \Hom_{R \otimes_k S}(X,X)$ denote a degree-one map with $[d_X, \lambda_i] = f_i \cdot 1_X$. We also set $\Lambda = \lambda_1 \ldots \lambda_n$. For example the operators $\lambda_i = \partial_{x_i} d_X$ would do, but it will be important to allow other choices as well. 
...
\begin{lemma}\label{lemma:morphismevalzero} There is a morphism of linear factorisations of $\widetilde{V}$ over $S^e$
\[
\eval_0: X \otimes_{R} X^\dual \lto S\,,
\]
defined for homogeneous $\eta \in X$ and $\nu \in X^\dual$ by
\begin{align}
\eval_0( \eta \otimes \nu ) &= (-1)^{\chi + n|\eta| +n} \Res_{S^e \otimes_k R/S^e} \left[ \frac{ \str( \Lambda \circ \eta \circ \nu) \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]\\
&= (-1)^{\chi + n|\eta| + |\nu| +n} \Res_{S^e \otimes_k R/S^e} \left[ \frac{ \nu \Lambda( \eta ) \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]
\end{align}
\end{lemma} 

There is a stabilisation morphism $\pi_\Delta: \Delta \lto S$, and while $X \otimes_R X^\dual$ is not free of finite rank over $S^e$, it is a direct summand of a finite-rank matrix factorisation in $\HMF(S^e, \widetilde{V})$. The unique lifting statement of Proposition \ref{prop:liftingresult} can be extended to summands (for the argument, see the proof of the proposition below) so there is up to homotopy a unique morphism of matrix factorisations $\widetilde\eval_X$ making the following diagram commute:
...
\begin{definition} $\widetilde\eval_X$ is the unique morphism in $\HMF(S^e, \widetilde{V})$ making \eqref{eq:defineeval} commute.
\end{definition}
...
\xymatrix@C+2pc{
\Hom_{S^e}( (X \otimes_R X^\vee)[n], \Delta ) \ar[d]_{\pi^\bullet_\Delta} \ar@<-1ex>[r]_-{\psi_\bullet} & \Hom_{S^e}(\bar{X} \otimes_{\bar{R}} \bar{X}^\vee, \Delta ) \ar@<-1ex>[l]_-{\vartheta_\bullet} \ar[d]^{\pi^\bullet_\Delta}\\
\Hom_{S^e}( (X \otimes_R X^\vee)[n], S ) \ar@<-1ex>[r]_-{\psi_\bullet} & \Hom_{S^e}( \bar{X} \otimes_{\bar{R}} \bar{X}^\vee, S) \ar@<-1ex>[l]_-{\vartheta_\bullet}
}\,.
\ee
where a ``$\bullet$'' as a superscript indicates post-composition and as a subscript it denotes pre-composition. If we ignore the first isomorphism of \eqref{eq:firstisoevalsec} which only contributes a sign, then $\eval_0$ is a closed element in the bottom left corner of the diagram. By Proposition \ref{prop:liftingresult} the right hand vertical map is a homotopy equivalence and therefore so is the left hand vertical map. This in particular justifies why in \eqref{eq:defineeval} there is a unique morphism $\widetilde\eval_X$ making the diagram commute : this map is the inverse image, in cohomology, of $\eval_0$ under $\pi_{\Delta}^\bullet$.
...
\[
\xymatrix{
\bar{X} \otimes_{\bar{R}} \bar{X}^\vee \ar[r] & S \otimes_{S^e} (\bar{X} \otimes_{\bar{R}} \bar{X}^\vee) \cong \Hom(\bar{X}, \bar{X}) \ar[r] & S
}
\]
defined by
\[
...
\]
We denote by $\eval'_{0,\textup{lift}}$ the morphism $\bar{X} \otimes_{\bar{R}} \bar{X}^\vee \lto \Delta$ lifting $\eval'_0$ which is produced by Proposition \ref{prop:liftingresult}. To run the construction we need to fix an $S^e$-basis for $\bar{X} \otimes_{\bar{R}} \bar{X}^\vee$. The basis we pick is $\{ g_\alpha e_i \otimes e_j ^* \}_{\alpha, i, j}$ where $g_\alpha$ gives a $k$-basis of $\bar{R}$. We define $\widetilde\eval_X = \vartheta_\bullet( \eval'_{0, \textup{lift}} ) = (-1)^n \eval'_{0, \textup{lift}} \circ (\Lambda \otimes 1)$. By construction this morphism makes \eqref{eq:defineeval} commute, and it just remains to compute it explicitly.

We have
\[
\widetilde\eval_X = \sum_{m \ge 0} (-1)^{m+n} \Psi \circ (1 \otimes \eval''_0) \circ [d, d_{X \otimes X^\vee}]^m \circ ( \Lambda \otimes 1 )
\]
where $\eval''_0: \bar{X} \otimes_{\bar{R}} \bar{X}^\vee \lto S^e$ is the $S^e$-linear map defined on the basis by
\[
...
\end{align*}
Applying $\eval''_0$ involves taking the supertrace $\str( e_{k_m} \circ e_j^* ) = (-1)^{|e_j|}\delta_{j,k_m}$ so that
\[
= \sum_{m \ge 0} \sum_{k,k_1,\ldots,k_m} (-1)^{m+n+n|e_i|+|e_j|+\chi} \Lambda_{ji} \Psi( d(D_{k_1,k}) \cdots d(D_{j,k_{m-1}}) ) \Res_{S \otimes_k R/S} \left[ \frac{ g \cdot \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]
\]
...
\end{tikzpicture}
\ee
Here we label the domains by the rings $R = k[x]$ and $S = k[z]$ pertaining to the two objects of $\LG$. We keep in mind that there are two $\Delta$'s which appear: one of which is an $R$-$R$-bimodule, and the other is an $S$-$S$-bimodule. We write $\Re = R_1 \otimes_k R_2$ with $R_i = R$ and $S^e = S_1 \otimes_k S_2$ with $S_i = S$.
...
\big\langle\!\big\langle \alpha \big\rangle\!\big\rangle = (-1)^{\chi} \Res_{R^e/R_2} \left[ \frac{ \varepsilon\Psi( \alpha ) \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]\,.
\end{gather*}
...
Let $\{ e_i \}_{i \in I}$ be a homogeneous $S \otimes_k R$-basis of $X$, $\{ e_i^* \}_{i \in I}$ the dual basis and $\{ e_{ij} = e_i \circ e_j^* \}_{i \in I, j \in J}$ the determined basis of $\End(X) = \Hom_{S \otimes_k R}(X,X)$. By the Atiyah class $\At = \At( \End(X) )$ of this complex we mean the operator on $\End(X) \otimes_{R_1} \Bar$ which would more properly be called the Atiyah class of $\End(X) \otimes_{R_1} R^e$ determined by the basis $e_{ij}$ using the convention of Remark \ref{remark:linearatother}.

Let $\lambda_i$ be a null-homotopy on $X$ for the action of $\partial_{x_i} W$, for example $\lambda_i=\partial_{x_i}d_X(x,z)$, and set 
$$
\Lambda = \lambda_1\ldots \lambda_n \, . 
$$
Post-composition with $\Lambda$ gives a well-defined operator on $\End(X) \otimes_{R_1} \Bar$.
...
&= \sum_j (-1)^{(n+1)|e_j|+\chi}\Res_{R^e/R_2}\! \left[ \frac{ \varepsilon\Psi \str\Big( \Lambda \circ \At^n( e_q \circ e_j^* ) \Big) \underline{\operatorname{d}\! x} }{\partial_{x_1}W \ldots \partial_{x_n} W} \right] \cdot e_j\\
...
&\qquad+ (-1)^{n-p+1} \bigdlangle \eta \str\big( \big[ D, \Lambda_{i_1,\ldots,i_p} \big]( d \varphi ) \big) \drangle \, . \label{eq:hutch3}
\end{align}
