...
There is something really silly about taking a factorisation $X$ over $k[z,x]$, tensoring with $\Delta$ which is a factorisation over $k[x,y]$ and noting that the canonical map
\[
\rho: X \otimes \Delta \lto X
\]
...
\begin{equation}
\Hom_{k[x]-k[x]-\textup{Bimod}}( \Delta, X^\dual \otimes_{k[z]} X ) \lto \Hom_{k[z,x]}(X,X)\,. \label{eq:isogivescoev}
\end{equation}
\end{proposition}
\begin{proof}
...
The idea is to use the nondegenerate pairing on $\hmf(k[z,x], V-W)$ to prove that $\mathcal{Z} \simeq 1_X$. If $k$ is a field this is the Kapustin-Li pairing of \cite{??,??} but in general we use the contents of Section \ref{??} (\textbf{more}). The upshot is that there is a homotopy equivalence of $\mathbb{Z}_2$-graded complexes over $k$
\be
\Hom_{k[z,x]}(X,X) \lto \Hom_k( \Hom_{k[z,x]}(X,X), k )[m+n]\,, \qquad \varphi \mapsto \langle \varphi, - \rangle_{\textup{KL}}\,.
\ee
To prove that $\mathcal{Z} \simeq 1_X$ it is therefore enough to prove that the functionals $\langle \mathcal{Z}, - \rangle_{\textup{KL}}$ and $\langle 1_X, - \rangle_{\textup{KL}}$ on $\Hom_{k[z,x]}(X,X)$ are \emph{homotopic}. When $k$ is a field this equivalent to showing that the two functionals take the same value on any closed endomorphism of $X$, and in general it amounts to showing that $\langle \mathcal{Z}, \varphi \rangle_{\textup{KL}}$ and $\langle 1_X, \varphi \rangle_{\textup{KL}}$ differ, as functionals of $\varphi$, by a functional of $D(\varphi)$. This will follow from a careful study of the map $\str( \mathcal{Z} \circ - )$ with the key point being an appropriate interpretation of (\ref{eq:trueproofeq}).
