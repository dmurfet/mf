...
We say a bicategory~$\mathcal B$ \textsl{has left adjoints} if for each 1-morphism $X\in \mathcal B(A,B)$ there exists a 1-morphism ${}^\dual X \in \mathcal B(B,A)$ together with 2-morphisms (\textbf{todo} $\circ$ not $\otimes$ here?)
\be\label{evcoev}
...
Let $W \in R$ be arbitrary and set $\widetilde{W} = W \otimes 1 - 1 \otimes W \in \Re$. Our first observation is that $X \,\widehat{\otimes}\, \Bar$ can be equipped as a linear factorisation of $U + \widetilde{W}$. One checks that there are well-defined operators on $X \,\widehat{\otimes}\, \Bar$ defined by
\begin{align*}
