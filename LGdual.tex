...
\begin{definition}
The \emph{associative Atiyah classes} of $(X,D)$ are operators on $X \otimes_{R^e} \Bar$ defined by
\begin{align*}
...
In addition to these four distinct kinds of Atiyah classes, we will sometimes need purely cosmetic variants where we switch the order of elements of $X$ and $\Bar$ in the tensors. This creates an awkward situation with the notation, since we want elements of $R^e$ traversing the tensor product to act on the same side of $\Bar$ as before. The clearest way to indicate this is to use the twist isomorphism
\begin{equation}\label{eq:attau}
\tau: R^e \otimes_k R^e \lto R^e \otimes_k R^e, \qquad \tau ( a \otimes a' ) = a' \otimes a
\end{equation}
so that in $\tau_*( \Bar ) \otimes_{R^e} X$ we have $\omega \otimes a x$ = $a \omega \otimes x$. If we extend $d$ and $s$ to operators on $\tau_*( \Bar ) \otimes_{R^e} X$ as before, then we define operators on this tensor product by
\[
...
X \otimes_{R^e} \Bar \ar[d]_{\At_{R^e/R_2}(X)^m} \ar[r]^-{\cong} & \tau_*( \Bar ) \otimes_{R^e} X \ar[d]^{\Atlarrow_{R^e/R_2}(X)^m}\\
X \otimes_{R^e} \ar[r]_-{\cong} & \tau_*( \Bar ) \otimes_{R^e} X
}
\]
...
Let $k$ be a ring and $A$ a $k$-algebra, $\Omega A = \Omega_k A$ the differential graded algebra $(\Omega A, d, \dot)$ of noncommutative differential forms explained in Section \ref{??}. A \emph{connection} on a $\mathbb{Z}_2$-graded $A$-module $X$ is a $k$-linear map
\[
...
As part of the structure of the bicategory $\LG$ we have specified for any $1$-morphism $X \in \LG(W,V)$ a pair of isomorphisms
...
\end{equation}
where $\Psi$ is the canonical map given in (\ref{intro_psi}) and $\rho'$ is explained below. Rather than trying to invert~$\rho$ directly, it turns out to be easier to invert $\rho'$ and then compose with $\Psi$. Roughly speaking the inverse of $\rho'$ is the geometric series in powers of the Atiyah class of $X$ and we make use of a completion of $X \otimes \Bar$ in order to guarantee that this geometric series converges.
...
\begin{proposition}\label{prop:finalpertdefo} There is an $R_2$-linear deformation retract datum
\[
\xymatrix@C+2pc{
...
\]
In particular, $\pi$ is an $R_2$-linear homotopy equivalence with inverse $\sigma_\infty$.
\end{proposition}
\begin{proof}
...
\begin{proposition}\label{prop:finalpertdefo} There is a left $R_1$-linear deformation retract datum
\[
\xymatrix@C+2pc{
(X \otimes R, D \otimes 1) \ar@<-1ex>[r]_-{\sigma_\infty} & (X \, \widehat{\otimes} \, \Bar, D + b' + d\widetilde{W} \times (-)) \ar@<-1ex>[l]_-{\pi}
}
...
\]
In particular, $\pi$ is a left $R_1$-linear homotopy equivalence with inverse $\sigma_\infty$.
\end{proposition}
\begin{proof}
\textbf{todo}.
\end{proof}
...
With $\tau$ as in (\ref{eq:attau}) we consider on the tensor product
\[
\tau_*(\Bar) \otimes_{\Re} ( X^\dual \otimes_{k[z]} X)\,
\]
the operator
\[
...
\[
\xymatrix@C+1pc{
X^\dual \otimes_{k[z]} X \ar[rr]^-{\Atlarrow(X^\dual)^m} & & \tau_*(\Bar) \otimes_{\Re} ( X^\dual \otimes_{k[z]} X ) \ar[r]^-{\Psi} & K \otimes_{\Re} (X^\dual \otimes_{k[z]} X)
}
\]
