...
On general grounds it is expected that the bicategorical description of TFTs with defects involves additional structure. For example, the fact that we consider \textsl{oriented} defect lines should give rise to adjunctions on the level of 1-morphisms: the ``same'' defect line with reversed orientation should be adjoint to the defect with opposite orientation. In the case of Landau-Ginzburg models this means that for every matrix factorisation~$X$ of $V-W$ there should be a matrix factorisation~$X^\dual$ of $W-V$ together with evaluation and coevaluation maps
\be\label{evcoevintro}
\widetilde\eval_X: X \otimes X^\dual \lra \Delta 
\, , \qquad
\widetilde\coev_X: \Delta \lra X^\dual \otimes X
\ee
that are the counit and unit of an adjunction between~$X$ and~$X^\dual$. Similarly, one expects a left adjoint~${}^\dual X$ of~$X$. The special case where~$V$ and~$W$ are the same polynomial and depend on only one variable was worked out in~\cite{cr1006.5609}; we refer to that paper's introductory section for further motivation from physics.

In the present paper we prove that every $1$-morphism $X$ in $\LG$ has left and right adjoints given by ${}^\dual X = X^{\vee}[m]$ and $X^\dual = X^{\vee}[n]$ where $W \in k[x_1,\ldots,x_n], V \in k[z_1,\ldots,z_m]$ and $X^{\vee} = \Hom_{k[x,z]}(X, k[x,z])$ is the dual factorisation of $W - V$. 

Our focus is on providing explicit expressions for the above evaluation and coevaluation maps. For example if $\gamma$ is an element of the exterior algebra $\Delta$, and $\gamma' = \theta_{b_1} \ldots \theta_{b_l}$ is the unique form with $\gamma \wedge \gamma' = (-1)^{s(\gamma)} \theta_1 \ldots \theta_n$ and $b_1 < \cdots < b_l$ for some $s(\gamma) \in \mathbb{Z}_2$ then
\[
\widetilde\coev_X( \gamma ) = \sum_{i,j} (-1)^{(l+1)|e_j| + s(\gamma)} \big\{ \partial_{[b_l]}(d_X) \ldots \partial_{[b_1]}(d_X) \big\}_{ji} e_i^* \otimes e_j\,.
\]
