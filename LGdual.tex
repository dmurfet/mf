...
\usepackage{dsfont}
\usepackage{tikz}
\def\nicecolourscheme{\shadedraw[top color=blue!22, bottom color=blue!22, draw=blue!50!black, dashed]}
\def\nicenocolourscheme{\shadedraw[top color=gray!2, bottom color=gray!25, draw=gray!50!black, dashed]}
\def\nicereallynocolourscheme{\shadedraw[top color=white!2, bottom color=white!25, draw=white!50!white, dashed]}
...
In the cases of interest to us the sum $\sum_{m \ge 0} (\delta h)^m$ converges, and so $\tau = \sum_{m \ge 0} (\delta h)^m \delta$ and
\[
...
The bulk of the argument is given in the following setting: $k$ is an arbitrary commutative ring, $A, R, S$ are $k$-algebras and we assume given a morphism of $k$-algebras $\theta: R \lto \Ae$ and
\[
X \in \HMF(S \otimes_k R, V)
\]
together with a homogeneous basis $\{ e_i \}_{i \in I}$ of $X$ over $S \otimes_k R$. Let $D$ denote the differential on $X$. The two cases of interest are when $R = \Ae, S = k[z]$ and $\theta$ is the identity, or, the situation of (\ref{eq:pertdia1}) where $R = A$ and $\theta(a) = a \otimes 1$. We return to these examples at the end of the section.

Let $W \in A$ be arbitrary and set $\widetilde{W} = W \otimes 1 - 1 \otimes W \in \Ae$. For convenience in distinguishing between the left and right copy of $A$ in $\Ae = A \otimes_k \Aop$ we write $\Aop$ even though $A$ is commutative. We assume that $V + \widetilde{W}$ belongs to the image of $S \otimes_k \Aop$ in $S \otimes_k \Ae$, where $V$ denotes the image of $V$ under $S \otimes_k R \lto S \otimes_k \Ae$ via $\theta$.

Consider the $S \otimes_k \Ae$-module
\[
...
\]
As a $\mathbb{Z}_2$-graded $S \otimes_k \Ae$-module, $X \,\widehat{\otimes}_R\, \Bar$ is the inverse limit of the system
\[
\cdots \lto X \otimes_R \Bar/\Bar_{\ge 2} \lto X \otimes_R \Bar/\Bar_{\ge 1}
\]
where $\Bar_{\ge m} = \bigoplus_{i \ge m} \Bar_i \subseteq \Bar$. Our first observation is that this graded module can be equipped as a linear factorisation of $V + \widetilde{W}$ over $S \otimes_k \Ae$. One checks that there are well-defined operators
\begin{align*}
...
\end{align*}
on $X \,\widehat{\otimes}_R\, \Bar$, where $d$ is extended to an $S \otimes_k \Aop$-linear operator on $X \otimes_R \Bar$ by the rule
\[
d( e_i \otimes \omega ) = (-1)^{|e_i|} e_i \otimes d(\omega)\,.
\]

\begin{lemma} $( X \, \widehat{\otimes}_R\, \Bar, D + b' + d\widetilde{W} \times (-) )$ is a linear factorisation of $V + \widetilde{W}$ over $S \otimes_k \Ae$.
\end{lemma}
\begin{proof}
...
\]
where the first map is the projection and the second is the product $\Ae \lto A$. Next we show using the perturbation lemma that this map is a homotopy equivalence, and we give an explicit homotopy inverse in terms of the Atiyah class $\At_X = [d, D]$ on $X \otimes_R \Bar$. When $R = A$ and $\theta(a) = a \otimes 1$ this map $\pi$ is the morphism $\rho_{\Bar}$ mentioned above.

We begin with the deformation retract which exists due to the defining relation of $d$ with $b'$.

\begin{lemma}\label{lemma:firstdefo} There is an $S \otimes_k \Aop$-linear deformation retract datum of $\mathbb{Z}_2$-graded complexes
\[
\xymatrix@C+2pc{
(X \otimes_R A, 0) \ar@<-1ex>[r]_-{\sigma} & (X \, \widehat{\otimes}_R \, \Bar, b') \ar@<-1ex>[l]_-{\pi}
}%
\!\!\!\xymatrix{%
{}\ar@(ur,dr)[]^{-d}
}%
...
\begin{proposition}\label{prop:finalpertdefo} There is an $S \otimes_k \Aop$-linear deformation retract datum
\[
\xymatrix@C+2pc{
...
\pi: X \otimes_R \Delta \lto X \otimes_R \Ae \lto X \otimes_R A
\end{equation}
where the first map is the projection to $\Delta_0 = \Ae$ and the second map is the product. This is compatible with the map $\pi$ defined for the bar resolution of the diagonal in the sense that there is a commutative diagram
\[
\xymatrix{
...
\begin{lemma}\label{lemma:koszulstab1} The map $\pi$ of (\ref{eq:koszulpi}) is an $S \otimes_k \Aop$-linear homotopy equivalence.
\end{lemma}
\begin{proof}
...
\[
\xymatrix@C+2pc{
(X \otimes_R A, 0) \ar@<-1ex>[r]_-{\sigma} & (X \otimes_R \Delta, 1 \otimes \delta_{-}) \ar@<-1ex>[l]_-{\pi}
}%
\!\!\!\xymatrix{%
{}\ar@(ur,dr)[]^{-h}
}%
\]
into an $S \otimes_k \Aop$-linear deformation retract datum. Moreover $\delta = D \otimes 1 + 1 \otimes \delta_{+}$ is a small perturbation and the perturbation lemma applies to show that $\pi$ is a homotopy equivalence.
\end{proof}

\begin{corollary}\label{corollary:koszulstab2} An $S \otimes_k \Aop$-linear homotopy inverse to $\pi$ of (\ref{eq:koszulpi}) is given by
\[
...
%\]
As recalled in Section~\ref{subsec:bicatLG} there are two coevaluation $2$-morphisms associated with~$X$. The derivation is very similar in both cases, so we give the full details for 
\[
\widetilde\coev_X: \Delta_W \lto X^\dual \otimes_{k[z]} X
\]
and leave $\coev_X$ to the reader. Recall that $\Delta_W$ and $X^\dual \otimes X$ are $k[x]$-bimodules and the coevaluation is $k[x]$-bilinear. In the remained of this section often allow ourselves to simply write~$\Delta$ for $\Delta_W$. 

At this point the coevaluation morphism we construct would be more accurately described as a \textsl{candidate} for the coevaluation, as it is only later in Section~\ref{sec:Zorro} that we show that our candidate is actually the unit of an adjunction by proving the Zorro moves.
...
\]
Composing with the natural morphism $\varepsilon: K \lto \Re[n]$ and taking the adjoint,
\[
\zeta: \Delta' \lto \Delta^\vee [n]\, , \qquad \omega \lmt \varepsilon( \omega \wedge - )
\]
is an isomorphism of matrix factorisations. Combining this with various canonical maps and the morphism $\pi: \Delta' \lto \Delta'_0 = \Re \lto R$ we have the following diagram of \textsl{right} $R$-linear morphisms of complexes: 
\begin{equation}
\xymatrix{
\Hom_{\Re}( \Delta, X^\vee[n] \otimes_{k[z]} X)\\
\Delta^\vee \otimes_{\Re} \left( X^\vee[n] \otimes_{k[z]} X \right) \ar[u]^-{\cong}_-{\xi}\\
\left( X^\vee[n] \otimes_{k[z]} X \right) \otimes_{\Re} \Delta^\vee \ar[u]^-{\cong}_-{\textup{twist}}\\
...
\ee
which we prove is homotopic to the identity on $X$. Note that here we already started to discard subscripts that are clear from the context, e.\,g.~$\widetilde\eval = \widetilde\eval_X$. 
...
Let us introduce the notation (TODO: keep or change due to clash with shadows?)
$$
...
\widetilde{\eval}: X \otimes X^{\dagger} \lto \Delta, \qquad \widetilde{\eval}(\alpha) &= (-1)^{\binom{n+1}{2}}\Res_{k[x,y,z]/k[y,z]}\! \left[ \frac{\str ( \alpha|_{z=z'} \underline{\lambda}) \underline{\operatorname{d}\! x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right] + \mathcal{O}(\theta)\,.
\end{align*}
Note that tensoring with the diagonal defines a map $(-)|_{z=z'}: X \otimes X^{\dagger} \lto \Hom_{k[x,z]}(X,X)$ (\textbf{more} there is no longer a $z'$ -- so there's no need to mention it!) and $\mathcal{O}(\theta)$ denotes higher order terms which do not survive $\lambda$ and therefore may be ignored for present purposes. Also define $\nabla$ and Atiyah classes (\textbf{todo}).
...
\end{align} 
In the first step we use that $\Psi$ intertwines the shuffle product on $\Bar$ with the exterior product on~$\Delta$, and in the last step we use Lemma \ref{lemma:shuffleatiyah} below, whose proof is delayed to the end of the section. Applying $\widetilde\eval$ in the first two components of $X \otimes X^{\dagger} \otimes X$ to the expression in (\ref{firstzorromaps}) has the effect of identifying $z$ with $z'$ (TODO: not necessary). This changes the associative Atiyah class of $X \otimes X^{\dagger}$ to the associative Atiyah class of $\End(X)$ and so
\begin{align}
...
\]
where the sign counts the number of $\lambda$'s to the left of $\lambda_a$ in $\underline{\lambda}_{i_1,\ldots,i_p}$. Substituting, the last summand (\ref{eq:hutch3}) in the above is, up to a sign, 
\[
...
\[
\big\langle\!\big\langle \str \big( \underline\lambda \At^n (\varphi)\big) \big\rangle\!\big\rangle = \str(\varphi)\big|_{x \lmt y} + \sum_{p=0}^{n-1} h_i( D \At^{n-p-1}(\varphi) )\,.
\]
\end{lemma}
\begin{proof}
Applying Lemma \ref{lemma:zorromain} repeatedly yields
\begin{align*}
\big\langle\!\big\langle \str \big( \underline\lambda \At^n (\varphi)\big) \big\rangle\!\big\rangle = (-1)^n \sum_{\sigma \in S_n} (-1)^{|\sigma|} \big\langle\!\big\langle \nabla f_{\sigma(1)} \cdots \nabla f_{\sigma(n)} \str(\varphi) \big\rangle\!\big\rangle + \sum_{p=0}^{n-1} h_i( D \At^{n-p-1}(\varphi) )\,.
\end{align*}
%where 
%$$
%\delta = 
...
The idea is to use the nondegenerate pairing on the morphism spaces of $\hmf(k[x,z], V-W)$. If~$k$ is a field this is the Kapustin-Li pairing of \cite{??,??} but in general we use the contents of Section \ref{??} (\textbf{more}). The precise statement (Theorem \ref{??}) is that there is a homotopy equivalence of $\mathbb{Z}_2$-graded complexes over $k$ (TODO: write $\langle \varphi, - \rangle_{X}$ instead of $\langle \varphi, - \rangle_{\textup{KL}}$ everywhere?)
\be
\Hom_{k[x,z]}(X,X) \lto \Hom_k( \Hom_{k[x,z]}(X,X), k )[m+n]\,, \qquad \varphi \lmt \langle \varphi, - \rangle_{\textup{KL}}\,.
\ee
induced by the pairing
\begin{equation}\label{eq:bracketkl}
\langle -, - \rangle_{\textup{KL}} = \frac{1}{n!} \sum_{\sigma \in S_n} (-1)^{|\sigma|} \Res_{k[y,z]/k} \left[ \frac{ \str( - \circ - \circ \underline{\lambda} \underline{\mu}) \underline{\operatorname{d}\!y} \underline{\operatorname{d}\!z}}{\partial_{x_1}W \ldots \partial_{x_n} W \partial_{z_1} V \ldots \partial_{z_n} V} \right]
\end{equation}
where $\underline{\lambda} = ??$ and $\underline{\mu} = ??$. To prove that $\mathcal{Z} \simeq 1_X$ it is therefore enough to prove that the functionals $\langle \mathcal{Z}, - \rangle_{\textup{KL}}$ and $\langle 1_X, - \rangle_{\textup{KL}}$ on $\Hom_{k[x,z]}(X,X)$ are homotopic, or what is the same, that $\langle H, \varphi \rangle_{\textup{KL}}$ can be written as a function of $D(\varphi)$. This will follow from a careful study of the map $\str( H \circ - )$.
...
& = \sum_{i,j} (-1)^{|e_i| + |e_j| + n} e^*_i \Big( \big\langle\!\big\langle \str( \underline\lambda \At^n (\varphi(e_i)\otimes e^*_j \otimes e_j)) \big\rangle\!\big\rangle \Big) - \str(\varphi)\nonumber \\
& = \sum_{i,j} (-1)^{|e_i| + |e_j| + n + n|e_j|} e^*_i \Big( \big\langle\!\big\langle \str( \underline\lambda \At^n (\varphi(e_i)\otimes e^*_j )) \big\rangle\!\big\rangle e_j \Big) - \str(\varphi)\nonumber \\
& = \big\langle\!\big\langle \str \big( \underline\lambda \At^n \big(\sum_i \varphi(e_i)\otimes e^*_j \big)\big) \big\rangle\!\big\rangle - \str(\varphi)\nonumber \\
& = \big\langle\!\big\langle \str \big( \underline\lambda \At^n (\varphi)\big) \big\rangle\!\big\rangle - \str(\varphi) \nonumber\\
...
\end{theorem}
\begin{proof}
As we have already mentioned, it suffices by nondegeneracy of \eqref{eq:bracketkl} to prove that $\langle H, - \rangle_{\textup{KL}}$ is a null-homotopic function $\Hom_{k[x,z]}(X,X) \lto k$. For $\varphi' \in \End(X)$ the numerator in the residue which computes $\langle H, \varphi' \rangle_{\textup{KL}}$ is, using \eqref{strlambdaAtn}, a differential form multiplied by
\[
\str( H \varphi' \underline{\lambda} \underline{\mu}) = \sum_{p=0}^{n-1} h_p( D \At^{n-p-1}(\varphi' \underline{\lambda} \underline{\mu}) )
\]
...
&\qquad + (-1)^{|\phi'|} h_p \At^{n-p-1}\big( \varphi' D(\underline{\lambda}) \underline{\mu} ) \label{eq:mainzorro4}\\
&\qquad + (-1)^{|\phi'| + n} h_p \At^{n-p-1}\big( \varphi' \underline{\lambda} D(\underline{\mu}) ) \, . \label{eq:mainzorro5}
\end{align}
...
\[
\langle H, - \rangle_{\textup{KL}} = 
\frac{1}{n!} \sum_{\sigma \in S_n}\sum_{p=0}^{n-1} (-1)^{|\sigma|} \Res_{k[y,z]/k} \left[ \frac{ h_p \At^{n-p-1}( D(-) \underline{\lambda} \underline{\mu} ) \underline{\operatorname{d}\!y} \underline{\operatorname{d}\!z}}{\partial_{x_1}W \ldots \partial_{x_n} W \partial_{z_1} V \ldots \partial_{z_n} V} \right]\,.
\]
...
We note that the maps~\eqref{betaQ} can be recovered from the duality and shadow structure of $\LG$ as follows. On the one hand we have 
$$
...
\draw (0,0) circle (2);
\draw[->, very thick] (0,2) -- (-0.001,2) node[above] {{\small$\eval_Y$}}; 
\draw[->, very thick] (0,-2) -- (0.001,-2) node[below] {{\small$\widetilde\coev_Y$}}; 
\fill (45:2) circle (2.5pt) node[right] {{\small$\psi$}};
%
\draw (0,0) circle (1);
\draw[->, very thick] (0,1) -- (0.001,1) node[above] {{\small$\widetilde\eval_X$}}; 
\draw[->, very thick] (0,-1) -- (-0.001,-1) node[below] {{\small$\coev_X$}}; 
...
To put ourselves in a position to better understand and manipulate~\eqref{annulus} let us spell out its constituents. From the general expressions~\eqref{TODO} for evaluations and coevaluations we find 
\begin{align}
& \widetilde\eval_X = \str_k(-) \, , \qquad \eval_Y = (-1)^{n+1\choose 2} \Res_{k[x]/k} \left[ \frac{\str_k( \, - \circ \Lambda_Y)}{\partial_1 W\ldots \partial_n W} \right] \, , \nonumber \\
...
We will now compute the diagram~\eqref{annulus} in two different ways and then compare the results. One possibility is to first collapse the $X$-loop to obtain a ``$(k[x],W)$-bubble'', and then collpase its boundary loop~$Y$ in turn to arrive at an expression for $C:k \lra k$. It follows from the general results of Section~\ref{sec:defectaction} or directly from~\eqref{evcoeveasy} that the first step of collapsing~$X$ produces the 2-morphism $\str_k(\varphi \Lambda_X)\in\End(\Delta_W)$. Similarly, the second step of collapsing~$Y$ produces a residue: 
$$
C 
= 
...
\fill (1.5,0) circle (0pt) node[white] {{\small$W$}};
\draw (0,0) circle (2);
\draw[->, very thick] (0,2) -- (-0.001,2) node[above] {}; 
\draw[->, very thick] (0,-2) -- (0.001,-2) node[below] {}; 
\fill (45:2) circle (2.5pt) node[right] {{\small$\psi$}};
%
\draw (0,0) circle (1);
\draw[->, very thick] (0,1) -- (0.001,1) node[above] {}; 
\draw[->, very thick] (0,-1) -- (-0.001,-1) node[below] {}; 
\fill (135:1) circle (2.5pt) node[left] {{\small$\varphi$}};
\end{tikzpicture} 
= 
\begin{tikzpicture}[very thick,scale=0.6,color=blue!50!black, baseline,>=stealth]
...
\end{tikzpicture} 
= 
(-1)^{n+1\choose 2} \Res_{k[k]/k} \left[ \frac{\str_k \big( \str_k( \varphi \Lambda_X) \psi \Lambda_Y \big)}{\partial_1 W \ldots \partial_n W} \right] \, .
$$
Using the expressions $\beta^X(\varphi) = (-1)^{n\choose 2} \str_k(\varphi \Lambda_X)$ and $\beta^Y(\psi) = (-1)^{n\choose 2} \str_k(\psi \Lambda_Y)$ for the boundary-bulk maps from Section~\ref{sec:shadows} we thus find from the above that
\be\label{C1}
C = 
...
$$
\eval_{X^\dual \otimes Y} = \str_k(-) \, , \qquad \widetilde\coev_{X^\dual \otimes Y} : 1 \lmt 1_{\End(X^\dual \otimes Y)}
$$
to arrive at
\be\label{C2}
C 
...
\end{tikzpicture} 
= 
\str_k (\psi \circ (-) \circ \varphi ) \, .
\ee
Here the second step follows from the general calculus of bicategories with duals; see e.\,g.~\cite[Lemma~3.1(iii)]{cr1006.5609} for a detailed proof. Comparing~\eqref{C1} and~\eqref{C2} we thus observe: 
...
\varphi \lmt \langle \varphi, - \rangle_{\textup{KL}}
\end{gather*}
...
\end{gather*}
This connection connection has components $\partial_{f_i}$ defined by $\partial_{f_i}( r e_{ji} ) = \partial_{f_i}(r) \cdot e_{ji}$ which are $k$-linear operators on $\Hom(X,Y)$. The \textsl{Atiyah class} of $\Hom(X,Y)$ is the commutator
\[
...
\]
and to this direct sum decomposition corresponds $2^n$ orthogonal $k$-linear idempotents on $\bar{Z}$.
...
\]
or we can choose to project out $\theta_n$ by applying $\rho$ instead. If after ``keeping'' $\theta_n$ we proceed to apply the $\varepsilon$ projections for $\theta_{n-1},\ldots,\theta_1$ in that order, we obtain a split epimorphism $\varepsilon: Z \otimes K(f) \lto Z[n]$ defined by $\varepsilon(x \theta_1 \cdots \theta_n) = (-1)^{n|x|} x$ with left inverse $\vartheta$. The formula given in \cite[Section 4]{dm1102.2957} depends on a chosen ordering of the $\lambda$'s, but we can symmetrise over all permutations to obtain the left inverse
\[
...
If instead we project out with $\rho$ at each step we obtain a split epimorphism
\[
Z \otimes K(f) \lto Z,
\]
with left inverse the inclusion $\kappa(x) = x \otimes 1$. The precise formula depends on the order in which we choose to project out the $\theta_i$. If for $\sigma \in S_n$ we project 

 and if we symmetrise over all possible permutations we obtain the morphism defined for $i_1 < \cdots < i_p$ and $x \in Z$ by
\[
\rho( x \theta_{i_1} \cdots \theta_{i_p} ) = (-1)^{p|x| + \binom{p}{2}} 
\]
