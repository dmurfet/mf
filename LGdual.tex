...
\]
Given $1$-morphisms $f: A \lto B$ and $g: B \lto C$ we write $g \circ f$ for their \textsl{composite} $c_{ABC}( f, g )$, and given $2$-morphisms $\gamma: f \lto f'$ and $\delta: g \lto g'$ we write $\delta * \gamma$ for $c_{ABC}(\gamma, \delta)$.
...
\be\label{evcoev}
\eval : g \circ f \lra \Delta_A \, , \qquad \coev : \Delta_B \lra f \circ g
\ee
satisfying 
\begin{align}
\rho_f \circ (1_f * \eval) \circ \alpha_{f,g,f} \circ (\coev * 1_f) \circ \lambda_f^{-1} & = 1_f \, , \label{uglyZorro1}\\
\lambda_{g} \circ (\eval * 1_{g}) \circ \alpha^{-1}_{g,f,g} \circ (1_{g} \otimes \coev) \circ \rho_{g}^{-1} & = 1_{g} \, . \label{uglyZorro2}
\end{align}
Equivalently, the following two composites evaluate to the identity:
\begin{align}
\xymatrix@C+1.5pc
{
f \ar[r]^-{\lambda_f^{-1}} & \Delta_B \circ f \ar[r]^-{\coev * 1_f} & ( f \circ g ) \circ f \ar[r]^-{\alpha_{f,g,f}} & f \circ ( g \circ f ) \ar[r]^-{1_f * \eval} & f \circ \Delta_A \ar[r]^-{\rho_f} & f
}\,,\\
\xymatrix@C+1.5pc
{
g \ar[r]^-{\rho_g^{-1}} & g \circ \Delta_B \ar[r]^-{1_g * \coev} & g \circ ( f \circ g ) \ar[r]^-{\alpha^{-1}_{g,f,g}} & (g \circ f) \circ g \ar[r]^-{\eval * 1_g} & \Delta_A \circ g \ar[r]^-{\lambda_g} & g
}\,.
\end{align}
In this case we say that $g$ is \emph{left adjoint} to $f$ and that $f$ is \emph{right adjoint} to $g$, and we write $g \dashv f$. The $2$-morphisms $\eval$ and $\coev$ are referred to as the \textsl{evaluation} and \textsl{coevaluation} maps.
\end{definition}
...
\be\label{evcoev2}
\eval_f : {}^\dual f \circ f \lra \Delta_A \, , \qquad \coev_f : \Delta_B \lra f \circ {}^\dual f\,.
\ee
For the adjunction $f \dashv f^\dual$ we write the evaluation and coevaluation maps as
\be\label{evcoev3}
\widetilde\eval_f : f \circ f^\dual \lra \Delta_B \, , \qquad \widetilde\coev_f : \Delta_A \lra f^\dual \circ f\,.
\ee
It is convenient to denote identities in bicategories with adjoints in string diagram notation. We will explain a refined version of this notation in Section \ref{??}, but for the moment string diagrams have the meaning given in \cite{JSGoTCI,JSGoTCII} and also explained very clearly in \cite{ladia,khovdia}. In this language the evaluation and coevaluation maps~\eqref{evcoev2} are written as
$$
\eval_{f} = 
%%%%%%%%%%%%%%%%%%%%%%
...
Let $\cat{B}$ be a bicategory. The following lemma recasts the definition of adjunction given in Section \ref{subsec:bicat} in terms of a natural isomorphism. Since the proof is essentially the same as the usual proof for adjoint functors, we omit it. Observe however that we insist on naturality with respect to both $1$ and $2$-morphisms. Throughout this section we write $\Hom(f,g)$ for $\Hom_{\cat{A}(A,B)}(f,f')$.

\begin{lemma} Let $f: A \lto B$ and $g: B \lto A$ be $1$-morphisms in $\cat{B}$. Specifying $2$-morphisms $\eval, \coev$ satisfying \eqref{uglyZorro1}, \eqref{uglyZorro2} is equivalent to specifying a family of bijections
\[
\Phi_{h, l}: \Hom(g \circ h, l) \lto \Hom(h, f \circ l),
\]
...
\[
\xymatrix@C+1pc{
\Hom(g \circ h, l) \ar[d]\ar[r]^-{\Phi} & \Hom(h, f \circ l) \ar[d]\\
\Hom( (g \circ h) \circ h', l \circ h') \ar[d] & \Hom(h \circ h', (f \circ l) \circ h') \ar[d]\\
\Hom( g \circ (h \circ h'), l \circ h') \ar[r]^-{\Phi} & \Hom(h \circ h', f \circ (l \circ h') )
}
\]
\end{itemize}
\end{lemma}
...
The differential on $\Delta_W$ is given by (\textbf{todo} contraction)
\be\label{DeltaW}
...
One of the main results of the present paper is that $\LG$ (and also $\LG^{\mathrm{gr}}$) is endowed with an additional duality structure. Let us recall the relevant notion (\textbf{todo} now moved).
...
Let us for the moment be slightly more general and consider an arbitrary unital associative $k$-algebra~$A$. In our applications to matrix factorisations we will set $A=R=k[x]$. (\textbf{todo} ref Quillen, others)
...
\end{align}
These maps were studied in~\cite{sw0911.0917}, we only rephrase the presentation of~$\Psi$ in terms of the difference quotient operators~$\partial_{[i]}$ suitable for our setting. One easily verifies that $\Psi\Phi = 1_\Delta$. (\textbf{todo} Discuss possible reorderings, linearity)
...
In the notation of Section \ref{section:atiyahclasses} we can now describe the homotopy inverses of $\pi$ using Atiyah classes. Note that $X$ is a free $(S \otimes_k \Re)$-module so we are using the convention of Remark \ref{remark:linearatother}. (\textbf{todo} Are we clear enough what $\At_2$ means on the completed tensor product?)
...
\ee
In particular, given a morphism of linear factorisations $\varphi: Y \lto R$ there is a unique (up to homotopy) morphism $\varphi_{\textup{lift}}$ making the following diagram commute up to homotopy
\[
\xymatrix{
...
\begin{remark}
For each $l \ge 0$ the summand of $\varphi_{\textup{lift}}$ is $(-1)^l$ times the composite
\[
\xymatrix@C+2pc{
Y \ar[r]^-{[d, d_Y]^l} & \Bar \otimes_{\Re } Y \ar[r]^-{1 \otimes \varphi'} & \Bar \otimes_{\Re } \Re  \cong \Bar \ar[r]^-{\Psi} & \Delta
}\,.
\]
\end{remark}
% see lipert3
...
Seminar, pages 1Ð-77. Springer, Berlin, 1967.
...
G.~M.~Kelly and R.~Street, \textsl{Review of the elements of 2-categories}, In Category Seminar (Proc. Sem., Sydney, 1972/1973), pages 75Ð103. Lecture Notes in Math., Vol. 420. Springer, Berlin, 1974.
