...
\ee
Here $\gamma = \theta_{a_1} \ldots \theta_{a_{n-r}}$ is an element of the exterior algebra $\Delta$ and $\gamma' = \theta_{b_1} \ldots \theta_{b_r}$ is the ``complement'' of $\gamma$, i.e. $b_1 < \cdots < b_r$ is the unique sequence with $\gamma \wedge \gamma' = (-1)^{s} \theta_1 \ldots \theta_n$ a scalar multiple of the top degree form, and $\partial_{[i]}$ is a divided difference operator, see \eqref{diffquotop}. The evaluation maps have similar elementary presentations, which involve in addition supertraces and residues, see Section \ref{??}.
...
\[
\cat{LG}( \mathbb{I}, W ) \lto \cat{LG}( \mathbb{I}, V ), \qquad Y \mapsto X \otimes Y
\]
Moreover an adjunction in $\LG$ gives rise in this way to a pair of adjoint functors between categories of matrix factorisations. One way to say this is that $\cat{LG}( \mathbb{I}, - )$ defines a psuedofunctor from $\cat{LG}$ to the bicategory of categories, functors and natural transformations, and adjoint pairs are preserved by a psuedofunctor; see \cite[\S $4$]{benabou} and \cite{lack}.

Less tersely, given an adjunction $X \dashv X^\dual$ and $1$-morphisms $Y: \mathbb{I} \lto W$ and $Z: \mathbb{I} \lto V$ one deduces from the identity on the evaluation and coevaluation a natural isomorphism
\[
...
X \otimes \Bar \ar[d]_{\At_{u}(X)^m} \ar[r]^-{\cong} & \reprod_*( \Bar ) \otimes X \ar[d]^{\Atlarrow_{u}(X)^m}\\
...
\begin{remark}\label{remark:curvature_superconnection} A \emph{superconnection} $\Theta$ on a $\mathbb{Z}_2$-graded $R$-module $X$ is a $k$-linear degree one operator $\Theta$ on $X \otimes_R \Omega R$ satisfying the graded Leibniz rule
\[
\Theta( \xi \omega ) = \Theta( \xi ) \omega + (-1)^{|\xi|} \xi d \omega
\]
for all homogeneous $\xi \in X \otimes_R \Omega R$ and $\omega \in \Omega R$. If $\nabla$ is a connection on $X$ then we can extend $\nabla$ to an operator on $X \otimes_R \Omega R$, and it is easily checked that for any odd $R$-linear operator $D$ on $X$
\[
\Theta = \nabla + D
\]
is a superconnection. The \emph{curvature} of this superconnection is
\[
\Theta^2 = \nabla^2 + D^2 + [\nabla, D]\,.
\]
...
\]
where $\sigma_2(e_i) = e_i \otimes (1 \otimes 1)$. In practice \eqref{eq:formula_rhoinverse} is the most useful presentation of $\rho^{-1}$, but note that using the formulas \eqref{eq:powerofatiyah} and \eqref{intro_psi} we can be very concrete
\[
...
Using the formulas for exponentiated Atiyah classes from Section \ref{section:atiyahclasses} and the explicit formula for $\Psi$ we also give completely elementary expressions for these maps involving only divided difference operators, see Remarks \ref{remark:explicitev} and \ref{remark:coev_divideddiff}. While concrete, these expressions are very difficult to work with directly, and in particular they will not be used in the sequel.
...
\begin{align}
\widetilde\eval_X( g e_j \otimes e_i^* ) = \sum_{l \ge 0} \sum_{i_1 < \cdots < i_l} &(-1)^{\binom{n-l+1}{2} + l + (n+1)|e_i|} \theta_{i_1} \ldots \theta_{i_l} \cdot\\
...
\end{align}
In this formula the divided difference operators act on $S^e = k[z,z']$. Similarly for $g \in S$,
\begin{align}
\eval_X( e_i^* \otimes g e_j ) = \sum_{l \ge 0} \sum_{i_1 < \cdots < i_l} &(-1)^{\binom{m-l+1}{2} + (m+l)|e_i|} \theta_{i_1} \ldots \theta_{i_l} \cdot \\
...
\end{align}
where the divided difference operators in this case act on $R^e = k[x,x']$.
\end{remark}
...
\]
The next step is to apply $(-) \otimes_{S^e} S$, that is, to identify the left and right actions of $S$, after which $X \otimes_R X^\vee$ becomes identified $\End(X)$, and we are left with
\[
...
\[
h_p( \alpha ) = \sum_{\ell(\bs{i}) = p} (-1)^{\gamma(\bs{i}) + n + p} \Big\langle\!\!\Big\langle d \left\{ f_{i_1} \cdots d\left\{ f_{i_p} \str\big( [ \Lambda_{i_1,\ldots,i_p}^\bullet, d](\alpha) \big) \right\} \cdots \right\} \Big\rangle\!\!\Big\rangle\,.
\]
\end{lemma}
\begin{proof}
...
\[
\sum_{q \notin \{ i_1,\ldots, i_p \}} (-1)^{q+1 + \sum_{1 \le b \le p} \delta_{i_b<q}} \bigdlangle d\left\{ f_{i_1} \cdots d\left\{ f_{i_p} f_q \str\big( \Lambda_{i_1,\ldots,i_p,q} \circ d \varphi \big) \right\} \cdots \right\} \bigdrangle \, .
\]
...
\[
\big\langle\!\big\langle \str \big( \Lambda \circ \At^n (\varphi)\big) \big\rangle\!\big\rangle = (-1)^{\binom{n}{2}} \str(\varphi) + \sum_{p=0}^{n-1} h_i( D \At^{n-p-1}(\varphi) )\,.
\]
\end{lemma}
\begin{proof}
...
&= (-1)^{\binom{n}{2}}\sum_{\sigma \in S_n} (-1)^{|\sigma|} \big\langle\!\big\langle d \left\{ f_{\sigma(1)} \cdots d\left\{ f_{\sigma(n)} \str(\varphi) \right\} \cdots \right\} \big\rangle\!\big\rangle + H'\,.
\end{align*}
%where 
%$$
%\delta = 
...
\ee
Here $\At_1$ operates on $\End(X) \otimes_{R_2} \Bar$. Replacing $d$ with $s$ and $\At_2$ with $\At_1$ the technical results Lemma \ref{lemma:lameadjoints} and Lemma \ref{lemma:zorromain} still hold, and the analogue of Lemma \ref{lemma:langlecalc1} is the statement
\be
...
\begin{theorem}
The bicategory $\LG$ has left and right adjoints: for any $1$-morphism $Y: V \lto W$,
\[
\xymatrix{
...
\[
(-)^\dual = (-)^\vee[n]: \LG(V, W) \lto \LG(W, V)
\]
is well-defined, and hypothesis $(ii)$ of the Appendix is easily checked using the explicit formula for the evaluation. The upshot is that every $1$-morphism $Y$ in $\LG(V,W)$ has right adjoint $Y^{\vee}[n]$, and similarly one proves that $Y^{\vee}[m]$ is left adjoint to $Y$.
\end{proof}
...
\end{lemma}
\begin{proof}
The proof is essentially the same as the usual proof for adjoint functors, so we omit it. Given evaluation and coevaluation maps for an adjunction, one defines $\Phi$ of a $2$-morphism $\tau: g \otimes h \lto l$ to be the composite
\[
\xymatrix@C+2pc{
