...
\draw[line width=0pt] 
(0,0) node[line width=0pt] (gamma) {{\small $\gamma$}}
...
$$
Using $X^\dual[m+n] \cong R[m]\otimes_R R[n] \otimes_R X^\dual$ we have a simple diagrammatic presentation for this isomorphism: 
\be\label{eq:isoq}
q_X :=
%%%%%%%%%%%%%%%%%%%%%%
...
The story for the bicategory $\LG$ is subtler (TODO: more subtle?), as we only expect the left and right adjoints of $X$ to be isomorphic when the number of variables in the regions on either side of $X$ agree mod $2$. This is the case, for example, when $X$ is an endomorphism of an object of $\LG$. Indeed, what we prove in this section implies that the rigid monoidal categories $\hmf(\Re,\widetilde W)^\omega = \LG(W,W)$ are pivotal for any potential $W\in R$. See Remark \ref{remark:notpivotal} for the construction of an isomorphism between $X$ and $X^{\dual \dual}$ in the case of equal variable parity.
...
\]
Concretely, we define these morphisms by the diagrams (TODO: relate to Corollary \ref{cor:nicerpivotalitydiagrams})
\[
TODO: diagrams
\]
...
\begin{proof}
TODO: \textsl{Say} something about wiggly calculus, no more diagrams! 
\end{proof}
...
The basic argument is a standard calculation with string diagrams. There are however slight modifications needed due to the fact that $\LG$ is only ``pivotal up to shifts'' as discussed in Section~\ref{sec:pivotality}. 
...
%%%%%%%%%%%%%%%%%%%%%%%
$$
where we used the Zorro moves to reexpress the identity on $Y\otimes X$, and here and below we do not display the argument of the defect operators. Now we are in a position to apply Corollary~\ref{cor:nicerpivotalitydiagrams} to the lower part of the diagram on the right-hand side to find that $\mathcal D_l(X) \circ \mathcal D_l(Y)$ is equal to
$$
\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=0.7,color=blue!50!black, baseline=-0.75cm,>=stealth]
...
%
% line labels: 
\draw (1.6,0.23) node {{\scriptsize $Y \otimes X$}};
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
\, , 
$$
...
The diagram~\eqref{annulus} can be computed in two different ways. One possibility is to first collapse the $X$-loop to obtain a ``$(k[x],W)$-bubble'', and then collapse the boundary loop of this bubble in turn to arrive at a scalar. It follows from the main result on defect actions (Proposition \ref{prop:defectaction}) that the first step of collapsing~$X$ produces the 2-morphism $(-1)^{n+1\choose 2} \str(\varphi \Lambda_X)\in\End(\Delta_W)$ where we write $\Lambda_Z = \partial_{x_1}(d_Z) \ldots \partial_{x_n}(d_Z)$ for $Z \in \{X,Y\}$. 

Similarly, the second step of collapsing~$Y$ produces a residue
$$
C 
= 
%%%%%%%%%%%%%%%%%%%%%%
...
On the other hand, we may compute~$C$ by first fusing the two loops in~\eqref{annulus} together to obtain a single loop labelled by ${}^\dual X\otimes Y \cong \Hom(X,Y)[n] \in \End_{\LG}( (k,0) )$. Then we use
$$
