...
Given a linear factorisation $X$ of $W$ the \emph{dual factorisation} $X^\vee = \Hom_R(X, R)$ is a linear factorisation of $-W$ with $d_{X^\vee}( \nu ) = -(-1)^{|\nu|} \nu \circ d_X$. In terms of matrices
...
\ee
We call $\Delta_W$ the unit matrix factorisation as it is the unit with respect to the tensor product of matrix factorisations. It is also referred to as the \emph{stabilised diagonal} \cite{d0904.4713} or \emph{Koszul model} of the diagonal since the morphism of linear factorisations of $\widetilde W$
...
For our purposes it will be sufficient to consider matrix factorisations $X\in \HMF(R_1\otimes_k R_2, W_2-W_1)$ and $Y\in \HMF(R_2\otimes_k R_3, W_3-W_2)$ (where of course $R_i = k$ is a possibility for any~$i$). Then the \textsl{tensor product matrix factorisation} $Y\otimes X \in \HMF(R_1\otimes_k R_3, W_3-W_1)$ is the $\nZ_2$-graded module 
$$
...
\textbf{todo} Spell out that $\Bar$ is a unit too.
