...
\be\label{DrXpsi}
\mathcal D_r(X)(\psi)  = (-1)^{{m\choose 2} + {n\choose 2}} \Res_{k[x,z]/k[z]} \left[ \frac{\psi \str\big( \partial_{z_1} D\ldots \partial_{z_m} D \, \partial_{x_1} D\ldots \partial_{x_n} D\big) \underline{\operatorname{d}\! x}}{\partial_{x_1} W \ldots \partial_{x_n} W} \right]
\ee
...
\begin{lemma}\label{lemma:atshufat} For $l \ge 0$ and $u \in \{1,2\}$ we have
\begin{align}
...
\begin{remark} To relate this to the earlier construction, observe that if $X$ is a free $\Re $-module with basis $\{ e_i \}_{i}$ then $\Omega R$ can be identified with $\Bar$ and the operator $d: X \lto X \otimes \Bar_1 = X \otimes_R \Omega^1 R$ of \eqref{eq:extendd} is a connection. \textbf{todo} Talk about extending $\nabla$ to operator on $X \otimes \Omega$. Reference for connections.
\end{remark}

The Atiyah class can be viewed as a component of the curvature of a superconnection.

\begin{remark}\label{remark:curvature_superconnection} A \textsl{superconnection} $\Theta$ on a $\mathbb{Z}_2$-graded $R$-module $X$ is a $k$-linear degree one operator~$\Theta$ on $X \otimes_R \Omega R$ satisfying the graded Leibniz rule
\[
\Theta( \xi \omega ) = \Theta( \xi ) \omega + (-1)^{|\xi|} \xi d \omega
\]
for all homogeneous $\xi \in X \otimes_R \Omega R$ and $\omega \in \Omega R$. If $\nabla$ is a connection on $X$ then we can extend $\nabla$ to an operator on $X \otimes_R \Omega R$, and it is easily checked that for any odd $R$-linear operator $D$ on $X$ the operator
\[
\Theta = \nabla + D
\]
...
\begin{proposition}\label{prop:alt_formula_coev} A representative for the coevaluation morphism is the chain map
\be\label{eq:alt_formula_coev}
\widetilde\coev^R_X(\gamma) = \varepsilon \left( \sum_{l \ge 0} (-1)^{l+nl} \Psi \At_{1}(X)^l( \iota_X ) \wedge \gamma \right) .
\ee
\end{proposition}
...
\[
\coev^R_X( \gamma ) = \sum_{l \ge 0} (-1)^{l + ml} \varepsilon \left( \Psi \At_1(X^{\vee})^l( \iota'_X ) \wedge \gamma \right) .
\]
...
We claim that the morphism $\widetilde\eval_X$ is canonical in the sense that it is independent up to homotopy of the choice of null-homotopies $\lambda_i$. It clearly suffices to argue that $\widetilde\eval_0$ is independent of these choices, and for this we use a variant of the argument in \cite[Appendix A]{dm1102.2957}. 
...
\begin{align*}
&\Res_{R/k} \left[ \frac{ \str( \Lambda^{(x)} \circ -) \, \underline{\operatorname{d}\!x}}{f_1, \ldots, f_n} \right] = \Res_{R/k} \left[ \frac{ \str( f_1 \lambda_1 \Lambda_{> 1} \circ -) \, \underline{\operatorname{d}\!x}}{f_1^2, f_2, \ldots, f_n} \right]\\
...
\end{align*}
The second summand is a null-homotopic functional, so we have proven that $\widetilde\eval_0$ is homotopic to the map defined using $\lambda_1' \Lambda_{> 1}$ in place of $\Lambda^{(x)}$, as claimed. The argument for general $i$, and for the map $\eval_X$, is the same. In much the same way one can show that up to a sign $\widetilde\eval_X$ and $\eval_X$ are also independent of the way in which we order the $\lambda_i$. 
\end{remark}

\begin{remark}\label{remark:explicitev} (\textbf{todo} fix signs) The formulas for $\widetilde\eval_X$ and $\eval_X$ can be written in terms of divided difference operators as follows: for $g \in R$ we have
\begin{align}
\widetilde\eval_X( g e_j \otimes e_i^* ) = \sum_{l \ge 0} \sum_{i_1 < \cdots < i_l} &(-1)^{\binom{n-l+1}{2} + l + (n+1)|e_i|} \, \theta_{i_1} \ldots \theta_{i_l} \\
...
\begin{align}
\eval_X( e_i^* \otimes g e_j ) = \sum_{l \ge 0} \sum_{i_1 < \cdots < i_l} &(-1)^{\binom{m-l+1}{2} + (m+l)|e_i|} \, \theta_{i_1} \ldots \theta_{i_l}  \\
...
\begin{remark} Before giving the proof of the lemma, let us roughly sketch how it leads to an argument that $\mathcal Z$ is homotopic to the identity. Although we will not formalise the argument in precisely these terms, the Atiyah class is ``anti-self-adjoint'' (\textbf{todo} still compatible with signs?) with respect to $\langle\!\langle \str( - ) \rangle\!\rangle$: 
...
Throughout the proof we write $\At$ for $\At_2$. The image of a basis element $e_q$ under the first two maps of \eqref{eq:zorro1a1} is computed by the expressions in \eqref{eq:formula_rhoinverse} and \eqref{eq:coev10}
\begin{align}
...
\]
The next step is to apply $(-) \otimes_{\Se} S$, that is, to identify the left and right actions of $S$, after which $X \otimes_R X^\vee$ becomes identified $\End(X)$, and we are left with
\[
...
&= \dlangle \str\big( \big[ \psi^\bullet, d \big] ( D\varphi ) \big) \drangle\\
&\qquad-  \dlangle d \str\big( \big[ D, \psi^\bullet \big] \varphi \big) \drangle\\
&\qquad- (-1)^{|\psi|}\dlangle \str\big( \big[ D, \psi^\bullet \big]( d \varphi ) \big) \drangle \, .
\end{align*}
...
\[
\big\langle\!\big\langle \str( \lambda_1 \ldots \lambda_n \circ \At(\varphi) ) \big\rangle\!\big\rangle = \sum_{i=1}^n (-1)^{i} \Big\langle\!\!\Big\langle d \left\{ f_i \str( \lambda_1 \ldots \widehat{\lambda_i} \ldots \lambda_n \circ \varphi ) \right\} \Big\rangle\!\!\Big\rangle + h_0( D(\varphi))
\]
...
\be
|\bs{i}| = \sum_{1 \le a < b \le p} \delta_{i_a > i_b}\, , \qquad \gamma(\bs{i}) = |\bs{i}| + i_1 + \ldots + i_p + \binom{p}{2}\,.
\ee
...
\begin{align*}
\sum_{\ell(\bs{i}) = p} (-1)^{\gamma(\bs{i})} & \Big\langle\!\!\Big\langle d \left\{ f_{i_1} \cdots d \left\{ f_{i_p} \str\big( \Lambda_{i_1,\ldots,i_p} \circ \At(\varphi) \big) \right\} \cdots \right\}  \Big\rangle\!\!\Big\rangle\\
&= \sum_{\ell(\bs{i}) = p+1} (-1)^{\gamma(\bs{i})} \Big\langle\!\!\Big\langle d \left\{ f_{i_1} \cdots d\left\{ f_{i_{p+1}} \str\big( \Lambda_{i_1,\ldots,i_p,i_{p+1}} \circ \varphi \big) \right\} \cdots \right\} \Big\rangle\!\!\Big\rangle + h_p( D \varphi )
\end{align*}
...
\[
h_p( \alpha ) = \sum_{\ell(\bs{i}) = p} (-1)^{\gamma(\bs{i})} \Big\langle\!\!\Big\langle d \left\{ f_{i_1} \cdots d\left\{ f_{i_p} \str\big( [ \Lambda_{i_1,\ldots,i_p}^\bullet, d](\alpha) \big) \right\} \cdots \right\} \Big\rangle\!\!\Big\rangle\,.
\]
\end{lemma}
\begin{proof}
Fix a sequence $\bs{i}$ of length $p$, set $\psi = \Lambda_{i_1,\ldots,i_p}$ and let $\eta$ be the operator $\eta = d \circ f_{i_1} \circ \cdots \circ d \circ f_{i_p}$ on $\Bar$, where the $f_i$ stand for multiplication (on the left) by $f_i$. Then by Lemma \ref{lemma:lameadjoints}, or rather the proof with $\dlangle - \drangle$ replaced by $\dlangle \eta( - ) \drangle$ we have
\begin{align}
...
\ee
where the sign counts the number of $\lambda$'s appearing to the left of $\lambda_q$ in $\Lambda_{i_1,\ldots,i_p}$. By construction
\be\label{eq:lambdasigns}
\gamma( ( i_1, \ldots, i_p, q ) ) = \gamma(\bs{i}) + q + p + \sum_{1 \le a \le p} \delta_{i_a > q} = \gamma(\bs{i}) + q + \sum_{1\le b \le p} \delta_{i_b<q} \,.
\ee
Substituting, the last summand (\ref{eq:hutch3}) in the above is, up to a sign, 
\[
\sum_{q \notin \{ i_1,\ldots, i_p \}} (-1)^{q + \sum_{1\le b \le p} \delta_{i_b<q}} \bigdlangle d\left\{ f_{i_1} \cdots d\left\{ f_{i_p} f_q \str\big( \Lambda_{i_1,\ldots,i_p,q} \circ d \varphi \big) \right\} \cdots \right\} \bigdrangle \, .
\]
If we sum over all sequences $\bs{i}$ with the signs given in the statement of the lemma this vanishes, because $f_{i_p} f_q$ and $f_q f_{i_p}$ appear in the sum with opposite signs. This completes the proof, since the sum over $\bs{i}$ of (\ref{eq:hutch1}) and (\ref{eq:hutch2}) gives us the right-hand side of the equation in the statement of the lemma, and the signs are correct by \eqref{eq:lambdasigns}.
\end{proof}

\begin{lemma}\label{lemma:langlecalc1} For $\varphi \in \End(X)$ we have
\[
\big\langle\!\big\langle \str \big( \Lambda \circ \At^n (\varphi)\big) \big\rangle\!\big\rangle = (-1)^{n} \str(\varphi) + \sum_{p=0}^{n-1} h_i( D \At^{n-p-1}(\varphi) )\,.
\]
\end{lemma}
\begin{proof}
Applying Lemma \ref{lemma:zorromain} repeatedly yields
\begin{align*}
\big\langle\!\big\langle \str \big( \Lambda \circ \At^n (\varphi)\big) \big\rangle\!\big\rangle &= \sum_{\ell(\bs{i}) = n} (-1)^{\gamma(\bs{i})} \big\langle\!\big\langle d \left\{ f_{i_1} \cdots d\left\{ f_{i_n} \str(\varphi) \right\} \cdots \right\} \big\rangle\!\big\rangle + H'\\
&= (-1)^{n}\sum_{\sigma \in S_n} (-1)^{|\sigma|} \big\langle\!\big\langle d \left\{ f_{\sigma(1)} \cdots d\left\{ f_{\sigma(n)} \str(\varphi) \right\} \cdots \right\} \big\rangle\!\big\rangle + H'\,.
\end{align*}
%where 
%$$
%\delta = 
...
\[
(-1)^{n} \str( \varphi ) + \sum_{p=0}^{n-1} h_i( D \At^{n-p-1}(\varphi) ) \, .
\]
\end{proof}
...
The idea is to use the nondegenerate pairing on the morphism spaces of $\hmf(k[x,z], V-W)$. If~$k$ is a field this is the Kapustin-Li pairing of \cite{kl0305,hl0404} but in general we use the contents of Section \ref{??} (\textbf{more}). The precise statement (Theorem \ref{??}) is that there is a homotopy equivalence of $\mathbb{Z}_2$-graded complexes over $k$
\be
...
\mathcal{Z} = \sum_j (-1)^{\binom{n+1}{2} + |e_j|} \dlangle \str\left( \Lambda \circ \At_1^n( e_j \circ - ) \right) \drangle \cdot e_j^*\,.
\ee
...
\be
\dlangle \str( \Lambda \circ \At_1^n( \varphi ) \drangle = (-1)^{\binom{n+1}{2}} \str(\varphi)\,.
\ee
...
\begin{align*}
\Hom( X \otimes_{k[x]} Y, Z ) & \xlto{\cong} \Hom( Y, X^\dual \otimes_{k[z]} Z )\,,\\
\Hom( {}^\dual X \otimes_{k[z]} Z, Y ) & \xlto{\cong} \Hom( Z, X \otimes_{k[x]} Y ) \, .
\end{align*}
...
\mathcal D_r^\Phi(X)(\psi) & = (-1)^{{m\choose 2} + {n\choose 2}}  \Res_{k[x,z]/k[z]} \left[ \frac{\psi(x) \str\big( \Phi \, \partial_{z_1} d_{X}\ldots \partial_{z_m} d_{X} \partial_{x_1} d_{X}\ldots \partial_{x_n} d_{X} \big) \underline{\operatorname{d}\! x}}{\partial_{x_1} W, \ldots, \partial_{x_n} W} \right] \, . 
\end{align*}
\end{proposition}
...
\ee
commutes.
\item[(iii)] (TODO: do we need this part of the remark)In diagrammatic language the explicit construction of the $2$-morphism ${}^\dual \zeta$ can be presented in terms of the diagram: (TODO?)

The commutativity of \eqref{eq:dualzeta} and \eqref{eq:dualzeta2} expresses the following two identities, which tell us that we can ``slide'' $2$-morphism around cups and caps: (TODO?)
\end{itemize}
\end{remark}
