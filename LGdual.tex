...
In this section we write $B = k[z]$ and $B^e = B \otimes_k B$ so that $\widetilde\eval_X$ is a morphism of (infinite rank) matrix factorisations of $\widetilde{V} = V \otimes 1 - 1 \otimes V$ over $B^e$. The polynomials $\partial_{x_i} W$ act null-homotopically on $X$ and we let $\lambda_i \in \Hom_{k[x,z]}(X,X)$ denote a degree-one map with $[D, \lambda_i] = f_i \cdot 1_X$. For example $\lambda_i = \partial_{x_i} D$ would do, but it will be important to allow other choices.

The evaluation map will be produced by a perturbation whose initial data is the morphism $\eval_0$ of the next proposition.

\begin{proposition} There is a morphism of linear factorisations of $\widetilde{V}$ over $B^e$
\[
...
\[
\Hom_{k[x,z]}(\bar{X}, \bar{X}) = \Hom_{k[x,z]}(X, X) \otimes_{k[x]} k[x]/(f_1,\ldots,f_n)\,.
\]
...
\]
There is a chain of $B^e$-linear morphisms of linear factorisations of $\widetilde{V}$
\begin{equation}
\xymatrix{
