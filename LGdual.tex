...
The key point comes in the proof of the Zorro moves: in \eqref{eq:zorroequalsH} we find that the Zorro move holds \emph{up to some map} $H$, which we consequently want to prove is null-homotopic. The only way we know to do this is to use a nondegenerate pairing. If $k$ is a field, then we may use the Kapustin-Li formula of \cite{kl0305,hl0404,m0912.1629,dyckmurf}, but if $k$ is an arbitrary noetherian $\mathbb{Q}$-algebra there is no longer a nondegenerate pairing on the \emph{triangulated} category. Instead we use a ``homotopically'' nondegenerate pairing on the dg-category, which is constructed in Section \ref{section:dualityadjointop} for this purpose.
\end{remark}
...
If one knew \emph{a priori} that this functional was the residue map, this would prove \eqref{eq:integrateagainstd}, but we do not know a reference for this fact! In any case, we give a proof in \cite{??}, which in particular proves \eqref{eq:integrateagainstd} for any noetherian $\mathbb{Q}$-algebra $k$.
\end{proof}
...
\begin{proposition}\label{prop:alt_formula_coev} A representative for $\widetilde\coev_X$ is the chain map
...
The proof of Lemma \ref{lemma:Zorrointermediate} rests on the ``calculus'' of Atiyah classes from Section \ref{section:calculusatiyah}.
...
\bibitem[MR]{mazur}
B.~Mazur and L.~Roberts, \textsl{Local Euler Characteristics}, Inventiones math. 9, 201--234 (1970).
