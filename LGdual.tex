...
(TODO: there are no new adjoints here anymore, only the graphical notation is new)
...
(TODO: add labelled wigglies and their crossings, show map $X^\dual[m+n]\lra {}^\dual X$)
...
(TODO: remark that part (ii) isn't so bad if $m=p=0$ mod 2)
...
(2,3) node[line width=0pt, right] (XY) {{\small $\bar Y\otimes \bar X$}}; 
% wiggly labels:
\draw[line width=1pt] 
