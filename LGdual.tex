...
\ee
defining an adjunction between~$X$ and~$X^\dual$. Similarly, one expects a left adjoint~${}^\dual X$ of~$X$. Similarly, one expects maps $\eval_X: X \otimes {}^\dual X \lto \Delta$ and $\coev_X: \Delta \lto {}^\dual X \otimes X$ for a left adjoint ${}^\dual X$ of $X$, together with natural isomorphisms between $X$ and its double-adjoint, closely related to a so-called pivotal structure. The special case where~$V$ and~$W$ are the same polynomial and depend on only one variable was worked out in~\cite{cr1006.5609}; we refer to that paper's introductory section for further motivation from physics.

In the present paper we prove that every $1$-morphism $X$ in $\LG$ has left and right adjoints given by ${}^\dual X = X^{\vee}[m]$ and $X^\dual = X^{\vee}[n]$ where $X^{\vee} = \Hom_{k[x,z]}(X, k[x,z])$ is the dual factorisation. One of our aims is to provide explicit expressions for the above evaluation and coevaluation maps, with the aim of making it simple and convenient to do algebra in the bicategory $\LG$, and in particular to evaluate correlators such as \eqref{worldsheetwithdefects}. \textbf{todo} pivotality?
...
\begin{remark}\label{remark:explicitev} The formulas for $\widetilde\eval_X$ and $\eval_X$ given above can be written in terms of divided difference operators, as follows: for $g \in R$ we have
\begin{align}
...
\end{align}
In this formula the divided difference operators act on $S^e = k[z,z']$.

Similarly for $g \in S$,
\begin{align}
...
Throughout we write $\At$ for $\At_2$, and Atiyah classes are taken with respect to $R/k$. The image of a basis element $e_q$ under the first two maps of \eqref{eq:zorro1a1} is computed by the expressions in \eqref{eq:formula_rhoinverse} and \eqref{eq:coev10}
\begin{align}
...
The basic adjointness that we need is contained in the next lemma. Throughout we write $\psi^\bullet$ for the operator $\psi^\bullet(\alpha \otimes \omega) = \psi \circ \alpha \otimes \omega$ on $\End(X) \otimes_{R_1} \Bar$, and we denote $[d_X,-]$ by~$D$. (\textbf{todo} Be clearer here about which Atiyah class we're using)
...
\begin{remark}\label{remark:otherzorros}
