...
The \textsl{associative Atiyah classes} of $(X,D)$ are operators on $X \otimes \Bar$ defined by the graded commutators 
\begin{align*}
\At_{2}(X) &= [d, D] = d D + D d\,,\\
\At_{1}(X) &= [s, D] = s D + D s\,.
\end{align*}
...
With the proof that the bicategory $\LG$ has adjoints behind us we now want to reap some further rewards of our efforts by turning to applications. These will be discussed in the next sections, while in the present one we shall pave the way by clarifying two conceptual and practically relevant issues: an suitable refinement of the graphical calculus with adjoints, and the nature of double-adjoints. 
...
%%%%%%%%%%%%%%%%%%%%%%
\]
whose precise relation is the content of Corollary~\ref{cor:nicerpivotalitydiagrams} below. Note that the graphical presentation makes it manifest that the above two maps are indeed isomorphisms in $\LG$: applying the Zorro moves one checks their inverses are given by copies of themselves if rotated by 180 degrees. 
...
In the bicategory $\LG$ there are natural maps between the endomorphism spaces of unit 1-morphisms constructed by ``capturing'' a 2-endomorphism of a unit 1-morphism inside a loop labelled by an arbitrary 1-morphism. We give natural interpretation of this process in terms of defect actions on bulk fields in Landau-Ginzburg models. 
...
\item $\mathcal D_l(X[a]) = (-1)^a \mathcal D_l(X)$ and $\mathcal D_r(X[a]) = (-1)^a \mathcal D_r(X)$.
\end{enumerate}
Analogous results hold when the operators are decorated with endomorphisms $\Phi, \Phi'$ of $X$ and $Y$.
\end{proposition}
...
\ee
On the other hand, we may compute~$C$ by inflating the $X$-loop in \eqref{annulus} and fusing it with the outer $Y$-loop, creating a single loop labelled by $X^\vee[n] \otimes Y \cong \Hom(X,Y)[n] \in \End_{\LG}( (k,0) )$. Rigorously, this process is described by the series of equalities
\[
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=0.8,color=blue!50!black, baseline,>=stealth]
\nicecolourscheme (0,0) circle (2);
...
\fill (180:0.9) circle (0pt) node[right] {{\small$X^{\vee}[n]$}};
\fill (0:2.7) circle (0pt) node[left] {{\small$Y$}};
\end{tikzpicture} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
=
...
\fill (0:2.7) circle (0pt) node[right] {{\small$X^{\vee}[n] \otimes Y$}};
\end{tikzpicture} 
%%%%%%%%%%%%%%%%%%%%%%
\]
where in the first step we use Proposition \ref{prop:defectactionproperties}(ii) to reorient the inner loop, and in the second step we use (iii) to fuse the two loops (note that the proof of (iii) relies on Proposition~\ref{prop:monoidalproperty}). The complexes $X^{\vee}[n] \otimes Y$ and $\Hom(X,Y)[n]$ are isomorphic, so
...
\fill (0:2.7) circle (0pt) node[right] {{\small$\Hom(X,Y)[n]$}};
\end{tikzpicture} 
%%%%%%%%%%%%%%%%%%%%%%
...
The value of an oriented loop, labelled by a finite rank $\mathbb{Z}_2$-graded $k$-complex $F$ with the insertion of a morphism of complexes $\phi: F \lto F$, is easily checked to be the supertrace of $\phi$. In this case $\Hom(X,Y)$ is a complex of infinite rank $\mathbb{Z}_2$-graded free modules. If $k$ were a field this complex would be isomorphic, in $\HMF(k,0)$, to the finite dimensional $\mathbb{Z}_2$-graded vector space $H^*\Hom(X,Y)$ and the value of \eqref{eq:finaldiagramcardycond} would be the supertrace of the action of ${}_\psi m_{\varphi}$ on this cohomology.

In general $\Hom(X,Y)$ is not isomorphic to its cohomology, but it is at least a summand in the homotopy category of a finite rank $\mathbb{Z}_2$-graded free $k$-module and therefore any operator on $\Hom(X,Y)$ has a well-defined supertrace. Indeed, once we choose such a split embedding of $\Hom(X,Y)$ in something finite rank, the supertrace can be computed by writing out the diagram \eqref{eq:finaldiagramcardycond} as a series of morphisms, using the evaluation and coevaluation maps for infinite rank objects produced in Appendix \ref{??} from such a choice of embedding.

Comparing~\eqref{C1} and~\eqref{C2} we thus observe: 
