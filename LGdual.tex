...
With these formulas in hand we return to the lifting problem discussed at the beginning of this section. We show that given $Y \in \hmf(R^e, \widetilde{W})$ composing with $\pi_{\Delta}$ induces an isomorphism
\be\label{eq:liftingresult0}
H^0 \Hom_{R^e}(Y, \Delta) \lto H^0 \Hom_{R^e}(Y, R)\,.
\ee
More concretely: given an $R^e$-linear morphism of linear factorisations $\varphi: Y \lto R$ there is a unique (up to homotopy) $R^e$-linear morphism $\varphi_{\textup{lift}}$ making the following diagram commute up to homotopy
\[
\xymatrix{
...
\begin{proposition}\label{prop:liftingresult} The map \eqref{eq:liftingresult0} is an isomorphism and, in the above notation
...
\]
This is actually a morphism of complexes, since the commutator of $\Lambda$ with the differentials on the mapping complexes is by definition a linear combination of the $f_i$. The supertrace gives another $S$-linear morphism of complexes
\[
\xymatrix@C+2pc{
...
There is a stabilisation morphism $\pi_\Delta: \Delta \lto S$ and while $X \otimes_R X^\dual$ is not free of finite rank over $S^e$, it is a direct summand of a finite rank matrix factorisation in $\HMF(S^e, \widetilde{V})$ and it follows that the unique lifting of Proposition \ref{prop:liftingresult} applies. That is, there is up to homotopy a unique morphism of matrix factorisations $\widetilde\eval_X$ making the following diagram commute
...
\begin{proposition} A representative for the evaluation morphism is the chain map
\[
