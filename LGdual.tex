...
Let us fix the ring $R=k[x_1,\ldots,x_n]$, where~$k$ is a commutative noetherian $\nQ$-algebra; relevant examples are $k=\nC$ and $k=\nC[t_1,\ldots,t_d]$. Typically we will simply write $k[x]$ for $k[x_1,\ldots,x_n]$. We call an element $W\in R$ a \textsl{potential} if $R/(\partial_{x_i}W)$ is a finitely generated free $k$-module and the $\partial_{x_i}W$ form a regular sequence. In this case $k=\nC$ this simply means that the Jacobi ring $R/(\partial_{x_i}W)$ is a finite-dimensional vector space. 
...
we briefly recall some of its basic properties; see~\cite{TODO} for more details. Given a regular sequence $(f_1,\ldots,f_n)$ in $k[x]$ it is the $k[y]$-linear map that sends a polynomial $g\in k[x,y]$ to the expression
$$
...
Let $k$ be a ring and $R$ a $k$-algebra, $\Omega R = \Omega_k R$ the differential graded algebra $(\Omega R, d, \cdot)$ of noncommutative differential forms explained in Section \ref{subsec:Bar}. A \emph{connection} on a $\mathbb{Z}_2$-graded $R$-module $X$ is a $k$-linear map
\[
...
\]
while simply stating the formulas for $\coev_X$ and $\eval_X$, with the details left to the reader. To simplify the notation we set $R = k[x]$ and $S = k[z]$ so that $R^e = R_1 \otimes_k R_2$ with $R_i = R$ and $S^e = S_1 \otimes_k S_2$ with $S_i = S$. Then $\Delta_W$ and $X^\dual \otimes X$ are $R^e$-modules and the coevaluation is $R^e$-linear, while $\Delta_V$ and $X \otimes X^\dual$ are $S^e$-modules and the evaluation is $S$-linear. The differentials on the matrix factorisations $\Delta_V, \Delta_W$ are given by \eqref{DeltaW}. We fix a homogeneous basis $\{ e_i \}_{i \in I}$ for $X$ with dual basis $\{ e_i^* \}_{i \in I}$.
...
\]
defined for $\eta \in X$ and $\nu \in X^\dual$ by
\[
\eval_0( \eta \otimes \nu ) = (-1)^{\chi + n|e_i| +n} \Res_{k[x,z]/k[z]} \left[ \frac{ \str( \Lambda \circ \eta \circ \nu) \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]
\]
\end{lemma} 
