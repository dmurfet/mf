...
\be
\Atlarrow_{2}(X)^l( \omega \otimes e_i ) = (-1)^{l|\omega| + \binom{l}{2}} \sum_{j_1,\ldots,j_l} d(D_{j_l,j_{l-1}}) \ldots d(D_{j_2,j_1}) d(D_{j_1,i}) \omega \otimes e_{j_l}\,.
\ee
...
\ee
where we write $D_{ji} = \sum D_{ji}^{(1)} \otimes D_{ji}^{(2)} \in \Re$. The additional sign compared to \eqref{eq:at2nopower} comes from the sign in the definition \eqref{eq:definition_s} of the operator $s$. More generally for $l \ge 1$,
\be
...
\begin{align}
\At_u(X \otimes Y)^l( e_i \otimes f_j ) &= \sum_{p+q = l} \At_u(X)^p(e_i) \times \Atlarrow_u(Y)^q(f_j)\label{eq:atshufat}
\end{align}
...
\]
Iterating, we see that $\At_2(X \otimes Y)^l(e_i \otimes f_j)$ is a sum over all indices of terms
\be\label{eq:proofofatiyahshuff}
e_{i_{l-p}} \otimes f_{j_p} \otimes \sigma_\bullet( \underbrace{d(d_{X,i_{l-p}i_{l-p-1}}), \ldots, d(d_{X,i_1i})}_{l-p}, \underbrace{d(d_{Y,j_pj_{p-1}}), \ldots, d(d_{Y,j_1j})}_p )
\ee
where $\sigma$ is an $(l-p,p)$ shuffle and the sign that is attached to such a term is $(-1)^q$ where
\[
q = |e_{i_{l-p}}| + \ldots + |e_{i_1}| + |f_{j_p}| + \ldots + |f_{j_1}| + (l-p)|f_{j_p}| + |\sigma|\,.
\]
...
In Sections \ref{subsec:derivcoeval} and \ref{subsec:eval} below we define evaluation and coevaluation maps (\textbf{todo} pictures)
\begin{align}
...
\[
\xymatrix@C+0.5pc{
X^\dual \otimes_{S} X \ar[rr]^-{\Atlarrow_2(X^\dual)^m} & & \reprod_*(\Bar) \otimes_{\Re} ( X^\dual \otimes_{S} X ) \ar[r]^-{\Psi} & \Delta_W \otimes_{\Re} (X^\dual \otimes_{S} X)
}
\]
...
\begin{align*}
\mathcal D_l^\Phi(X)(\phi) & 
= (-1)^{{m\choose 2} + {n\choose 2}}\Res_{k[x,z]/k[x]} \left[ \frac{\phi(z) \str\big( \Phi \, \partial_{z_1} d_{X}\ldots \partial_{z_m} d_{X} \partial_{x_1} d_{X}\ldots \partial_{x_n} d_{X} \big) \underline{\operatorname{d}\! z}}{\partial_{z_1} V, \ldots, \partial_{z_m} V} \right] \, , \\
