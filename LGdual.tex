...
\begin{gather*}
(-,-): X \otimes_R X^{\vee} \lto \Se \otimes_k R\,,\\
...
\begin{proof}
(i) TODO: cite Kapustin-Rozansky. 

(ii): In the proof of the explicit formula for $\mathcal D_l(X)$ in Proposition~\ref{prop:defectaction} we could also have used the alternative representations for $\eval_X$ and $\widetilde\coev_X$ in terms of $d_{X^\dual}$. Then the assertion immediately follows. (TODO: maybe better: look at Proposition~\ref{prop:defectaction} and use relation between $d_{X^\dual}$ and $d_X$)
...
\ee
commutes.
\item[(iii)] In diagrammatic language the explicit construction of the $2$-morphism ${}^\dual \zeta$ can be presented in terms of the diagram: (TODO?)
