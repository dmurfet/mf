...
\end{equation}
where $\Psi$ is the canonical map given in (\ref{intro_psi}). Roughly speaking the inverse of $\rho'$ is the geometric series in powers of the Atiyah class of $X$ and by postcomposing with $\Psi$ we obtain the desired inverse to $\rho$. A similar argument works for inverting $\lambda$. A completion of $X \otimes \Bar$ is used in order to guarantee that the geometric series converges. We return to this example in Remark \ref{remark:rhoandlambdainverse} below.
...
\]
The solution to this lifting problem is given in Proposition \ref{prop:liftingresult}, and will be used to give an explicit formula for the evaluation maps in Section \ref{sec:derivcoeval}.
...
\begin{remark}\label{remark:rhoandlambdainverse} To return to the problem of inverting the unit actions $\lambda$ and $\rho$, recall the special case of $R = k[x], S = k[z]$, $X'$ a matrix factorisation over $S \otimes_k R$ of $V - W$ and $X = X' \otimes_R \Re$ discussed in Remark \ref{remark:specialcaseinvert}. In this case $\pi_{\Delta}$ is precisely the right unit action $\rho = \pi_{\Delta}: X' \otimes_R \Delta \lto X'$.

The relevant $S$-$R$-bimodule structure on $X' \otimes_R \Delta$ is the one coming from the action of $S$ on $X'$ and the right action of $R$ on $\Delta$ (which we have called $R_2$ above) and with respect to this bimodule structure the homotopy inverse of $\rho$ is given by \eqref{eq:koszulstab2_1}, that is,
