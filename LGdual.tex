...
\begin{proposition} The element $\delta = \det( (\partial_{[i]} f_j)_{i,j} )$ in $k[x,x']$ has the property that for any $g \in k[x]$
...
\end{align}
where $\varepsilon$ is as defined in~\eqref{eq:vareps} and
\be\label{eq:iotaX}
...
\gamma \mapsto \varepsilon \left( \sum_{l \ge 0} (-1)^{l+nl} \Psi \At_{1}(X)^l( \iota_X ) \wedge \gamma \right) .
\ee
\end{proposition}
...
\be
\gamma \mapsto \sum_{l \ge 0} (-1)^{l + ml} \varepsilon \left( \Psi \At_1(X^{\vee})^l( \iota'_X ) \wedge \gamma \right) .
\ee
...
\textbf{todo} which divided diff operator is this?
...
Let $\{ e_i \}_{i}$ be a homogeneous $(S \otimes_k R)$-basis of $X$, $\{ e_i^* \}_{i}$ the dual basis and $\{ e_{ij} = e_i \circ e_j^* \}_{i,j}$ the determined basis of $\End(X) = \Hom_{S \otimes_k R}(X,X)$. Throughout this section an undecorated $\At$ denotes the Atiyah class $\At_2( \End(X) )$ of this complex, which recall is the operator on $\End(X) \otimes_{R_1} \Bar$ determined by the basis $e_{ij}$ using the convention of Remark \ref{remark:linearatother}.
...
\begin{remark} Before giving the proof of the lemma, let us sketch out the proof that $\mathcal Z$ is homotopic to the identity. The idea is that this should follow from skew-symmetry of $\At$, i.e., a homotopy
...
&- \dlangle d f_{i_{\sigma(1)}} \cdots f_{i_{\sigma(p-1)}} df_{i_{\sigma(p)}} \omega \drangle\,.
\end{align}
...
\end{align*}
where in the last step we use the residue identity from Lemma \ref{??}.
\end{proof}
...
\end{theorem}
\begin{proof}
It suffices to prove that $\langle H, - \rangle_X$ can factored via the differential $D$. Writing $\Res\,\big[ - \big]$ for the residue in \eqref{eq:bracketkl} we compute using Lemma \ref{lemma:strhalpha} that for $\alpha \in \End(X)$,
\begin{align*}
...
\draw (-1,0) -- (XY);
%
\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-0.35,-0.76) .. controls +(-0.7,0.5) and +(0.1,-1) .. (-0.85,1.6); 
%
\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (2.15,-0.65) .. controls +(0,1) and +(0.7,-0.15) .. (1.5,0.75); 
%
\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (3.15,-0.65) .. controls +(0,1.1) and +(1.8,0) .. (1.5,1.5); 
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
=
%%%%%%%%%%%%%%%%%%%%%%%
...
%\draw (2,0) -- (XY);
%%
%\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (1.85,1.59) .. controls +(-0.01,-1.6) and +(0.7,0.1) .. (1.25,-0.7); 
%%
%\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-1.15,-0.65) .. controls +(0,1) and +(-0.7,0) .. (-0.4,0.77); 
%%
%\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-2.15,-0.65) .. controls +(0,1.1) and +(-1.8,0) .. (-0.4,1.5); 
%\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=0.8,color=blue!50!black, baseline, rotate=180]
...
\draw (2,0) -- (XY);
%
\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (2.15,1.59) .. controls +(-0.01,-1.0) and +(0.7,0.1) .. (1.25,-0.7); 
%
\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-0.85,-0.65) .. controls +(-0.3,0.9) and +(-0.7,0) .. (-0.4,0.77); 
%
\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-1.85,-0.65) .. controls +(-0.3,1.0) and +(-1.8,0) .. (-0.4,1.5); 
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
\ee
\end{enumerate}
...
\draw (-1,0) -- (XY);
%
\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-0.35,-0.76) .. controls +(-0.7,0.5) and +(0.1,-1) .. (-0.85,2.67); 
%
\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (2.15,-1.735) .. controls +(0,2) and +(0.7,-0.15) .. (1.5,0.75); 
%
\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (3.15,-1.735) .. controls +(0,2) and +(1.8,0) .. (1.5,1.5); 
%
%
...
$$
(-1)^{{n\choose 2} + m|e_b| + (m+n)|f_a|} \, 
g_\alpha^* \circ \varepsilon \Psi \At_2(X^\vee \otimes Y^\vee)^m (e_b^* \otimes f_a^*) \, . 
$$
(TODO: explain notation in rest of argument) Similarly the right-hand side of~\eqref{eq:leftmonoidaldetail} maps $(f_a \otimes g_\alpha e_b)^*$ to 
$$
(-1)^{{p\choose 2} + mp + m|e_b| + (m+p) |f_a|} \, 
...
Let $X\in \hmf(k[x_1,\ldots,x_n,z_1,\ldots,z_m], V-W)$ as before. In this section when we write $\Hom$ and $\End$ we mean the spaces of 2-(endo)morphisms in $\LG$. We define \textsl{defect operators} (\textbf{todo} these are closed maps, point this out, also rewrite following formulas)
$$
...
%	
% wiggly lines: 
\draw[very thin, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] 
