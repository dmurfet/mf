...
\[
\lambda_X^{-1} = \sum_{m \ge 0} (-1)^m \Psi \At_{1}(X)^m \sigma_1\,, \qquad \rho_X^{-1} = \sum_{m \ge 0} (-1)^m \Psi \At_{2}(X)^m \sigma_2\,.
\]
\end{proposition}
...
In this section we derive formulas for the evaluation and coevaluation $2$-morphisms in the bicategory $\LG$ of Landau-Ginzburg models. We fix two potentials $W\in k[x] = k[x_1,\ldots,x_n]$ and $V\in k[z] = k[z_1,\ldots,z_m]$ and a matrix factorisation $X\in \hmf(k[x,z], V-W)$. 

\begin{definition}
The \textsl{right adjoint} of~$X$ is $X^\dual = X^\vee[n] \in \hmf(k[x,z], W-V)$ with 
$$
d_{X^\dual} = \begin{pmatrix} 0 & (d_X^0)^\vee \\ -(d_X^1)^\vee & 0\end{pmatrix}[n] \, .
$$
Similarly, ${}^\dual X = X^\vee[m]$ is the \textsl{left adjoint}. 
\end{definition}

As recalled in Section~\ref{subsec:bicatLG} there are two kinds of evaluation and coevaluation $2$-morphisms associated with the two adjoints of~$X$. The derivation is very similar in both cases, so we give the full details for 
\[
\widetilde\coev_X: \Delta_W \lto X^\dual \otimes_{k[z]} X
\, , \qquad
\widetilde\eval_X: X \otimes_{k[x]} X^\dual \lto \Delta_V
\]
while simply stating the formulas for $\coev_X$ and $\eval_X$. To simplify the notation we set 
$$
R = k[x]\, , \qquad S = k[z]
$$ 
so that $\Re  = R_1 \otimes_k R_2$ with $R_i = R$ and $\Se  = S_1 \otimes_k S_2$ with $S_i = S$. Then $\Delta_W$ and $X^\dual \otimes X$ are $\Re $-modules and $\widetilde\coev_X$ is $\Re $-linear, while $\Delta_V$ and $X \otimes X^\dual$ are $\Se$-modules and $\widetilde\eval_X$ is $\Se$-linear. The differentials on the matrix factorisations $\Delta_V, \Delta_W$ are given by \eqref{DeltaW}. We fix a homogeneous basis $\{ e_i \}_{i}$ for $X$ with dual basis $\{ e_i^* \}_{i}$.
...
\widetilde\coev^L_X(\gamma) = \varepsilon \left( \gamma \wedge \sum_{m \ge 0} (-1)^m \Psi \Atlarrow_2(X^\vee)^m( \iota_X ) \right)\,.
\end{equation}
\end{theorem}
\begin{proof}
...
If we define
\[
\iota_X = \sum_j (-1)^{m|e_j|} e_j \otimes e_j^* \in X \otimes_R X^\vee[m]
\]
then
\[
\coev^L_X( \gamma ) = \sum_{i \ge 0} (-1)^i \varepsilon \left( \gamma \wedge \Psi \Atlarrow_2(X)^i( \iota_X ) \right)
\]
and the alternative formula is
\[
\coev^R_X( \gamma ) = \sum_{i \ge 0} (-1)^{i + im} \varepsilon \left( \Psi \At_1(X^{\vee})^i( \iota_X ) \wedge \gamma \right)\,.
\]
\subsection{Evaluation}\label{subsec:eval}
% see (eval)

In this section $\Delta$ denotes $\Delta_V$. The evaluation will be produced by writing down a natural morphism [TODO: write tilde on top of $\eval_0$ etc.~everywhere?] $\eval_0: X \otimes_R X^\dual \lto S$ and then lifting this via perturbation to the morphism $\widetilde\eval_X$ into $\Delta$. To begin with there is a canonical map (an $\Se $-linear morphism of linear factorisations of $\widetilde{V}$)
