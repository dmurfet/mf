...
\widetilde\eval_X: X \otimes X^\dual \lra \Delta_W 
\, , \qquad
\widetilde\coev_X: \Delta_V \lra X^\dual \otimes X
\ee
that are the counit and unit of a (right) adjunction between~$X$ and~$X^\dual$. Similarly, one expects maps $\eval_X: X\otimes {}^\dual X \lra \Delta_V$ and $\coev_X: \Delta_W \lra {}^\dual X \otimes X$ for a left adjoint~${}^\dual X$ of~$X$, together with natural isomorphisms between~$X$ and its double-adjoint, closely related to a so-called pivotal structure. The special case where~$V$ and~$W$ are the same polynomial and depend on only one variable was worked out in~\cite{cr1006.5609}; we refer to that paper's introductory section for further motivation from the physical picture. 

In the present paper we prove that $\LG$ has adjoints and is ``pivotal up to shifts'' in the general case. We do so by developing a conceptual framework from which we obtain explicit expressions for the above adjunction maps, opening the door for many applications. The two key ingredients for our description are \textsl{homological perturbation} and the introduction of \textsl{associative Atiyah classes} to the world of matrix factorisations, following a suggestion by Buchweitz. In this way the adjunction structures can be organised in a rather clean and elegant way, the basic intuition behind which is roughly as follows: [TODO: some words on Atiyah classes, curvature, and all that] 

Regardless of the conceptual origins of the adjunction properties of $\LG$, in the end they produce very explicit formulas one can actually calculate with in practice: for a matrix factorisation~$X$ of $V-W$ with differential~$D$, the maps are given by[TODO: footnote or comment that we usually only use $X$, not $(X,D)$, to label a defect]
...
\fill (50:2) circle (3.3pt) node[right] {{\small$\lambda_{{}^\dual X}$}};
\fill (-50:2) circle (3.3pt) node[right] {{\small$\lambda^{-1}_{{}^\dual X}$}};
...
\; ,
\end{align}
where as usual (and explained in more detail in Section~\ref{subsec:bicatLG}) evaluation and coevaluation maps are denoted as caps and cups, respectively, $\lambda_{{}^\dual X}$ is the left action of $\Delta_W$ on~${}^\dual X$, and we always read diagrams like the above from bottom to top. Thus~\eqref{defectactionIntro} equals $\widetilde\eval_X[m+n] \circ (1\otimes \lambda_{{}^\dual X}) \circ (1\otimes \psi \otimes 1) \circ (1\otimes \lambda_{{}^\dual X}^{-1}) \circ \coev_X$, from which in Section~\ref{sec:defectaction} we will prove the general formula 
\be\label{DrXpsi}
\mathcal D_r(X)(\psi)  = (-1)^{{m\choose 2} + 0 (TODO)} \Res_{k[x,z]/k[z]} \left[ \frac{\psi \str\big(  \partial_{x_1} D\ldots \partial_{x_n} D \, \partial_{z_1} D\ldots \partial_{z_m} D \big) \underline{\operatorname{d}\! x}}{\partial_{x_1} W \ldots \partial_{x_n} W} \right]
\ee
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
= 
(-1)^{0 (TODO)} \Res_{k[x]/k} \!\!\left[ \frac{\psi \str\big( \Phi \Lambda_X^{(x)} \big) \underline{\operatorname{d}\! x}}{\partial_{x_1} W \ldots \partial_{x_n} W} \right] , 
\qquad
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=0.4,color=blue!50!black, baseline,>=stealth]
\nicepalecolourscheme (0,0) circle (3.5);
...
In this way we respectively recover the \textsl{Kapustin-Li disk correlator} and the \textsl{boundary-bulk map} (which reduces to the \textsl{Chern character} $(-1)^{{m+1\choose 2}} \str( \partial_{z_1} D\ldots \partial_{z_m} D)$ for $\Phi=1$). 
...
The rest of the present paper is organised as follows. In Section~\ref{sec:Background} we collect necessary background material on matrix factorisations, bicategories with adjoints, noncommutative forms, residues, and homological perturbation theory. Section~\ref{section:atiyahclasses} introduces various notions of associative Atiyah classes, which together with homological perturbation allow us to invert or lift up to homotopy certain maps pertaining to the stabilised diagonal in Section~\ref{section:pertandhtpy}. Using these results we construct explicit evaluation and coevaluation maps in Section~\ref{sec:derivcoeval} and prove that they indeed endow the bicategory $\LG$ with left and right adjoints in Section~\ref{sec:Zorro}. The next three section discuss applications of this main result: defect action on bulk fields in Section~\ref{sec:defectaction}, a bicategorical trace and its relation to open/closed topological field theory in Section~\ref{sec:shadows}, and the Cardy condition in Section~\ref{sec:CardyCondition}. Finally, in an appendix we provide a proof, similar in spirit to the construction of Section~\ref{sec:Zorro}, of the nondegeneracy of the Kapustin-Li pairing, for the general base~$k$ over which we work also in the bulk of the paper. 
...
Let us fix the ring $R=k[x_1,\ldots,x_n]$, where~$k$ is a commutative noetherian $\nQ$-algebra; relevant examples are $k=\nC$ and $k=\nC[t_1,\ldots,t_d]$. Typically we will simply write $k[x]$ for $k[x_1,\ldots,x_n]$. We call an element $W\in R$ a \textsl{potential} if $R/(\partial_{x_i}W)$ is a finitely generated free $k$-module and the partial derivatives $\partial_{x_i}W$ form a regular sequence. In this case $k=\nC$ this simply means that the Jacobi ring $R/(\partial_{x_i}W)$ is a finite-dimensional vector space. 
...
\pi: \Delta_W \lra R
\ee
for the projection $\Delta_W \lra \Re$ to the $\theta$-degree~$0$ component composed with multiplication $\Re \lra R$. Typically we will omit the wedge product and write  e.\,g.~$\theta_i\wedge \theta_j$ simply as $\theta_i \theta_j$. By making a choice of how to order the~$\theta_i$ we obtain an $\Re$-linear map 
\be\label{eq:vareps}
...
To describe the differential $d_{\Delta_W}$ we further need the variable-changing map
$$
...
[TODO: what is called left adjoint here is called right adjoint in the paper with Ingo. The reason is that ${}^\dual X$ looks stupid and Sections 5 and 6 are full of right duals. OK to have notational clash with [CR2010]?]

We say a bicategory~$\mathcal B$ \textsl{has left adjoints} if for each 1-morphism $X\in \mathcal B(A,B)$ there exists a 1-morphism ${}^\dual X \in \mathcal B(B,A)$ together with 2-morphisms 
\be\label{evcoev}
\eval_X : {}^\dual X \otimes X \lra 1_A \, , \qquad \coev_X : 1_B \lra X \otimes {}^\dual X
\ee
satisfying 
\begin{align}
\rho_X \circ (1_X \otimes \eval_X) \circ \alpha_{X,{}^\dual X,X} \circ (\coev_X \otimes 1_X) \circ \lambda_X^{-1} & = 1_X \, , \label{uglyZorro1}\\
\lambda_{{}^\dual X} \circ (\eval_X \otimes 1_{{}^\dual X}) \circ \alpha^{-1}_{{}^\dual X,X,{}^\dual X} \circ (1_{{}^\dual X} \otimes \coev_X) \circ \rho_{{}^\dual X}^{-1} & = 1_{{}^\dual X} \, . \label{uglyZorro2}
\end{align}
\end{definition}

It is convenient to denote identities in bicategories with duals in string diagram notation, see e.\,g.~\cite{JSGoTCI,JSGoTCII} for more details (\textbf{todo} So we fix specific adjunctions for each $X$). In this language the evaluation and coevaluation maps~\eqref{evcoev} are written as
$$
\eval_{X} = 
%%%%%%%%%%%%%%%%%%%%%%
...
(2,0) node[line width=0pt] (s) {\small{${}^\dual X$}}; 
...
(3,0) node[line width=0pt] (D) {{\small$X\vphantom{{}^\dual X}$}}
(2,0) node[line width=0pt] (s) {{\small${}^\dual X$}}; 
...
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=0cm]
\draw[line width=0] 
(1,1.25) node[line width=0pt] (A) {{\small ${}^\dual X$}}
(-1,-1.25) node[line width=0pt] (A2) {{\small ${}^\dual X$}}; 
\draw[directed] (0,0) .. controls +(0,1) and +(0,1) .. (-1,0);
\draw[directed] (1,0) .. controls +(0,-1) and +(0,-1) .. (0,0);
\draw (-1,0) -- (A2); 
\draw (1,0) -- (A); 
...
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=0cm]
\draw[line width=0] 
(0,1.25) node[line width=0pt] (A) {{\small ${}^\dual X$}}
(0,-1.25) node[line width=0pt] (A2) {{\small ${}^\dual X$}}; 
\draw (A2) -- (A); 
\end{tikzpicture} \, .
\ee

Similarly one says that~$X^\dual$ is \textsl{right adjoint} to~$X$ if there are maps 
$$
\widetilde\eval_{X} = 
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=.6cm]
...
Specifying the adjoint of a $1$-morphism is often straightforward, but providing explicit evaluation and coevaluation maps and checking that the Zorro moves hold is no easy task. For example, given a $1$-morphism $X: (k[x_1,\ldots,x_n],W) \lto (k[z_1,\ldots,z_m],V)$ in $\LG$, that is, an object $X \in \hmf(k[x,z], V - W)$, we will see that its left and right adjoints are given by ${}^\dual X = X^\vee[m]$,  $X^\dual = X^\vee[n]$ with differentials
\be\label{eq:differentials_adjoints}
d_{{}^\dual X} = \begin{pmatrix} 0 & (d_X^0)^\vee \\ -(d_X^1)^\vee & 0\end{pmatrix}[m] 
\, , \qquad 
d_{X^\dual} = \begin{pmatrix} 0 & (d_X^0)^\vee \\ -(d_X^1)^\vee & 0\end{pmatrix}[n] 
\ee
where $(-)^\vee$ denotes taking the dual in the category of $k[x,z]$-modules. On the other hand, the whole of Sections~\ref{sec:derivcoeval} and~\ref{sec:Zorro} are devoted to construct the evaluation and coevaluation morphisms and prove the Zorro moves. This is complicated enough that instead of tackling it directly we will need to introduce the completed bar complex of the ring~$R$. As preparation we shall next discuss some background on noncommutative forms. 
...
\textsl{Noncommutative $n$-forms over~$A$} are elements in 
$$
\Omega^n A = A\otimes \bar A^{\otimes n} 
$$
where $\bar A = A/k$ and in this section by ``$\otimes$'' we mean ``$\otimes_k$''. We denote the projection of $a_0\otimes a_1 \otimes \ldots \otimes a_n \in A^{\otimes (n+1)}$ to $\Omega^n A$ as $(a_0,a_1,\ldots,a_n)$. The direct sum 
$$
...
\begin{lemma} There is a natural morphism of linear factorisations of $V - W$, 
$$
...
\begin{lemma} There are natural isomorphisms of linear factorisations of $W + V$, 
$$
...
One can easily check that for $\omega \in \Bar$ we have
\[
...
\At_{2}(X)^m( e_i \otimes \omega ) = (-1)^{m|e_i| + \binom{m+1}{2}} \sum_{j_1,\ldots,j_m} e_{j_m} \otimes d( D_{j_m, j_{m-1}} ) \ldots d(D_{j_2,j_1})d( D_{j_1 i} ) \omega\,.
\end{equation}
% for signs see for example shati
There are similar formulas for $\At_{1}$. We call these \textsl{associative} Atiyah classes to distinguish them from the more standard Atiyah classes defined using commutative differential forms, but since the latter do not appear in this paper (with the exception of Appendix \ref{section:dualityadjointop}) we will omit the qualifier ``associative''. Without conditions on $D^2$ there is no interpretation of $\At_{2}$ as a cohomology class, so we should perhaps refer to Atiyah \textsl{operators}, but we persist with the above terminology.

To discuss the linearity we need to be clear that $X \otimes \Bar$ is made into a right $\Re $-module via the right action of $\Re $ on $\Bar$ by multiplication. It is then apparent from \eqref{eq:powerofatiyah} that both $\At_{2}(X)$ and $\At_{1}(X)$ are right $\Re $-linear. The subscripts on the Atiyah classes arise because we think of $d$ (resp.~$s$) as differentiating in the $R_1$-directions (resp.~$R_2$-directions).

\begin{remark}\label{remark:linearatother} 
Suppose that $S$ is a $k$-algebra and $X$ a $\mathbb{Z}_2$-graded free $(S \otimes_k \Re)$-module with $(S \otimes_k \Re)$-linear operator $D$. If we choose a homogeneous $(S \otimes_k \Re)$-basis for~$X$ and extend $d,s$ to $S$-linear operators on $X \otimes \Bar$ via \eqref{eq:extendd}, \eqref{eq:extends} then $[d, D]$ and $[s, D]$ are $S$-linear and right $\Re $-linear operators on $X \otimes \Bar$ and we use the same notation and terminology for these operators. All our remarks extend \textsl{mutatis mutandis} to this kind of Atiyah class.
\end{remark}
...
\end{equation}
From this expression it is clear that $\lAt_{2}$ and $\lAt_{1}$ are both left $\Re $-linear, using the action of left multiplication on $\Bar$ in $\Bar \otimes X$.
...
\ee
We adopt similar notational conventions for the other Atiyah classes.

\subsection{Properties}

With the above notation, let $(X,d_X)$ and $(Y,d_Y)$ be $\mathbb{Z}_2$-graded free $\Re $-modules equipped with odd $\Re $-linear differentials (again, no condition on $d_X^2$ or $d_Y^2$) and respective homogeneous bases $\{ e_i \}_{i}$ and $\{ f_j \}_{j}$. Throughout all Atiyah classes are of the same ``type'' and we write $\At$ for $\At_{2}$. 

Giving $X \otimes Y$ the differential $d_{X \otimes Y} = d_X \otimes 1 + 1 \otimes d_Y$ and the basis $\{ e_i \otimes f_j \}_{i,j}$ we have
\begin{align*}
...
\]
But considering \eqref{eq:powerofatiyah} it is straightforward to check that the right-hand side of~\eqref{eq:atshufat} is a sum over the same collection of terms, and the signs match.
\end{proof}
...
\]
The solution to this lifting problem is given in Proposition \ref{prop:liftingresult}. In order to address these examples and others at the same time, we work in the following general setting: $k$ is a ring, $R, S$ are $k$-algebras and $\Re  = R_1 \otimes_k R_2$ where $R_i = R$ for $i \in \{1,2\}$. We assume
\[
X \in \HMF(S \otimes_k \Re, U)
\]
is given together with a homogeneous basis $\{ e_i \}_{i}$. The reader should keep in mind the special case where $S = k[z], R = k[x]$ and $X$ the extension of scalars from $S \otimes_k R$ to $S \otimes_k \Re$ of some factorisation of $V - W$ over $k[x,z]$. Let $D$ denote the differential on $X$ and unless specified otherwise in this section $\otimes$ denotes the tensor product $\otimes_{\Re}$. Consider the module
\[
...
\]
where $\Bar_{\ge m} = \bigoplus_{i \ge m} \Bar_i \subseteq \Bar$ and the maps are the obvious quotients $\Bar/\Bar_{\ge m+1} \lto \Bar/\Bar_{\ge m}$. Since $\Bar$ is an $\Re $-bimodule via left and right multiplication there are natural left and right $\Re $-actions on $X \,\widehat{\otimes}\, \Bar$. Since the left and right actions of $R_2$ on $\Bar$ agree, this amounts to an $R_2$-action on $X \,\widehat{\otimes}\, \Bar$ together with the structure of an $R_1$-bimodule. There is of course an $S$-module structure as well, which is preserved by all maps in this section and which we will henceforth not emphasise.
...
\end{align*}
Clearly $\pi \delta = \pi D$ and $\pi$ vanishes on $\delta d$, so $\pi \tau = \pi D$. It follows that $b_\infty = D \otimes 1$ and $\pi_\infty = \pi - \pi D d = \pi$. So there is a deformation retract datum of the stated form, where we may use $d^2 = 0$ and $d \sigma_2 = 0$ to write
\[
...
&+ \ldots + (-1)^n a_0 da_1 \ldots da_n dW \otimes a_{n+1}\,.
\end{align*}
On the other hand
\begin{align*}
\alpha \times d\omega &= dW da_0 da_1 \ldots da_n \otimes a_{n+1} - da_0 dW da_1 \ldots da_n \otimes a_{n+1}\\
&+ \ldots + (-1)^{n+1} da_0 da_1 \ldots da_n dW \otimes a_{n+1}\,,
\end{align*}
from which it is clear that $d( \alpha \times \omega ) + \alpha \times d\omega = \alpha \cdot d\omega$.
\end{proof}

%The left $R$-linear inverse to $\pi$ is given in terms of the Atiyah class $\At_{1}(X) = [s,d_X]$.
...
\begin{equation}
\pi^{-1}_2 = \sum_{m \ge 0} (-1)^m \Psi \At_{2}(X)^m \sigma_2\,,
\end{equation}
while a left $R_1$-linear homotopy inverse is given by
\begin{equation}
\pi^{-1}_1 = \sum_{m \ge 0} (-1)^m \Psi \At_{1}(X)^m \sigma_1\,.
\end{equation}
where $\sigma_1, \sigma_2$ are as in \eqref{eq:sigmsandd}.
\end{corollary}

With these formulas in hand we return to the lifting problem discussed at the beginning of this section. We show that given $Y \in \hmf(\Re , \widetilde{W})$ composing with $\pi_{\Delta}$ induces a quasi-isomorphism
\be\label{eq:liftingresult0}
\Hom_{\Re }(Y, \Delta) \lto \Hom_{\Re }(Y, R)\,.
\ee
In particular, given an $\Re $-linear morphism of linear factorisations $\varphi: Y \lto R$ there is a unique (up to homotopy) $\Re $-linear morphism $\varphi_{\textup{lift}}$ making the following diagram commute up to homotopy
\[
\xymatrix{
...
\]
To produce a formula for $\varphi_{\textup{lift}}$ we choose a homogeneous basis $\{ e_i \}_{i}$ for $Y$.

\begin{proposition}\label{prop:liftingresult} 
The map \eqref{eq:liftingresult0} is a homotopy equivalence, and in the above notation
\begin{equation}\label{eq:liftingresult}
\varphi_{\textup{lift}} = \sum_{m \ge 0} (-1)^m \Psi \circ \varphi' \circ \lAt_{2}(Y)^m
\end{equation}
...
\[
\xymatrix@C+2pc{
Y \ar[r]^-{[d, d_Y]^m} & \Bar \otimes_{\Re } Y \ar[r]^-{1 \otimes \varphi'} & \Bar \otimes_{\Re } \Re  \cong \Bar \ar[r]^-{\Psi} & \Delta
}\,.
\]
\end{remark}
% see lipert3
\begin{proof}[Proof of Proposition~\ref{prop:liftingresult}]
We apply Corollary \ref{corollary:koszulstab2} with $X = Y^{\vee}$ to see that the composite
\[
\xymatrix@C+2pc{
\Hom_{\Re }(Y, \Delta) \ar[r]^-{\cong}_-{\xi^{-1}} & Y^{\vee} \otimes_{\Re } \Delta \ar[r]^-{\pi_{\Delta}} & Y^{\vee} \otimes_{\Re } R \ar[r]^-{\cong}_-{\xi} & \Hom_{\Re }(Y, R)
}
\]
is a homotopy equivalence, with $\xi$ denoting the canonical isomorphisms. Evaluating the homotopy inverse $\xi \circ \pi_2^{-1} \circ \xi^{-1}$ on the cohomology class of $\varphi$ yields the map $\varphi_{\textup{lift}}$ defined by
\[
\varphi_{\textup{lift}} = \sum_i \sum_{m \ge 0} (-1)^m \xi \Psi [d, d_X]^m \sigma_2( e_i^* \otimes \varphi(e_i) )\,.
\]
A straightforward computation yields (with $D = d_Y$)
\[
\varphi_{\textup{lift}}(e_j) = \sum_{m \ge 0}\sum_{i_0, i_1, \ldots, i_{m-1}} (-1)^{m|e_{i_0}| + m} \Psi d( D_{i_{m-1},j}) d( D_{i_{m-2},i_{m-1}} ) \ldots d( D_{i_0, i_1} ) \cdot (1 \otimes \varphi(e_{i_0}) )
\]
...
For future reference, let us conclude by stating the special case of inverses for the unit actions in \eqref{eq:pertandhtpy1}. So we take $S = k[z]$, $R = k[x]$, and a finite-rank matrix factorisation~$X$ of $U = V - W$ over $k[x,z]$. Applying Corollary~\ref{corollary:koszulstab2} to $X \otimes_R \Re$ or $\Re \otimes_R X$ we deduce: 

\begin{proposition}\label{prop:inverses_unitact} 
[TODO: join with \ref{corollary:koszulstab2}]
$k[z]$-$k[x]$-bilinear homotopy inverses for $\lambda_X$ and $\rho_X$ are given by
\[
\lambda_X^{-1} = \sum_{m \ge 0} (-1)^m \Psi \Atlarrow_{1}(X)^m \sigma_1\,, \qquad \rho_X^{-1} = \sum_{m \ge 0} (-1)^m \Psi \At_{2}(X)^m \sigma_2\,.
\]
\end{proposition}
...
In the bicategory $\LG$ a $1$-morphism $W \lto V$ is represented by a finite rank matrix factorisation $X$ of $V - W$ over $k[x,z]$, where $W\in k[x] = k[x_1,\ldots,x_n]$ and $V\in k[z] = k[z_1,\ldots,z_m]$ are potentials. The \textsl{right} and \emph{left adjoints} of $X$ are the $1$-morphisms $V \lto W$ defined respectively by
\[
X^\dual = X^\vee[n], \qquad {}^\dual X = X^\vee[m]\,.
\]
These are objects of $\hmf(k[x,z], W-V)$, with the differentials spelled out in \eqref{eq:differentials_adjoints}.

In Sections \ref{subsec:derivcoeval} and \ref{subsec:eval} below we define canonical evaluation and coevaluation morphisms 
\begin{align}
...
\begin{gather}
\widetilde\coev_X(\gamma) = \varepsilon \left( \gamma \wedge \sum_{l \ge 0} (-1)^l \Psi \Atlarrow_2(X^\vee)^l( \iota_X ) \right)\,, \label{eq:coev10}\\
\iota_X = \sum_{j} (-1)^{|e_j|} e_j^* \otimes e_j \in X^\dual \otimes_{k[z]} X \nonumber
\end{gather}
and
\begin{gather}
\coev_X( \gamma ) = \sum_{l \ge 0} (-1)^l \varepsilon \left( \gamma \wedge \Psi \Atlarrow_2(X)^l( \iota_X ) \right)\,,\label{eq:coev102}\\
\iota'_X = \sum_j (-1)^{m|e_j|} e_j \otimes e_j^* \in X \otimes_{k[x]} {}^\dual X\,. \nonumber
\end{gather}
A representative for the first evaluation morphism is given by ($\Lambda = \partial_{x_1}(d_X) \cdots \partial_{x_n}(d_X)$)
\be\label{eq:constructevalmap00}
\widetilde\eval_X( \eta \otimes \nu ) = \sum_{l \ge 0} (-1)^{n+l+n|\eta|+|\nu|} \Res_{k[x]/k} \left[ \frac{ \Psi \nu\left( \At_2(X)^l( \Lambda \eta ) \right) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]\,.
\ee
Since $\nu$ and $\eta$ involve different copies of the $z$-variables this formula is slightly dishonest: it does not naively make sense to evaluate $\nu$ on $\At_2(X)^l( \Lambda \eta )$. A fully accurate statement is given as Proposition \ref{prop:constructevalmap}. In the other evaluation the Atiyah class of $X^{\vee}$ is applied to the functional $\nu \circ \Lambda$, and the resulting functional-valued form is evaluated on $\eta$
\be\label{eq:constructevalmap01}
\eval_X( \nu \otimes \eta ) = \sum_{l \ge 0} (-1)^{\binom{n}{2}+l+n|\nu|} \Res_{k[x]/k} \left[ \frac{ \Psi \left( \At_2(X^{\vee})^l( \nu \circ \Lambda ) (\eta) \right) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]\,.
\ee
To simplify notation we set $R = k[x], S = k[z]$ and write $\Re  = R_1 \otimes_k R_2$ with $R_i = R$ and $\Se  = S_1 \otimes_k S_2$ with $S_i = S$. Then $\Delta_W$ and $X^\dual \otimes X$ are $\Re $-linear factorisations of $\widetilde{W} = W \otimes 1 - 1 \otimes W$ and $\eval_X, \widetilde\coev_X$ are $\Re $-linear, while $\Delta_V$ and $X \otimes X^\dual$ are $\Se$-linear factorisations of $\widetilde{V} = V \otimes 1 - 1 \otimes V$ and $\coev_X, \widetilde\eval_X$ are $\Se$-linear. The differentials on $\Delta_V, \Delta_W$ are given by \eqref{DeltaW}. 



\subsection{Coevaluation}\label{subsec:derivcoeval}

In this section $\Delta$ denotes $\Delta_W$. The coevaluation is produced by computing a chain representative for the inverse image of the identity under the canonical quasi-isomorphism of the next result. We will make use of the various canonical maps recalled in Section \ref{section:canonicalmaps}.

\begin{proposition}\label{prop:isogivescoev} There is a canonical homotopy equivalence of $\mathbb{Z}_2$-graded $k$-complexes
\begin{equation}
\Hom_{\Re }( \Delta, X^\dual \otimes_{S} X ) \lto \Hom_{R \otimes_k S}(X,X)\,. \label{eq:isogivescoev}
\end{equation}
\end{proposition}
\begin{proof}
...
\]
Composing with the natural morphism $\varepsilon: K \lto \Re[n]$ from~\eqref{eq:vareps} and taking the adjoint, we obtain the map
\[
\zeta: \Delta' \lto \Delta^\vee [n]\, , \qquad \omega \lmt \varepsilon( \omega \wedge - )
\]
which is an isomorphism of matrix factorisations. Combining this with various canonical maps and the morphism $\pi: \Delta' \lto \Delta'_0 = \Re \lto R$ we have the following diagram of $R_2$-linear morphisms:
\begin{equation}
\xymatrix{
\Hom_{\Re}( \Delta, X^\vee[n] \otimes_{S} X)\\
\Delta^\vee \otimes_{\Re} \left( X^\vee[n] \otimes_S X \right) \ar[u]^-{\cong}_-{\xi}\\
\Delta^\vee[n] \otimes_{\Re} \left( X^\vee \otimes_{S} X \right) \ar[u]^-{\cong}\\
\Delta' \otimes_{\Re} \left( X^\vee \otimes_S X \right) \ar[u]^-{\cong}_-{\zeta \otimes 1}\\
\left( X^\vee \otimes_{S} X \right) \otimes_{\Re} \Delta' \ar[u]^-{\cong}_-{\textup{swap}} \ar[d]^-{\pi}\\
\left( X^\vee \otimes_{S} X \right) \otimes_{\Re} R \ar[d]_-{\cong}\ar[d]^-{\kappa}\\
\Hom_{R \otimes_k S}(X,X)
}\label{eq:isogivescoev2}
\end{equation}
where for $f \in X^\vee$ and $g,g' \in X$: 
\[
\kappa( f \otimes g )(g') = (-1)^{|f||g|} f(g') \cdot g\,.
\]
To complete the proof we need only to show that the step marked $\pi$ is a homotopy equivalence. But this is a consequence of Lemma \ref{lemma:koszulstab1}.
\end{proof}

\begin{definition}\label{def:coeval}
$\widetilde\coev_X: \Delta \lra X^\dual \otimes X$ is the morphism in the homotopy category $\HMF(\Re, \widetilde{W})$ whose cohomology class maps to $1_X$ under the quasi-isomorphism \eqref{eq:isogivescoev}.
\end{definition}
...
With $\reprod$ as in (\ref{eq:attau}) we consider on the tensor product $\reprod_*(\Bar) \otimes_{\Re} ( X^\vee \otimes_{S} X)$ the operator
\[
...
\[
\xymatrix@C+1pc{
X^\vee \otimes_{S} X \ar[rr]^-{\Atlarrow_2(X^\vee)^m} & & \reprod_*(\Bar) \otimes_{\Re} ( X^\vee \otimes_{S} X ) \ar[r]^-{\Psi} & \Delta \otimes_{\Re} (X^\vee \otimes_{S} X)
}
\]
which we can apply to the tensor $\iota_X = \sum_{j} (-1)^{|e_j|} e_j^* \otimes e_j$.

\begin{theorem}\label{theorem:coev1} A representative for the coevaluation morphism is the chain map \eqref{eq:coev10}.
\end{theorem}
\begin{proof}
...
\end{equation}
Applying the swap isomorphism has the effect of exchanging $\At_2$ for $\Atlarrow_2$ in this formula, so that the cohomology class in $\Delta' \otimes_{\Re} (X^\vee \otimes_S X)$ mapping to $1_X$ is $\sum_{m \ge 0} (-1)^m \Psi \Atlarrow_2(X^{\vee})^m( \iota_X )$.

A tensor $\omega \otimes \alpha$ in $\Delta' \otimes_{\Re} (X^\vee \otimes_S X)$ is sent under the remaining maps in \eqref{eq:isogivescoev2} to the function $(-1)^{n(|\omega| + n)} \xi( \varepsilon( \omega \wedge - ) \otimes \alpha )$. This map sends a form $\gamma \in \Delta$ to
\begin{align}
...
\end{align}
This vanishes unless $|\gamma| = n - |\omega|$, so the final sign is $(-1)^{|\omega||\alpha| + n|\alpha| + n|\omega| + |\omega|}$. Since $\iota_X$ has degree zero the element $\sum_{m \ge 0} (-1)^m \Psi \Atlarrow_2(X^{\vee})^m( \iota_X )$ can be written as a sum of tensors $\omega \otimes \alpha$ with $|\omega| = |\alpha| = m$, in which case the sign on \eqref{eq:signcalccoeval} is just $+1$ and the formula \eqref{eq:coev10} follows.
\end{proof}

There is an alternative presentation which we will need. On $( X^\dual \otimes X) \otimes_{\Re } \Bar$ consider
\[
\At_{1}(X^\dual \otimes X) = [s, d_{X^\dual \otimes X}] = [s, d_X]
\]
with the second equality holding because $s$ is $k[x]$-linear. We write $\At_{1}(X)$ for this operator. 

\begin{theorem} A representative for the coevaluation morphism is the chain map
\[
\widetilde\coev^R_X(\gamma) = \varepsilon \left( \sum_{m \ge 0} (-1)^{m+mn} \Psi \At_{1}(X)^m( \iota_X ) \wedge \gamma \right)\,.
\]
\end{theorem}
...
\subsubsection{Other coevaluation}

On the theory that it's better to have \emph{something} rough here rather than nothing, with the same notation as above the other coevaluation is a morphism
\[
\coev_X : \Delta_V \lra X \otimes_R X^\vee[m]\,.
\]
If we define
\[
\iota'_X = \sum_j (-1)^{m|e_j|} e_j \otimes e_j^* \in X \otimes_R X^\vee[m]
\]
then
\[
\coev^L_X( \gamma ) = \sum_{i \ge 0} (-1)^i \varepsilon \left( \gamma \wedge \Psi \Atlarrow_2(X)^i( \iota'_X ) \right)
\]
and the alternative formula is
\[
\coev^R_X( \gamma ) = \sum_{i \ge 0} (-1)^{i + im} \varepsilon \left( \Psi \At_1(X^{\vee})^i( \iota'_X ) \wedge \gamma \right)\,.
\]

\subsection{Evaluation}\label{subsec:eval}
% see (eval)

In this section $\Delta$ denotes $\Delta_V$. The evaluation will be produced by writing down a natural morphism $\widetilde\eval_0: X \otimes_R X^\dual \lto S$ and then lifting this via perturbation to the morphism $\widetilde\eval_X$ into $\Delta$. To begin with there is a canonical map (an $\Se $-linear morphism of linear factorisations of $\widetilde{V}$)
\be\label{eq:firstisoevalsec}
X \otimes_R (X^{\vee}[n]) \cong (X \otimes_R X^\vee)[n] \lto S \otimes_{\Se } (X \otimes_R X^{\vee})[n] \cong \Hom_{S \otimes_k R}(X,X)[n]\,.
\ee
sending $e_i \otimes e_j^*$ to $(-1)^{n|e_i|} e_i \circ e_j^*$.

The partial derivatives $f_i = \partial_{x_i} W$ act null-homotopically on $X$ and we let $\lambda_i \in \Hom_{R \otimes_k S}(X,X)$ denote a degree-one map with $[d_X, \lambda_i] = f_i \cdot 1_X$. We also set 
$$
\Lambda = \lambda_1 \ldots \lambda_n \, . 
$$ 
For example the operators $\lambda_i = \partial_{x_i} d_X$ would do, but it will be important to allow other choices. 

Postcomposing with $\Lambda$ gives an $S$-linear map (we define $\Lambda^\bullet(\alpha) = \Lambda \circ \alpha)$
\[
\xymatrix@C+2pc{
\Hom_{S \otimes_k R}(X, X)[n] \ar[r]^-{(-1)^n \Lambda^\bullet} & \Hom(\bar{X}, \bar{X}) := \Hom_{S \otimes_k R}(X, X) \otimes_R R/(f_1,\ldots,f_n)\,.
}
\]
This is actually a morphism of complexes, since the commutator of $\Lambda$ with the differentials on the mapping complexes is an expression in the $f_i$.

The supertrace gives an $S$-linear morphism of complexes
\[
\xymatrix@C+2pc{
\Hom(\bar{X}, \bar{X}) \ar[r]^-{\str} & S \otimes_k R/(f_1,\ldots,f_n)
}
\]
and finally the residue symbol (see Section \ref{section:residuebackground}) gives an $S$-linear map
\[
\Res_{S \otimes_k R/S} \left[ \frac{ (-) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]: S \otimes_k R/(f_1,\ldots,f_n) \lto S\,.
\]
The composite of the above is the desired morphism $\eval_0$.

\begin{lemma}\label{lemma:morphismevalzero} There is a morphism of linear factorisations of $\widetilde{V}$ over $\Se $
\[
\eval_0: X \otimes_{R} X^\dual \lto S\,,
\]
defined for homogeneous $\eta \in X$ and $\nu \in X^\dual$ by
\begin{align}
\eval_0( \eta \otimes \nu ) &= (-1)^{n|\eta| +n} \Res_{\Se  \otimes_k R/\Se } \left[ \frac{ \str( \Lambda \circ \eta \circ \nu) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]\\
&= (-1)^{n|\eta| + |\nu| +n} \Res_{\Se  \otimes_k R/\Se } \left[ \frac{ \nu \Lambda( \eta ) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]
\end{align}
\end{lemma} 

There is a stabilisation morphism $\pi_\Delta: \Delta \lto S$, and while $X \otimes_R X^\dual$ is not free of finite rank over $\Se $, it is a direct summand of a finite-rank matrix factorisation in $\HMF(\Se , \widetilde{V})$. The unique lifting statement of Proposition \ref{prop:liftingresult} can be extended to summands (for the argument, see the proof of the proposition below) so there is up to homotopy a unique morphism of matrix factorisations $\widetilde\eval_X$ making the following diagram commute:
\be\label{eq:defineeval}
\xymatrix{
X \otimes_R X^\dual \ar[dr]_-{\eval_0} \ar@{.>}[rr]^-{\widetilde\eval_X} & & \Delta \ar[dl]^-{\pi_\Delta} \\
& S
}\,.
\ee
...
Before stating the formula for $\widetilde\eval_X$ we need to introduce the $S^e \otimes_k R$-linear map
\begin{gather*}
\theta: X \otimes_R X^{\vee} \lto S^e \otimes_k R\,,\\
\theta( e_i \otimes e_j^* ) = (-1)^{|e_i||e_j|} e_j^*( e_i ) = (-1)^{|e_i|} \delta_{ij}\,.
\end{gather*}

\begin{proposition}\label{prop:constructevalmap} A representative for the evaluation morphism is the chain map
\[
\widetilde\eval_X: X \otimes_R X^\dual \lto \Delta
\]
defined for homogeneous $\eta \in X$ and $\nu \in X^\dagger$ by
\be\label{eq:constructevalmap0}
\widetilde\eval_X( \eta \otimes \nu ) = \sum_{m \ge 0} (-1)^{m+n+n|\eta|} \Res_{\Se  \otimes_k R/\Se } \left[ \frac{ \Psi \theta\left( \At_2(X)^m \Lambda ( \eta ), \nu \right) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]
\ee
\end{proposition}

(\textbf{Todo} redo this part) Before giving the proof, let us be clear that the term under the residue is the composite
\[
\xymatrix@C+1pc{
X \ar[r]^{\Lambda} & X \ar[r]^-{\inc} & \Bar \otimes_S X \ar[r]^-{\At_2(X)^m} & \Bar \otimes_S X \ar[r]^-{1 \otimes \nu} & \Bar \otimes_S S \otimes_k R \cong \Bar \otimes_k R \ar[r]^-{\Psi \otimes 1} & \Delta \otimes_k R
}
\]
where $\At_2(X) = [d, d_X]$. The residue integrates out the $R$, leaving us with an element of $\Delta$.

As has already been mentioned, if $X \otimes_R X^\dual$ were finite-rank we could apply Proposition \ref{prop:liftingresult} directly to deduce a formula for the lifting $\widetilde\eval_X$. As this is not the case, we have to be a little indirect and appeal to the idempotent pushforward construction of \cite{??}. Specifically, with $\bar{R} = R/(f_1,\ldots,f_n)$, Theorem \ref{??} of \textsl{loc.cit.} shows that there is a diagram
\[
\xymatrix@C+2pc{
(X \otimes_R X^\vee)[n] \ar@<-1ex>[r]_-{\vartheta} & \bar{X} \otimes_{\bar{R}} \bar{X}^\vee := (X \otimes_R X^\vee) \otimes_R \bar{R} \ar@<-1ex>[l]_-{\psi}
}
\]
in $\HMF(\Se , \widetilde{V})$ with $\psi \circ \vartheta = 1$ in the homotopy category and $\vartheta = (-1)^n \Lambda \otimes 1$. Note that since $W$ is a potential, $R/(f_1,\ldots,f_n)$ is a finitely generated free $k$-module and $\bar{X} \otimes_{\bar{R}} \bar{X}^\vee$ is a finite-rank matrix factorisation.

\begin{proof}[Proof of Proposition \ref{prop:constructevalmap}] (\textbf{todo} Proof stupid, ignore) Consider the commutative diagram
\be\label{eq:proofconstructevalmap}
\xymatrix@C+2pc{
\Hom_{\Se }( (X \otimes_R X^\vee)[n], \Delta ) \ar[d]_{\pi^\bullet_\Delta} \ar@<-1ex>[r]_-{\psi_\bullet} & \Hom_{\Se }(\bar{X} \otimes_{\bar{R}} \bar{X}^\vee, \Delta ) \ar@<-1ex>[l]_-{\vartheta_\bullet} \ar[d]^{\pi^\bullet_\Delta}\\
\Hom_{\Se }( (X \otimes_R X^\vee)[n], S ) \ar@<-1ex>[r]_-{\psi_\bullet} & \Hom_{\Se }( \bar{X} \otimes_{\bar{R}} \bar{X}^\vee, S) \ar@<-1ex>[l]_-{\vartheta_\bullet}
}\,.
\ee
where a ``$\bullet$'' as a superscript indicates postcomposition and as a subscript it denotes precomposition. If we ignore the first isomorphism of \eqref{eq:firstisoevalsec} which only contributes a sign, then $\eval_0$ is a closed element in the bottom left corner of the diagram. By Proposition \ref{prop:liftingresult} the right-hand vertical map is a homotopy equivalence and therefore so is the left-hand vertical map. This in particular justifies why in \eqref{eq:defineeval} there is a unique morphism $\widetilde\eval_X$ making the diagram commute: this map is the inverse image, in cohomology, of $\eval_0$ under $\pi_{\Delta}^\bullet$.

The following diagram commutes
\[
\xymatrix{
X \otimes_R X^\vee \ar[d]_{(-1)^n \Lambda \otimes 1}\ar[r] & \Hom_{S \otimes_k R}(X, X) \ar[d]^{ (-1)^n \Lambda^\bullet}\\
X \otimes_R X^\vee \ar[r] & \Hom_{S \otimes_k R}(X, X)
}\,,
\]
where the rows are as in \eqref{eq:firstisoevalsec}, from which we deduce that $\eval_0 = \vartheta_\bullet( \eval'_0 )$, where $\eval'_0$ is the map
\[
\xymatrix{
\bar{X} \otimes_{\bar{R}} \bar{X}^\vee \ar[r] & S \otimes_{\Se } (\bar{X} \otimes_{\bar{R}} \bar{X}^\vee) \cong \Hom(\bar{X}, \bar{X}) \ar[r] & S
}
\]
defined by
\[
\eval'_0( \eta \otimes \nu ) = \Res_{S \otimes_k R/S} \left[ \frac{ \str( \eta \circ \nu ) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]\,.
\]
We denote by $\eval'_{0,\textup{lift}}$ the morphism $\bar{X} \otimes_{\bar{R}} \bar{X}^\vee \lto \Delta$ lifting $\eval'_0$ which is produced by Proposition \ref{prop:liftingresult}. To run the construction we need to fix an $\Se $-basis for $\bar{X} \otimes_{\bar{R}} \bar{X}^\vee$. The basis we pick is $\{ g_\alpha e_i \otimes e_j ^* \}_{\alpha, i, j}$ where $g_\alpha$ gives a $k$-basis of $\bar{R}$. We define $\widetilde\eval_X = \vartheta_\bullet( \eval'_{0, \textup{lift}} ) = (-1)^n \eval'_{0, \textup{lift}} \circ (\Lambda \otimes 1)$. By construction this morphism makes \eqref{eq:defineeval} commute, and it just remains to compute it explicitly.

We have
\[
\widetilde\eval_X = \sum_{m \ge 0} (-1)^{m+n} \Psi \circ (1 \otimes \eval''_0) \circ [d, d_{X \otimes X^\vee}]^m \circ ( \Lambda \otimes 1 )
\]
where $\eval''_0: \bar{X} \otimes_{\bar{R}} \bar{X}^\vee \lto \Se $ is the $\Se $-linear map defined on the basis by
\[
\eval''_0( f_\alpha e_i \otimes e_j^* ) = 1 \otimes \eval'_0( f_\alpha e_i \otimes e_j^* )\,.
\]
Since in $d_{X \otimes X^\vee} = d_X \otimes 1 + 1 \otimes d_{X^\vee}$ only $d_X$ involves polynomials in $R_1$, we have $[d, d_{X \otimes X^\vee}] = [d, d_X]$. We compute using \eqref{eq:leftatiyahexp} that for $g \in R$ (writing $D = d_X$, and putting in the sign $n|e_i|$ from the first map of \eqref{eq:firstisoevalsec})
\begin{align*}
\widetilde\eval_X( g e_i \otimes e_j^* ) &= \sum_{m \ge 0} (-1)^{m+n+n|e_i|} \Psi (1 \otimes \eval''_0)\left( g \sum_k [d, d_X]^m( \Lambda_{ki} e_k \otimes e_j ^*) \right)\\
&= \sum_{m \ge 0} (-1)^{m+n+n|e_i|} \Psi (1 \otimes \eval''_0)\left( g \sum_{k, k_1, \ldots, k_m} \Lambda_{ki} \cdot d( D_{k_1k} ) \ldots d( D_{k_m, k_{m-1}} ) \otimes e_{k_m} \otimes e_j^* \right)
\end{align*}
Applying $\eval''_0$ involves taking the supertrace $\str( e_{k_m} \circ e_j^* ) = (-1)^{|e_j|}\delta_{j,k_m}$ so that the above equals
\[
\sum_{m \ge 0} \sum_{k,k_1,\ldots,k_{m-1}} (-1)^{m+n+n|e_i|+|e_j|} \Lambda_{ki} \Psi( d(D_{k_1,k}) \ldots d(D_{j,k_{m-1}}) ) \Res_{S \otimes_k R/S} \left[ \frac{ g \,  \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]
\]
which agrees with \eqref{eq:constructevalmap0} for $\eta = g e_i$ and $\nu = e_j^*$, completing the proof.
\end{proof}
...
As before let $\lambda_i$ be a null-homotopy on $X$ for the action of $\partial_{x_i} W$, for example $\lambda_i=\partial_{x_i}d_X(x,z)$, and set 
$$
\Lambda = \lambda_1\ldots \lambda_n \, . 
$$
Postcomposition with $\Lambda$ gives a well-defined operator $\Lambda^\bullet$ on $\End(X) \otimes_{R_1} \Bar$.
...
Before giving the proof of the lemma, let us roughly sketch how it leads to an argument that $\mathcal Z$ is homotopic to the identity. Although we will not formalise the argument in precisely these terms, the Atiyah class is ``anti-self-adjoint'' with respect to $\langle\!\langle \str( - ) \rangle\!\rangle$: 
...
\begin{align*}
\rho^{-1}: X \lto X \otimes \Delta \, , \qquad \rho^{-1} &= \sum_{l \ge 0}(-1)^l \Psi \At_2(X)^l \sigma_2\,,\\
\widetilde{\coev}: \Delta \lto X^{\dagger} \otimes X \, , \qquad \widetilde{\coev}(\gamma) &= \varepsilon\Big( \gamma \wedge \sum_{l \ge 0}(-1)^l \Psi \Atlarrow_2(X^{\vee})^l(\iota_X) \Big)\,,\\
&= \sum_{l,j} (-1)^{l+|e_j|} \varepsilon \Big( \gamma \wedge \Psi\!\At_2(X^{\vee})^l( e_j^* ) \otimes e_j \Big)\\
\widetilde{\eval}: X \otimes X^{\dagger} \lto \Delta \, , \qquad \widetilde{\eval}(\eta \otimes \nu) &= (-1)^{n+n|\eta| + |\nu|}\Res_{\Se  \otimes_k R/\Se }\! \left[ \frac{ \Psi \nu \Lambda( \eta ) \underline{\operatorname{d}\! x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right] + \mathcal{O}(\theta)\,.
\end{align*}
...
Throughout the proof we write $\At$ for $\At_2$. The image of a basis element $e_q$ under the first two maps of \eqref{eq:zorro1a1} is computed by
\begin{align}
(1 \otimes \widetilde{\coev}) \circ \rho^{-1}( e_q ) &= (1 \otimes \widetilde{\coev}) \sum_{l \ge 0} (-1)^l \Psi \At(X)^l (e_q)\nonumber\\
&= \sum_{j} \sum_{l+l'=n} (-1)^{l+|e_j|} \varepsilon\Big( \Psi \At(X)^l( e_q ) \wedge \Psi \Atlarrow(X^{\vee})^{l'}( e_j^* ) \otimes e_j \Big) \nonumber\\
&= \sum_{j} \sum_{l+l' = n} (-1)^{n+|e_j|} \varepsilon\Psi\Big( \At(X)^l( e_q ) \times \Atlarrow(X^{\vee})^{l'}( e_j^* ) \otimes e_j \Big) \label{eq:zorro1a12}
\end{align} 
In the last step we use that $\Psi$ intertwines the shuffle product on $\Bar$ with the exterior product on~$\Delta$. Next we apply Lemma \ref{lemma:atshufat} to write this shuffle product of Atiyah classes as a single Atiyah class. As it stands in \eqref{eq:zorro1a12} the term inside the $\varepsilon \Psi ( - )$ belongs to $X \otimes \gamma_*(\Bar) \otimes X^\vee \otimes X$. Moving the $\Bar$ to the right of the $X^\vee$ introduces a sign $(-1)^{n( |e_j| + l' )}$. 

We account for this sign by using $\Atlarrow(X^{\vee}[n])^{l'} = (-1)^{nl'} \Atlarrow(X^{\vee})^{l'}$ to rewrite \eqref{eq:zorro1a12} as
\begin{align*}
\sum_{j} \sum_{l+l' = n} (-1)^{n+|e_j|+n(|e_j| + l') + n|e_j|} \varepsilon\Psi\Big( \At(X)^l( e_q ) \times \Atlarrow(X^{\vee}[n])^{l'}( e_j^* ) \otimes e_j \Big)\,.
\end{align*}
Then Lemma \ref{lemma:atshufat} applies directly to show that
\[
(1 \otimes \widetilde{\coev}) \circ \rho^{-1}( e_q ) = \sum_j (-1)^{n+|e_j| + n|e_j|} \varepsilon \Psi\Big( \At(X \otimes X^{\vee}[n])^n(e_q \otimes e_j^*) \otimes e_j \Big)\,.
\]
Applying the first isomorphism of \eqref{eq:firstisoevalsec} yields
\[
\sum_j (-1)^{n+(n+1)|e_j| + n|e_q|} \varepsilon \Psi \Big( \At((X \otimes X^\vee)[n])^n( e_q \otimes e_j^* ) \otimes e_j \Big)\,.
\]
Since the Atiyah class of $Y[n]$ differs from that of $Y$ by a sign $(-1)^n$, this is the same as
\[
\sum_j (-1)^{(n+1)|e_j|+n|e_q|} \varepsilon \Psi \Big( \At(X \otimes X^\vee)^n( e_q \otimes e_j^* ) \otimes e_j \Big)\,.
\]
The next step of \eqref{eq:firstisoevalsec} is to tensor $- \otimes_{S^e} S$, that is, to identify the left and right actions of $S$, after which $X \otimes X^\vee$ becomes identified $\End(X)$, so we are left with
\[
\sum_j (-1)^{(n+1)|e_j|+n|e_q|} \varepsilon \Psi \Big( \At( \End(X) )^n( e_q \circ e_j^* ) \otimes e_j \Big)\,.
\]
To finish applying $\eval_0$ it remains to compose with $(-1)^n \Lambda$, supertrace and integrate:
\begin{align*}
...
\end{align*}
This clearly vanishes unless $|e_q| = |e_j|$, so the sign in the last formula is in fact $(-1)^{n+(n+1)|e_j| + n|e_q|} = (-1)^{n+|e_j|}$. Since the Zorro map~$\mathcal Z$ is $S \otimes_k R$-linear it is fixed by its action on basis elements~$e_q$, which proves that $\mathcal{Z}$ is given by (\ref{Zorrointermediate}).
\end{proof}

The basic adjointness that we need is contained in the next lemma.

\begin{lemma}\label{lemma:lameadjoints} Given homogeneous $\psi \in \End(X)$ and $\varphi \in \End(X) \otimes_{k[x]} \Bar$
\begin{align*}
\big\langle\!\big\langle \str( \psi \circ \At(\varphi) ) \big\rangle\!\big\rangle = -(-1)^{|\psi|} &\big\langle\!\big\langle d \str( D( \psi ) \circ \varphi ) \big\rangle\!\big\rangle + (-1)^{|\psi|} \dlangle \str\big( \big[ \psi^\bullet, d \big] ( D\varphi ) \big) \drangle\\
&- \dlangle \str\big( D(\psi) \circ d\varphi ) \big) \drangle\,.
\end{align*}
\end{lemma}
\begin{proof}
We write $\psi^\bullet$ for the operator $\psi^\bullet(\alpha \otimes \omega) = \psi \circ \alpha \otimes \omega$ on $\End(X) \otimes_{k[x]} \Bar$. The super Jacobi identity for operators on this tensor product gives
...
\[
\big\langle\!\big\langle \str( \lambda_1 \ldots \lambda_n \circ \At(\varphi) ) \big\rangle\!\big\rangle = \sum_{i=1}^n (-1)^{i-1} \Big\langle\!\Big\langle d \left\{ f_i \str( \lambda_1 \ldots \widehat{\lambda_i} \ldots \lambda_n \circ \varphi ) \right\} \Big\rangle\!\Big\rangle + h_0( D(\varphi))
\]
...
We will need some notation: given a sequence $\bs{i} = (i_1,\ldots,i_p)$ in $\{1, \ldots, n\}$ (not necessarily in ascending order) let
\[
...
\begin{align*}
\sum_{\ell(\bs{i}) = p} (-1)^{\gamma(\bs{i}) + p} & \Big\langle\!\Big\langle d \circ f_{i_1} \circ \ldots \circ d \circ f_{i_p} \str\big( \Lambda_{i_1,\ldots,i_p} \circ \At(\varphi) \big) \Big\rangle\!\Big\rangle\\
&= \sum_{\ell(\bs{i}) = p+1} (-1)^{\gamma(\bs{i}) + p+1} \Big\langle\!\Big\langle d \circ f_{i_1} \circ \ldots \circ d \circ f_{i_{p+1}} \str\big( \Lambda_{i_1,\ldots,i_p,i_{p+1}} \circ \varphi \big) \Big\rangle\!\Big\rangle + h_p( D( \varphi ) )
\end{align*}
...
\[
h_p( \psi ) = \sum_{\ell(\bs{i}) = p} (-1)^{\gamma(\bs{i}) +p} \Big\langle\!\Big\langle d \circ f_{i_1} \circ \ldots \circ d \circ f_{i_p} \str\big( [ \Lambda_{i_1,\ldots,i_p}, d](\psi) \big) \Big\rangle\!\Big\rangle\,.
\]
\end{lemma}
\begin{proof}
Fix a sequence $\bs{i}$ of length $p$ and let $\psi = \Lambda_{i_1,\ldots,i_p}$ and let $\eta$ be the operator $\eta = d \circ f_{i_1} \circ \ldots \circ d \circ f_{i_p}$ on $\Bar$, where the $f_i$ stand for multiplication (on the left) by $f_i$. Then by Lemma \ref{lemma:lameadjoints}, or rather the proof with $\dlangle - \drangle$ replaced by $\dlangle \eta( - ) \drangle$ we have
\begin{align}
\dlangle \eta \str\big( \Lambda_{i_1,\ldots,i_p} \circ \At(\varphi) \big) \drangle &= \dlangle \eta \str\big( \big[ \Lambda_{i_1,\ldots,i_p}, d \big] ( D\varphi ) \big) \drangle \label{eq:hutch1}\\
&\qquad- \dlangle \eta \circ d \circ \str\big( \big[ D, \Lambda_{i_1,\ldots,i_p} \big] \varphi \big) \drangle \label{eq:hutch2}\\
&\qquad+ (-1)^{n-p+1} \dlangle \eta \str\big( \big[ D, \Lambda_{i_1,\ldots,i_p} \big]( d \varphi ) \big) \drangle \, . \label{eq:hutch3}
\end{align}
...
\[
\sum_{a \notin \{ i_1,\ldots, i_p \}} (-1)^{a-1 + \#\{ b \l i_b < a \}} \bigdlangle d \circ f_{i_1} \circ \ldots \circ d \circ f_{i_p} f_a \str\big( \big[ D, \Lambda_{i_1,\ldots,i_p,a} \big]( d \varphi ) \big) \bigdrangle \, .
\]
...
\[
\big\langle\!\big\langle \str \big( \Lambda \At^n (\varphi)\big) \big\rangle\!\big\rangle = \str(\varphi) + \sum_{p=0}^{n-1} h_i( D \At^{n-p-1}(\varphi) )\,.
\]
\end{lemma}
\begin{proof}
Applying Lemma \ref{lemma:zorromain} repeatedly yields
\begin{align*}
\big\langle\!\big\langle \str \big( \Lambda \At^n (\varphi)\big) \big\rangle\!\big\rangle = (-1)^n \sum_{\sigma \in S_n} (-1)^{|\sigma|} \big\langle\!\big\langle d \circ f_{\sigma(1)} \circ \ldots \circ d \circ f_{\sigma(n)} \str(\varphi) \big\rangle\!\big\rangle + \sum_{p=0}^{n-1} h_i( D \At^{n-p-1}(\varphi) )\,.
\end{align*}
%where 
%$$
%\delta = 
...
\[
\str( \varphi ) + \sum_{p=0}^{n-1} h_i( D \At^{n-p-1}(\varphi) ) \, .
\]
\end{proof}
...
Let $X\in \hmf(k[x_1,\ldots,x_n,z_1,\ldots,z_m], V-W)$ as before. In this section when we write $\Hom$ and $\End$ we mean the spaces of 2-(endo)morphisms in $\LG$. We define maps 
$$
...
\mathcal D_r(X)(\psi) & = \widetilde\eval_X[m+n] \circ (1_{X}\otimes (\lambda_{{}^\dual X} \circ (\psi\otimes 1_{{}^\dual X})\circ \lambda_{{}^\dual X}^{-1})) \circ \coev_X \, . \label{defectactionwrittenout}
\end{align}
...
%\end{tikzpicture}
\ee
in the nontrivial case, where we indicated our choice of variable names in the four domains, and both loops are labelled by~$X$. 
...
\be\label{bulktopmet}
\langle \phi, \psi \rangle_W = \Res_{k[x]/k} \left[ \frac{\phi \psi \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W\ldots\partial_{x_n} W}\right]
\ee
...
\begin{align*}
\mathcal D_l(X)(\phi) & = (-1)^{{m\choose 2} + {n+1\choose 2}}\Res_{k[x,z]/k[x]} \left[ \frac{\phi(z) \str\big( d_{X} \partial_{x_1} d_{X}\ldots \partial_{x_n} d_{X} \partial_{z_1} d_{X}\ldots \partial_{z_m} \big) \underline{\operatorname{d}\! z}}{\partial_{z_1} V \ldots \partial_{z_m} V} \right] \, , \\
\mathcal D_r(X)(\psi) & = (-1)^{{m+1\choose 2} + 0 (TODO)}  \Res_{k[x,z]/k[z]} \left[ \frac{\psi(x) \str\big( \partial_{x_1} d_{X}\ldots \partial_{x_n} d_{X} \partial_{z_1} d_{X}\ldots \partial_{z_m} d_{X} \big) \underline{\operatorname{d}\! x}}{\partial_{x_1} W \ldots \partial_{x_n} W} \right] \, . 
\end{align*}
\end{proposition}

\begin{proof}
We treat the case of $\mathcal D_r(X)$ in detail, the argument for $\mathcal D_l(X)$ works similarly. Since $\End(\Delta_W) = k[x]/(\partial_{x_i}W)$ and $\End(\Delta_V) = k[z]/(\partial_{z_i}V)$ we are free to set $x=x'$ and $z=z'$ at appropriate places, cf.~\eqref{defectaction}. Furthermore, $\lambda_{{}^\dual X}$ will project out all non-zero degree contributions coming from the action of $\lambda^{-1}_{{}^\dual X}$, so $\lambda_{{}^\dual X} \circ (\psi\otimes 1_{{}^\dual X})\circ \lambda_{{}^\dual X}^{-1}$ is simply multiplication by the polynomial $\psi(z)$. 

In the lower part of the expression for $\mathcal D_r(X)(\psi)$ in~\eqref{defectaction} we have 
$$
\coev_X(1) = \sum_j (-1)^{{m+1\choose 2} + m|e_j|} \partial_{[1]} d_X \ldots \partial_{[m]} d_X e_j \otimes e_j^* \, . 
$$
%\begin{align}
...
Next we apply~$\psi$ and $\widetilde\eval_X$. Since the latter maps back to~$\Delta_V$ we may set $\partial_{[i]} d_X = \partial_{z_i} d_X$ in the above as $z=z'$ in $\End(\Delta_V)$. Thus we obtain
$$
\mathcal D_r(X)(\psi) = (-1)^{{m+1\choose 2} + 0 (TODO)} \Res_{k[x,z]/k[z]} \left[ \frac{\psi(x) \str\big( d_{X} \partial_{x_1} d_{X}\ldots \partial_{x_n} d_{X} \partial_{z_1} d_{X}\ldots \partial_{z_m} \big) \underline{\operatorname{d}\! x}}{\partial_{x_1} W \ldots \partial_{x_n} W} \right] + \mathcal O(\theta) \, . 
$$
Here we collectively denote the contributions from $\widetilde\eval_X$ of non-zero degree in the Koszul complex $\Delta_V$ by $\mathcal O(\theta)$. Since we know that $\mathcal D_r(X)(\psi)$ is a morphism in $\End(\Delta_V) = k[z]/(\partial_{z_i}V)$ it follows that $\mathcal O(\theta)$ must be null-homotopic, thus concluding the proof. 
\end{proof}
...
\be\label{Dadjoint}
\big\langle \mathcal D_l(X)(\phi), \psi \big\rangle_W = (-1)^{{n+1\choose 2} TODO} \big\langle \phi , \mathcal D_r(X)(\psi) \big\rangle_V
\ee
for all $\phi\in \End(\Delta_V)$ and $\psi\in \End(\Delta_W)$. [TODO: why does $\mathcal D_r$ not have a nicer sign?]
\end{corollary}
...
\beta^Q : \psi \lmt (-1)^{n+1\choose 2} \str(\psi \, \partial_{x_1} d_Q \ldots \partial_{x_n} d_Q) \, . 
\ee
The hardest part in establishing this result is to prove the nondegeneracy of the Kapustin-Li pairing and that the Cardy condition holds; this was first done in~\cite{m0912.1629} and~\cite{pv1002.2116}, respectively, in the case where~$k$ is a field. In Sections~\ref{section:dualityadjointop} and~\ref{sec:CardyCondition} we will give new proofs for the general case. 
...
\psi & \lmt (-1)^{m+1\choose 2} \Res_{k[x,z]/k[x]} \left[ \frac{\str\left( \psi\,  \partial_{x_1} d_X \ldots \partial_{x_n} d_X \partial_{z_1} d_X \ldots \partial_{z_m} d_X \right) \underline{\operatorname{d}\!z}}{\partial_{z_1} W \ldots \partial_{z_m} W} \right] \, . 
\end{align*}


\section{Cardy condition}\label{sec:CardyCondition}

As another application of our explicit expressions for adjunctions in $\LG$ in this section we present a new proof of the Cardy condition. This deep result long eluded rigorous proofs, which were only recently given in~\cite{pv1002.2116, dm1102.2957} using rather heavy machinery. With our expressions for evaluations and coevaluations in $\LG$ it will follow rather effortlessly for arbitrary commutative noetherian $\nQ$-algebras~$k$, such as $k=\nC[t_1,\ldots,t_d]$, simply from considerations of one single diagram. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ee
which is a map $k\lra k$ that we call~$C$. 

\begin{remark}\label{rem:correlatorsare2morphisms}
Note that again we colour-code the domain representing the non-trivial object $(k[x],W)$ in $\LG$. This is not only a visual aid for interpreting the components of the map~$C$, but~\eqref{annulus} can also be interpreted as an annulus correlator with ``boundary conditions'' $X,Y$ and ``field insertions'' $\varphi, \psi$. 

More generally, we claim that for arbitrary Landau-Ginzburg models the correlator for \textsl{any} worldsheet with defects and insertions can be computed as a 2-morphism in $\LG$: all one has to do is to interpret the worldsheet as the associated string diagram. In this way the bicategory $\LG$ provides a complete and unified framework for all Landau-Ginzburg models. 

When reading off the relevant 2-morphism from the physical picture of a worldsheet with insertions, there is one detail that we should address: in order for the picture to make sense as a string diagram, one may have to suspend the adjunction maps (an example of which we already saw in the definition of the defect action maps~\eqref{defectactionwrittenout}) and include isomorphisms of the type $X\otimes Y[1] \lra X[1] \otimes Y$. For example, what we really mean by the expression in~\eqref{annulus} is the map
$$
%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=0.8,color=blue!50!black, baseline,>=stealth]
\nicecolourscheme (0,0) circle (2);
...
%
\draw (0,0) circle (1);
\draw[->, very thick] (0.100,1) -- (0.101,1) node[above] {{\small$\widetilde\eval_X[n] \tau$}}; 
\draw[->, very thick] (-0.100,-1) -- (-0.101,-1) node[below] {{\small$\coev_X$}}; 
\fill (135:1) circle (2.5pt) node[left] {{\small$\varphi$}};
%
\fill (135:2) circle (2.5pt) node[left] {{\small$\rho_{{}^\dual Y} \tau'$}};
\draw[densely dotted] (90:1) .. controls +(0,0.2) and +(0.5,-0.5) .. (135:2);
\end{tikzpicture} 
%%%%%%%%%%%%%%%%%%%%%%%%%%
$$
where in the case of odd~$n$ we have to suspend $\widetilde\eval_X$ (with the dotted line in~\eqref{annulus} representing $\Delta_W[n]$) and use the isomorphisms $\tau: X\otimes X^\vee[n] \lra X[n] \otimes X^\vee$ and $\tau': Y^\vee \otimes \Delta_W[n] \lra Y^\vee[n] \otimes \Delta_W$; for~$n$ even this additional notational clutter disappears. 
\end{remark}

To put ourselves in a position to better understand and manipulate~\eqref{annulus} let us spell out its constituents. From the general expressions in Section~\ref{sec:derivcoeval} for evaluation and coevaluation maps we find \begin{align}
...
& \coev_X (1) = \sum_j (-1)^{{n+1 \choose 2} + n |e_j|} \Lambda_X e_j \otimes e_j^* \, , \qquad 
\widetilde\coev_Y (1) = \sum_j (-1)^{|f_j|} f_j^* \otimes f_j \label{evcoeveasy}
\end{align}
where $\Lambda_X = \partial_1 d_X\ldots \partial_n d_X$ and $\Lambda_Y = \partial_1 d_Y\ldots \partial_n d_Y$, and $\{ e_i \}_i$ and $\{ f_j \}_j$ are homogeneous bases of~$X$ and~$Y$, respectively. 

The diagram~\eqref{annulus} can be computed in two different ways; equating the respective results amounts to the Cardy condition. Below we will present the details for the case of even~$n$. For odd~$n$ the proof works analogously, but one has to use the variant
$$
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=0.8,color=blue!50!black, baseline]
\draw[line width=1pt] 
(-2,-1) node[line width=0pt] (X) {{\small $Y^\vee$}}
(-1,-1) node[line width=0pt] (Y) {{\small $X \vphantom{Y^\vee}$}}
(2,2) node[line width=0pt] (XY) {{\small $(X^\vee[1]\otimes Y)^\vee$}}; 
\draw[redirected] (0,0) .. controls +(0,1) and +(0,1) .. (-1,0);
\fill (-0.5,0.65) circle (0pt) node[above] {{\small$\widetilde\eval_X[1]\tau$}};
\draw[directed] (1,0) .. controls +(0,2) and +(0,2) .. (-2,0);
\draw[densely dotted] (-0.5,0.75) .. controls +(0,0.2) and +(0.5,-0.5) .. (-1.56,1.1);
\fill (-1.56,1.1) circle (2.5pt) node[left] {{\small$\rho_{Y^\vee[1]}\tau'$}};
\draw[directed] (2,0) .. controls +(0,-1) and +(0,-1) .. (0.5,0);
\draw (-1,0) -- (Y);
\draw (-2,0) -- (X);
\draw[dotted] (0,0) -- (1,0);
...
\begin{tikzpicture}[very thick,scale=0.8,color=blue!50!black, baseline]
\draw[line width=1pt] 
(2,-1) node[line width=0pt] (Y) {{\small $Y^\vee$}}
(3,-1) node[line width=0pt] (X) {{\small $X \vphantom{Y^\vee}$}}
(-1,2) node[line width=0pt] (XY) {{\small $(X^\vee[1]\otimes Y)^\vee$}}; 
\draw[directed] (1,0) .. controls +(0,1) and +(0,1) .. (2,0);
...
%%%%%%%%%%%%%%%%%%%%%%
$$
 of~\eqref{eq:monoidalproperty} which can be etablished by an argument similar to the one leading to Proposition~\ref{prop:monoidalproperty}. 

One possibility to compute~\eqref{annulus} is to first collapse the $X$-loop to obtain a ``$(k[x],W)$-bubble'', and then collapse its boundary loop~$Y$ in turn to arrive at an expression for $C:k \lra k$. It follows from the general results of Section~\ref{sec:defectaction} or directly from~\eqref{evcoeveasy} that the first step of collapsing~$X$ produces the 2-morphism $(-1)^{n+1\choose 2} \str(\varphi \Lambda_X)\in\End(\Delta_W)$. Similarly, the second step of collapsing~$Y$ produces a residue: 
$$
C 
= 
%%%%%%%%%%%%%%%%%%%%%%
...
$$
Using the expressions $\beta^X(\varphi) = (-1)^{n+1\choose 2} \str(\varphi \Lambda_X)$ and $\beta^Y(\psi) = (-1)^{n+1\choose 2} \str(\psi \Lambda_Y)$ for the boundary-bulk maps from Section~\ref{sec:shadows} we thus find from the above that
\be\label{C1}
C = 
\Res_{k[x]/k} \left[ \frac{\beta^X(\varphi) \, \beta^Y (\psi) \, \underline{\operatorname{d}\! x}}{\partial_1 W \ldots \partial_n W} \right] .
\ee

On the other hand, we may compute~$C$ by first fusing the two loops in~\eqref{annulus} together to obtain a single loop labelled by ${}^\dual X\otimes Y = \Hom(X,Y) \in \End_{\LG}( (k,0) )$. Then we use
$$
\eval_{{}^\dual X \otimes Y} = \str(-) \, , \qquad \widetilde\coev_{{}^\dual X \otimes Y} : 1 \lmt 1_{{}^\dual X \otimes Y}
$$
to arrive at
\be\label{C2}
C 
...
\begin{remark}
The Cardy condition also holds if one or both of the maps $\varphi, \psi$ are odd (which we present as maps such as $X\lra X[1]$ in $\LG$). The proof of this fact proceeds like the above, with small changes coming from the suspensions as indicated in Remark~\ref{rem:correlatorsare2morphisms}. 
\end{remark}
...
\end{align*}
is a homotopy equivalence over $k$. The main theorem here is that under some reasonable hypotheses the pairing $\langle -, - \rangle$ is homotopically nondegenerate. This nondegeneracy is used in Section~\ref{sec:Zorro} to prove that specified evaluation and coevaluation morphisms give rise to adjoint $1$-morphisms in the bicategory of Landau-Ginzburg models over $k$.
...
is an isomorphism.
\end{itemize}
Then the pairing $\langle -, - \rangle$ is homotopically nondegenerate.
\end{theorem}
...
\[
\vartheta = \frac{1}{n!} \sum_{\sigma \in S_n} (-1)^{|\sigma|} \lambda_{\sigma(1)}^\bullet \ldots \lambda_{\sigma(n)}^{\bullet}\,.
\]
...
&= (\vartheta')^* \zeta\left( \frac{1}{n!} \sum_{\sigma \in S_n} (-1)^{|\sigma|} \lambda_{\sigma(1)} \circ \ldots \circ \lambda_{\sigma(n)} \circ \alpha \right)\\
...
%J.~Rickard, \textsl{Translation functors and equivalences of derived categories for blocks of algebraic groups}, in “Finite dimensional algebras and related topics”, Kluwer (1994), 255-–264.
%
