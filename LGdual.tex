...
\]
as in~\eqref{lambdarho}. This homotopy equivalence is part of the data of the bicategory $\LG$. In order to derive the evaluation and coevaluation maps and prove the Zorro moves we will need explicit homotopy inverses for $\rho$, and similar maps. These we construct using commutative diagrams of the form
\begin{equation}\label{eq:pertdia1}
\xymatrix{
X \,\widehat{\otimes}\, \Bar \ar[dr]_-{\rho'}\ar[rr]^{1 \otimes \Psi} & & X \otimes \Delta \ar[dl]^-{\rho}\\
& X
}
\end{equation}
where $\Psi$ is the canonical map given in (\ref{intro_psi}) and $\rho'$ is explained below. Rather than inverting $\rho$ directly, it turns out to be advantageous to invert $\rho'$ and then compose with $\Psi$. Roughly speaking the inverse of $\rho'$ is the geometric series in powers of the associative Atiyah class of $X$ and we make use of a completion of $X \otimes_R \Bar$ in order to guarantee that this geometric series converges.
...
\]
together with a homogeneous basis $\{ e_i \}_{i \in I}$ of $X$ over $S \otimes_k R$. Let $D$ denote the differential on $X$. The two cases of interest are when $R = \Ae, S = k[z]$ and $\theta$ is the identity, or when $R = A$ and $\theta(a) = a \otimes 1$ which the situation of~\eqref{eq:pertdia1}. We return to these examples at the end of the section.
...
Now we turn to the Koszul stabilisation of the diagonal. Assume that $A = k[x_1,\ldots,x_n]$ with $k$ noetherian and let $(\Delta, d_{\Delta})$ be the finite rank matrix factorisation of $\widetilde{W}$ given in Section~\ref{subsec:bicatLG} where $d_{\Delta} = \delta_{+} + \delta_{-}$. There is a morphism of linear factorisations
...
\[
\xymatrix{
X \,\widehat{\otimes}_{k[x]}\, \Bar \ar[dr]_-{\rho'}\ar[rr]^{1 \otimes \Psi} & & X \otimes_{k[x]} \Delta \ar[dl]^-{\rho}\\
& X
}
\]
