...
In this section we introduce a slightly different graphical presentation presentation of adjoints (which we basically borrow from~\cite{ct1007.2679}). Before doing so we wish however to give reasons for imposing new conventions on the reader, halfway through the paper. While the presentation of adjoints introduced in Section~\ref{subsec:bicat} and employed until now is standard and conceptually clean, for technical computations like in Section~\ref{sec:pivotality} or in our proof of the Cardy condition the slightly modified notation presented below turns out to be more useful. Furthermore, we will see in Section~\ref{sec:ocTFT} that it allows us to unambigiously translate TFT correlators into computable 2-morphisms. And finally, in the new notation we will no longer have to deal with two kinds of daggered adjoints~${}^\dagger X$ and~$X^\dagger$, but only with the dual matrix factorisation~$X^\vee$ (however at the expense of introducing shifted rings into string diagrams). 


Recall that for a 1-morphism $X: (R,W) \lra (S,V)$ in $\LG$ with $R=k[x_1,\ldots,x_n]$ and $S=k[z_1,\ldots,z_m]$ we have ${}^\dual X = X^\vee \otimes_S S[m]$ and $X^\dual = R[n] \otimes_R X^\vee$. From now on we will reflect the presence of shifted rings in the adjoints by explicitly representing them by special lines in our graphical notation. To wit, while we continue to associate upwards oriented solid lines to 1-morphisms~$X$, we will use thin wiggly lines to represent rings shifted by the number of variables on which they depend. Accordingly we will use the following presentation for the adjunction maps: 
\begin{align*}
&
\coev_X = 
%%%%%%%%%%%%%%%%%%%%%%
...
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
: r \lra R[n] \otimes_R R[n]  \, , \qquad 1 \lmt 1 \otimes 1 \, . 
$$
These maps satisfy the useful identity
$$
TODO
$$
...
\end{align*}
We also want to allow wiggly lines to cross solid lines when the two domains are associated to rings whose variable parity differs. To facilitate this we adopt the additional convention that we label a wiggly line by the number of times the associated ring get shifter whenever necessary; for instance for a 1-morphism $X\in \hmf(R\otimes_k S, V-W)$ as before we have a map
$$
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=0cm]
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-0.3,-0.75) .. controls +(0,0.5) and +(0,-0.5) ..  (0.3,0.75);
...
%
\draw[line width=1pt] 
(0.45,0.6) node[color=blue!55!white, line width=0pt] {{\tiny $m$}}; 
%
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
: S[m] \otimes_S X \lra X \otimes_R R[m] \, , \qquad 1\otimes \eta \lmt (-1)^{m |\eta|} \eta \otimes 1 \, .
$$

Labelled wiggly lines can cross each other, giving rise to maps of the type (TODO: cross!)
$$
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=0.3cm]
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}]  (0,0) .. controls +(0.35,1.0) and +(-0.35,-1.0) .. (0.75,1); 
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}]  (0,1) .. controls +(0.5,-1.0) and +(-0.35,1.0) .. (0.75,0); 
%
\draw[line width=1pt] 
(-0.13,0.1) node[color=blue!55!white, line width=0pt] {{\tiny $m$}}
(0.85,0.1) node[color=blue!55!white, line width=0pt] {{\tiny $n$}}
(-0.1,0.9) node[color=blue!55!white, line width=0pt] {{\tiny $n$}}
(0.87,0.9) node[color=blue!55!white, line width=0pt] {{\tiny $m$}};
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
: R[m]\otimes_R R[n] \lra R[n] \otimes_R R[m] \, , \qquad
r_1 \otimes r_2 \lmt (-1)^{mn} r_2 \otimes r_1 \, .
$$
For $m=n$~mod~2 such a crossing can be ``resolved'', i.\,e.~we have the following identities between maps: 
$$
TODO
cross_n = (-1)^n wiggly wiggly
$$
