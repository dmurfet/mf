...
\section{Perturbation and homotopy inverses}
...
In outline, let $\Bar$ be the linear factorisation of $\widetilde{W}$ given in Lemma \ref{PsiHF} and $\Delta$ the Koszul factorisation of $\widetilde{W}$. For any finite rank matrix factorisation $X \in \hmf(k[z,x], V - W)$ there is a $k[z]$-$k[x]$-bilinear homotopy equivalence
\[
...
\end{lemma}
\begin{proof}
The required identity $b' d + d b' = 1 - \sigma \pi$ follows immedlatey from (\ref{b'd+db'}).
\end{proof}
...
\begin{proposition}\label{prop:finalpertdefo} There is a deformation retract datum of linear factorisations of $V + \widetilde{W}$ over $S \otimes_k A^{op}$
\[
\xymatrix@C+2pc{
...
\begin{lemma} The map $\pi: X \otimes_R \Delta \lto X \otimes_R A$ is an $S \otimes_k A^{op}$-linear homotopy equivalence of linear factorisations of $V + \widetilde{W}$ over $S \otimes_k A^{op}$.
\end{lemma}
\begin{proof}
There is an exact sequence of free $k[y]$-modules
\[
0 \lto \Delta_n \lto \cdots \lto \Delta_0 \lto A \lto 0
\]
which splits, and provides the necessary $k[y]$-linear $\sigma: A \lto \Delta$ and homotopy $h$ on $\Delta$ to make
\[
\xymatrix@C+2pc{
...
\begin{corollary} An $S \otimes_k A^{op}$-linear homotopy inverse to $\pi: X \otimes_R \Delta \lto X \otimes_R A$ is given by
\[
...
\end{tikzpicture}
$$
That is, (Job for \textbf{Nils the Tikz fairy}: variable labels)
\[
\widetilde\coev_X: \Delta \lto X^\dual \otimes X\,.
\]
As recalled in Section \ref{??} there are two coevaluation $2$-morphisms associated with $X$. The details in each case are the same, so we give the full details for $\widetilde\coev_X$ and leave $\coev_X$ to the reader. Here we need to discuss $\Delta = \Delta^y_x$ and $\Delta^x_y$ as defined in (\emph{coevc}). We fix a homogeneous basis $\{ e_i \}_{i \in I}$ for $X$ with dual basis $e_i^*$.

A natural candidate for $\widetilde\coev_X$ is the inverse image of the identity $1_X$ under the homotopy equivalence of the next proposition. For the moment we concentrate on explaining the detailed formula for this candidate; then, in Section \ref{??}, we show that this candidate is actually the unit of an adjunction by proving the Zorro moves.

\begin{proposition}\label{prop:isogivescoev} There is a canonical homotopy equivalence of $\mathbb{Z}_2$-graded $k$-complexes
\begin{equation}
\Hom_{k[x,y]}( \Delta, X^\dual \otimes X ) \lto \Hom_{k[z,x]}(X,X)\,. \label{eq:isogivescoev}
\end{equation}
\end{proposition}
\begin{proof}
Recall that the graded free $k[x,y]$-module underlying $\Delta$ is the exterior algebra on symbols $\theta_1, \ldots, \theta_n$. Let $K$ denote the usual $\mathbb{Z}_2$-graded Koszul complex with the same underlying graded module, but only the differential $\delta_{-}$ of (\ref{??}). Then the product in the exterior algebra gives rise to a morphism of $\mathbb{Z}_2$-graded complexes
\[
\Delta^x_y \otimes_{k[x,y]} \Delta^y_x \lto K\,.
\]
Composing with the natural morphism $\varepsilon: K \lto k[x,y][n]$ and taking the adjoint we have a canonical isomorphism of matrix factorisations
\[
\Delta^x_y \lto (\Delta^y_x)^\vee [n]\, \qquad \omega \mapsto \varepsilon( \omega \wedge - )\,.
\]
Combining this with various canonical maps and the projection $\pi: \Delta^x_y \lto k[x]$ we have the following diagram
\begin{equation}
\xymatrix{
\Hom_{k[x,y]}( \Delta^y_x, X^\vee[n] \otimes_{k[z]} X)\\
(\Delta^y_x)^\vee \otimes_{k[x,y]} \left( X^\vee[n] \otimes_{k[z]} X \right) \ar[u]_-{\cong}\\
\left( X^\vee[n] \otimes_{k[z]} X \right) \otimes_{k[x,y]} (\Delta^y_x)^\vee \ar[u]_-{\cong}\\
\left( X^\vee \otimes_{k[z]} X \right) \otimes_{k[x,y]} (\Delta^y_x)^\vee[n] \ar[u]_-{\cong}\\
\left( X^\vee \otimes_{k[z]} X \right) \otimes_{k[x,y]} \Delta^x_y \ar[u]_-{\cong} \ar[d]^-{1 \otimes \pi}\\
\left( X^\vee \otimes_{k[z]} X \right) \otimes_{k[x,y]} k[x] \ar[d]^-{\cong}\\
\Hom_{k[z,x]}(X,X)\,.
}\label{eq:isogivescoev2}
\end{equation}
\end{proof}

\begin{definition} The \emph{coevaluation} is the morphism $\widetilde\coev_X: \Delta \lto X^\dual \otimes X$ in the homotopy category $\hmf(k[x,y], W(x) - W(y))$ mapping to $1_X$ under (\ref{eq:isogivescoev})\,.
\end{definition}

Clearly to give a representative chain map for the coevaluation we need to begin with $1_X$ at the bottom of (\ref{eq:isogivescoev2}) and apply the various maps in turn; the only nontrivial step will be to find a homotopy inverse for $1 \otimes \pi$. In fact we will need to present two different homotopy inverses, which lead to two different (but homotopic) formulas for the coevaluation.

On the tensor product
\[
\Bar \otimes_{k[x,y]} ( X^\dual \otimes X)\,
\]
we consider the operator $\At_{X^\dual \otimes X} = [d, d_{X^\dual \otimes X}]$ and its powers. Since $d$ is $k[y]$-linear, this is actually $[d, d_{X^\dual}]$ and for this reason we abuse notation and write $\At_{X^\dual}$ for $\At_{X^\dual \otimes X}$. If we apply $\Psi: \Bar \lto K$ after a power of the Atiyah class, we end up with maps
\[
\xymatrix{
X^\dual \otimes X \ar[r]^-{\At^i_{X^\dual}} & \Bar \otimes_{k[x,y]} ( X^\dual \otimes X ) \ar[r]^-{\Psi} & K \otimes_{k[x,y]} (X^\dual \otimes X)
}
\]
which we can apply to the tensor $\iota_X = \sum_{i} (-1)^|e_i| e_i^* \otimes e_i$.
...
\end{theorem}
\begin{proof}
As long as the Bar perturbation stuff is done previously, this is straightforward.
\end{proof}
