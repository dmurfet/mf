...
\ee
represents a $2$-morphism $\gamma: f \otimes h \lto g$ in $\cat{B}$ with $f: A \lto B, g: C \lto B$ and $h: C \lto A$ all $1$-morphisms. We call $\gamma$ the \textsl{value} of the diagram \eqref{eq:demonstratediagram}.
...
%%%%%%%%%%%%%%%%%%%%%%
$$
By definition a wiggly line in a domain associated to an object $(R,W)\in\LG$ represents the ring~$R$ shifted by the parity of its variable number. Furthermore we adopt the convention that in diagrams with wiggly lines a downwards oriented solid line represents the dual factorisation~$X^\vee$ (and not necessarily its left or right adjoint~${}^\dual X$ or~$X^\dual$) if the line is labelled by the 1-morphism~$X$. 
...
\end{align*}
We also want to allow wiggly lines to cross solid lines when the two domains are associated to rings whose variable parity differs. To facilitate this we adopt the additional convention that we label a wiggly line by the number of times the associated ring get shifted whenever necessary; for instance for a 1-morphism $X\in \hmf(R\otimes_k S, V-W)$ as before we have a map
$$
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=0cm]
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-0.3,-0.75) .. controls +(0,0.5) and +(0,-0.5) ..  (0.3,0.75);
...
$$
Using these maps we can freely move wiggly lines around (when paying attention to signs). In particular there are variants of the adjunction maps in~\eqref{eq:wigglyadjunctionmaps} where the wiggly lines enter the vertex from different directions. For example we define
$$
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=-.4cm, rotate=180]
\draw[redirected] (3,0) .. controls +(0,1) and +(0,1) .. (2,0);
...
$$
Using $X^\dual[m+n] \cong R[m]\otimes_R R[n] \otimes_R X^\dual$ we write this as the isomorphism
\be\label{eq:isoq}
q_x 
\; = \; 
%%%%%%%%%%%%%%%%%%%%%%
...
q(\nu) = (-1)^{m|\nu|} \nu 
\ee
which has a simple diagrammatic presentation. 
...
\[
\mathscr{R}: (Y \otimes X)^{\dual} \stackrel{\cong}{\lra} X^{\dual} \otimes Y^{\dual}\,.
\]
...
One can ask whether or not these isomorphisms $\mathscr{R}, \mathscr{L}$ are compatible in a sutiable sense with the isomorphism $q$ which exchanges left and right adjoints, and this is precisely the content of the next result. For the statement, we let $W \in R = k[x_1,\ldots,x_n]$, $V \in S = k[z_1,\ldots,z_m]$, $U \in T = k[y_1,\ldots,y_p]$ be potentials and $X \in \hmf(k[x,z],V-W)$ and $Y\in \hmf(k[y,z],U-V)$.

(NOTE: better to keep both parts in proposition if we call it ``pivotality up to shifts''. Is it OK to have the diagram only in the proof and not in the statement?)

\begin{proposition}\label{prop:monoidalproperty} 
\begin{enumerate}
\item For any map~$\varphi:Z\lra X$ in $\hmf(k[x,z], V-W)$ we have 
$$
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=0cm]
\draw[line width=0] 
...
\, .
\ee
We denote this morphism by $\mathscr{R}: (\bar{Y} \otimes \bar{X})^\dual \lto X^\dual \otimes Y^\dual$. (TODO: same $\mathscr{R}$ already used for the map whose source doesn't have bars. Use $\bar{\mathscr{R}}$ instead or speak of abuse of notation?)

Similarly, after adding $\kappa^{\vee}$ and migrating, the right-hand side of~\eqref{eq:monoidalproperty} becomes a morphism $\mathscr{L}: (\bar{Y} \otimes \bar{X})^\dual \lto X^\dual \otimes Y^\dual$. 
...
The basic argument is a standard calculation with string diagrams. There are however slight modifications needed due to the fact that $\LG$ is only ``pivotal up to shifts'' as discussed in Section~\ref{BLA}. 
...
\end{theorem}
\begin{proof}
For local rings $R$ this is the statement of local duality. A proof which works in the present level of generality is given in~\cite{TODO}.
\end{proof}
