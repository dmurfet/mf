...
C_{ij} \in k[x,y] \, ,
$$
together with the defining property $\Res_{k[x,y]/k[y]} \left[ \underline{\operatorname{d}\! x}/(x_1^{a_1} \ldots x_n^{a_n}) \right] = \delta_{a_1,1}\ldots \delta_{a_n,1}$. (\textbf{todo} denominator ordering).
...
In fact it will be convenient to prove the theorem more generally, since in order to equip~$R$ with a connection we may need to pass to the $(f_1,\ldots,f_n)$-adic completion. Consequently, for the rest of this section $R$ is an arbitrary noetherian $k$-algebra, $W \in R$, and our hypotheses are:
\begin{itemize}
...
is an isomorphism.
\end{itemize}
In this situation the pairing (\ref{eq:generalklpairing}) still makes sense and we prove that it is homotopically nondegenerate in Proposition \ref{prop:almostkltheorem}. The theorem follows by passing from $k[x_1,\ldots,x_n]$ to a completion, which satisfies (H3$'$) above even if the polynomial ring does not.
...
\begin{definition} Given a null-homotopy $\lambda_i$ for the action of $f_i$, that is, a map $\lambda_i \in \Hom(Y,Y)$ of degree one with $[D, \lambda_i] = f_i \cdot 1_Y$, we define the odd operator $\lambda_i^\bullet$ on $\Hom(\bar{X},\bar{Y})$ by
\[
