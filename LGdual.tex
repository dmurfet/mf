...
It is convenient to denote $2$-morphisms in a bicategory using \textsl{string diagram} notation. This was introduced in \cite{JSGoTCI,JSGoTCII} and the reader can also find very clear explanations in \cite{ladia,khovdia}. In order to fix our notation, recall that the diagram TODO 
...
\section{Diagrammatical calculus and pivotality}\label{sec:wiggliesandsigns}

With the proof that the bicategory $\LG$ has adjoints behind us we now want to reap some further rewards of our efforts by turning to applications. These will be discussed in the next sections, while in the present one we shall pave the way by clarifying two (TODO) technical though practically relevant issues: TODO the choice of representative of the isomorphism class of adjoints, and the nature of the double-adjoint. 
...
To explain what we mean, first we recall a formal result of category theory. If $X: W \lto V$ and $Y: V \lto U$ are $1$-morphisms in $\LG$ then the composites $X^{\dual} \otimes Y^{\dual}$ and ${}^\dual X \otimes {}^\dual Y$ are the right and left adjoint to $Y \otimes X$, respectively, and so by uniqueness of adjoints there is a canonical isomorphisms
\[
...
TODO: diagrams
\]
and we will explain their relation in Corollary \ref{cor:nicerpivotalitydiagrams}. TODO: 
