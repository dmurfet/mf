...
On the other hand our general construction also allows for concrete calculations. To give a flavour of how explicit our results can be made, let $\{ e_i \}_i$ be a basis for the free $k[x,z]$-module~$X$ with dual basis $\{ e_i^* \}_i$. Then e.\,g.~the coevaluation map $\widetilde\coev_X$ is given by
...
\begin{remark} When $k$ is a field the triangulated categories $\hmf(k[x], W)^\omega$ all have Serre functors, and so the \textsl{existence} of adjoints in $\LG$ can be derived from representability theorems, see for example \cite{ballard}. But these general methods are not well-suited for explicit computations, and in particular for deriving explicit evaluation and coevaluation maps.
\end{remark}
