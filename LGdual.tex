...
\At_{2}(X)^m( e_i \otimes \omega ) = \sum_{j_1,\ldots,j_m} (-1)^{m|e_i| + \binom{m+1}{2}} e_{j_m} \otimes d( D_{j_m, j_{m-1}} ) \cdots d(D_{j_2,j_1})d( D_{j_1 i} ) \omega\,.
\end{equation}
...
\end{tikzpicture}
\ee
Here we label the two domains to the left and right of the 1-morphism~$X$ by the variables pertaining to the objects $(k[x],W), (k[z],V) \in \LG$, using primes to distinguish between different copies of the same variable sets. We keep in mind that there are two distinct $\Delta$'s which appear: if we write $R = k[x]$ and $S = k[z]$ then one of the $\Delta$'s is an $R$-$R$-bimodule, and the other is an $S$-$S$-bimodule. It is convenient to identify $\Re = R \otimes_k R$ with $k[x,x']$ so that the right $R$-action is via $x'$-variables.
...
\ee
which we prove is homotopic to the identity on $X$. Note that we continue to discard subscripts that are clear from the context, e.\,g.~$\widetilde\eval = \widetilde\eval_X$. Our first order of business is to establish the expression for the Zorro map given in the next lemma. For this we need to introduce the $k[x',z]$-linear map
\begin{gather*}
\dlangle - \drangle: S \otimes_k \Bar \lto S \otimes_k R = k[x',z],\\
\dlangle \alpha \drangle = (-1)^{n+1\choose 2} \Res_{k[x,x',z]/k[x',z]} \left[ \frac{ \varepsilon\Psi( \alpha ) \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]\,.
\end{gather*}
We use the map $\varepsilon\Psi: \Bar \lto S \otimes_k R$ of Section~\ref{sec:Background} and $\underline{\operatorname{d}\!x}=\operatorname{d}\!x_1\ldots \operatorname{d}\!x_n$ as before. Here $\Bar$ is the bar complex of $R$ over $k$, and the two copies of $R$ in $R^e = k[x,x']$ play very different roles in $\langle\!\langle - \rangle\!\rangle$. The left copy (i.\,e.~$k[x]$) is integrated out, and the map is linear with respect to the right action.

Let $\{ e_i \}_{i \in I}$ be a homogeneous $k[x,z]$-basis of $X$, $\{ e_i^* \}_{i \in I}$ the dual basis and $\{ e_{ij} = e_i \circ e_j^* \}_{i \in I, j \in J}$ the determined basis of $\End(X)$. By the Atiyah class $\At = \At( \End(X) )$ of this complex we mean the operator on $\End(X) \otimes_R \Bar$ which might more properly be called the Atiyah class of $\End(X) \otimes_R R^e$ determined by the basis $e_{ij}$.
