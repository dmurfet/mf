...
\ee
as well as various properties such as $\mathcal D_r(X\otimes Y) = \mathcal D_r(X) \circ \mathcal D_r(Y)$. (TODO: sign?)
...
\ee
It is convenient to denote identities in bicategories with adjoints in \textsl{string diagram} notation. We will explain a refined version of this notation in Section~\ref{subsec:alternatives}, but for the moment string diagrams have the meaning given in \cite{JSGoTCI,JSGoTCII} and also explained very clearly in \cite{ladia,khovdia}. In this language the evaluation and coevaluation maps~\eqref{evcoev2} are written as
$$
\eval_{f} = 
%%%%%%%%%%%%%%%%%%%%%%
...
%\begin{remark} \textbf{todo} We can scale the evaluation and coevaluation by signs.
%\end{remark}
...
Since we work with polynomials rather than power series, $\hmf(R,W)$ is not necessarily idempotent complete; for a simple example of this phenomenon see \cite{??}. However $\HMF(R,W)$ has arbitrary coproducts and is therefore idempotent complete \cite{??}, and we denote by $\hmf(R,W)^\omega$ the idempotent closure of $\hmf(R,W)$ in this larger triangulated category. More concretely, this idempotent closure is the full subcategory of $\HMF(R,W)$ whose objects are those matrix factorisations $Y$ which are direct summands of finite-rank matrix factorisations (in the homotopy category). This is an idempotent complete triangulated category. 
...
Choosing an $\Re$-basis $\{ \theta_i \}_i$ of $(\Re)^{\oplus n}$, we have 
\be\label{DeltaW}
...
% see rfper3
\begin{proof}
As far as we know the first proof for $k$ a field is contained in \cite{pv1002.2116}, and the proof given there should generalise to the present setting. A related result in the literature is due to Tate \cite[Theorem A$.3$]{mazur}, who shows that there is an isomorphism of $k[x]$-modules between $J = k[x]/(f_1,\ldots,f_n)$ and $\Hom_k(J,k)$ sending the identity to a functional which has a property analogous to \eqref{eq:integrateagainstd}. 

If one knew \textsl{a priori} that this functional was the residue map, this would prove \eqref{eq:integrateagainstd}, but we do not know a reference for this fact! In any case, we give a proof in \cite{??}, which in particular proves \eqref{eq:integrateagainstd} for any noetherian $\nQ$-algebra $k$.
\end{proof}
...
\begin{lemma} Given an $A$-linear homogeneous map $\varphi: X \lto X'$ consider the diagram
\[
...
\[
\xymatrix{
0 \ar[r] & \Delta_n \ar[r]^-{\delta_{-}} & \cdots \ar[r]^-{\delta_{-}} & \Delta_0 \ar[r]^-{\delta_{-}} R \ar[r] & 0
}
\]
...
\]
with the second equality holding because $d$ is $R_2$-linear. For this reason we abuse notation and write $\Atlarrow_2(X^\dual)$ for $\Atlarrow_{2}(X^\dual \otimes X)$. Consider the map
\[
\xymatrix@C+0.5pc{
...
\[
\At_{1}(X^\dual \otimes X) = [s, d_{X^\dual \otimes X}] = [s, d_X] = \At_1(X)
\]
with the second equality holding because $s$ is $R$-linear. 
...
\be
\gamma \lmt \sum_{l \ge 0} (-1)^{l + ml} \varepsilon \left( \Psi \At_1(X^{\vee})^l( \iota'_X ) \wedge \gamma \right) .
\ee
...
\coev_X(\gamma) = \sum_{j,i} (-1)^{\binom{r}{2} + s(\gamma) + r + mr + m|e_j|} \big\{ \partial_{[b_1]}(d_X) \ldots \partial_{[b_r]}(d_X) \big\}_{ij} e_{i} \otimes e_j^* 
\ee
...
\xymatrix{
X^{\vee}[m] \otimes_S X \ar[r] & ( X^{\vee}[m] \otimes_S X ) \otimes_S \bar{S} \cong \bar{X}^{\vee}[m] \otimes_S \bar{X} \ar[rrr]^-{\lambda^{(z)}_{m \bullet} \circ \cdots \circ \lambda^{(z)}_{1 \bullet} \otimes 1} & & & \bar{X}^{\vee} \otimes_S \bar{X}
}
\ee
...
\begin{align}
\eval_X( e_i^* \otimes g e_j ) = \sum_{l \ge 0} \sum_{i_1 < \cdots < i_l} &(-1)^{\binom{l}{2}+ml+(m+l)|e_i|} \, \theta_{i_1} \ldots \theta_{i_l}  \\
...
In this formula $\At(X)$ is an operator on $X \otimes_{R_1} \Bar$, $\At(X^\dual)$ is an operator on $\gamma_*(\Bar) \otimes_{R_1} X^\dual$, and respectively these operators can be identified with Atiyah classes of the $(S \otimes_k \Re)$-modules $X \otimes_{R_1} \Re$ and $\Re \otimes_{R_1} X^\dual$. Tensored over $\Re$ these modules form the $(\Se \otimes \Re)$-module
\be\label{eq:spacehipposattack}
(X \otimes_{R_1} \Re) \otimes_{\Re} (\Re \otimes_{R_1} X^\dual) \cong \Re \otimes_{R_1} ( X \otimes_R X^\dual )\,.
\ee
...
\end{equation}
where $\Lambda^{(x)} = \lambda_1 \cdots \lambda_n$ is the product of homotopies for the action of $\partial_{x_i}W$ introduced earlier, and $\Lambda^{(z)} = \mu_1 \cdots \mu_m$ is the product of homotopies for the action of $\partial_{z_j}V$ on~$X$. That is, $D( \mu_j ) = \partial_{z_j} V \cdot 1_X$ (\textbf{todo} justify lack of symmetrisation).
...
\[
\xymatrix{
Y^{\vee}[m] = {}^\dual Y \ar@{-|}[r] & Y \ar@{-|}[r] & Y^\dual = Y^{\vee}[n]
} .
\]
...
\]
is well-defined, and hypothesis (ii) of the Appendix is checked below in Proposition \ref{prop.morphismmigration}. The upshot is that every $1$-morphism $Y$ in $\LG(W,V)$ has right adjoint $Y^{\vee}[n]$, and similarly one proves that $Y^{\vee}[m]$ is left adjoint to $Y$.
\end{proof}
...
\begin{remark}
The same results also hold for the graded bicategory $\LG^{\mathrm{gr}}$ of Remark~\ref{remark:gradedbicategory}. To see this all we have to check is that the evaluation and coevaluation 2-morphisms have $\nZ$-degree~0. For a potential~$W$ of degree $2c$ we fix the $\nZ$-grading of the unit $\Delta_W$ by assigning degree $-c$ to the~$\theta_i$ in~\eqref{DeltaW}; similarly an $n$-form in the bar complex~\eqref{BarComplex} has degree $-nc$. In this way both the left and right unit actions $\lambda, \rho$ as well as their homotopy inverses manifestly have degree~0. Furthermore, by Definition~\ref{def:coeval} the coevaluation is the image of the (degree~0) identity under the sequence of (degree~0) maps~\eqref{eq:isogivescoev2} and thus has degree~0. That the evaluation also has degree~0 then follows from the Zorro move. 
\end{remark}


\section{How I Learned To Stop Worrying and Love Signs}\label{sec:wiggliesandsigns}

With the proof that the bicategory $\LG$ has adjoints behind us we now want reap some further rewards of our efforts by turning to applications. These will be discussed in the next sections, while in the present one we shall pave the way by clarifying two technical though practically relevant issues: the choice of representative of the isomorphism class of adjoints, and the independence of variable orderings up to signs for all string diagrams in $\LG$. 

\subsection{Alternative adjoints and their graphical presentation}\label{subsec:alternatives}

If adjoints exist for 1-morphisms in a bicategory then they are unique only up to isomorphism. For many purposes this freedom is irrelevant, but in practice a particular choice of isomorphism may prove more useful than another. As we saw above for a 1-morphism~$X$ in $\LG$ its left and right adjoints~${}^\dagger X$ and~$X^\dagger$ are one natural choice. 

On the other hand, we found that a slightly different choice for left adjoints is more suitable in practice, and in this section we will introduce them together with their graphical presentation (which we basically borrow from~\cite{ct1007.2679}). Before doing so we wish however to give reasons for imposing new conventions and notation on the reader, halfway through the paper. While the presentation of adjoints introduced in Section~\ref{subsec:bicat} and employed until now is standard and conceptually clean, for technical computations like in Section~\ref{sec:pivotality} or in our proof of the Cardy condition the choice of adjoints presented below turns out to be more useful. Furthermore, we will see in Section~\ref{sec:ocTFT} that it allows us to unambigiously translate TFT correlators into computable 2-morphisms. And finally, in the new notation we will no longer have to deal with two kinds of daggered adjoints~${}^\dagger X$ and~$X^\dagger$, but only with the dual matrix factorisation~$X^\vee$ (however at the expense of introducing shifted rings into string diagrams). 


We begin by observing that for each 1-morphism $X: (R,W) \lra (S,V)$ in $\LG$ with $R=k[x_1,\ldots,x_n]$ and $S=k[z_1,\ldots,z_m]$ there are isomorphisms
\begin{align*}
X^\dual & = X^\vee[n] \lra (R \otimes_R X^\vee)[n] \lra R[n] \otimes_R X^\vee \, , \\
{}^\dual X & = X^\vee[m] \lra (X^\vee \otimes_S S)[m] \lra X^\vee \otimes_S S[m] \, . 
\end{align*}
By Lemma~\ref{lem:shiftsigns} the first map sends~$\nu$ to $1\otimes \nu$, and in this way we identify~$X^\dual$ and $R[n] \otimes_R X^\dual$, while the second map involves a sign: $\nu \lmt (-1)^{m |\nu|} 1\otimes \nu$. Thus the adjunction maps exhibiting $R[n] \otimes_R X^\vee$ as a right adjoint of~$X$ are precisely the maps $\widetilde\eval_X$ and $\widetilde\coev_X$ we derived earlier in Section~\ref{sec:derivcoeval}, while the adjunction maps making $X^\vee \otimes_S S[m]$ into a left adjoint of~$X$ differ from $\eval_X$ and $\coev_X$ by signs. Namely, the former are 
\begin{align*}
& (X^\vee \otimes_S S[m]) \otimes_S X \ni \nu \otimes \eta \lmt 
\sum_{l \ge 0} (-1)^{l} \Res_{k[z]/k} \left[ \frac{ \Psi \left( \lAt_2(X^{\vee})^l( \nu \circ \Lambda^{(z)} ), \eta\right) \, \underline{\operatorname{d}\!z}}{\partial_{z_1}V, \ldots, \partial_{z_m} V} \right] \in \Delta_W \, , \\
& \Delta_V \ni \gamma  \lmt
...
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (0,0) -- (0.5,0.75);
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (1,0) -- (0.5,0.75);
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{Independence of variable ordering}\label{subsec:variableordering}

Objects of the bicategory $\LG$ are pairs consisting of a polynomial ring $R$ and potential $W$, together with a chosen ordering of the ring variables. Thus the value of a diagram in $\LG$ depends \textsl{a priori} on the ordering of the ring variables in each of its $2$-dimensional regions. In this section we show that the value is actually independent of these orderings, up to the natural permutation signs.
...
\, . 
\ee
We show in Section \ref{sec:defectaction} that the value of this diagram is the endomorphism of $\Delta_V$ given by multiplication with the polynomial $(-1)^{{m+1\choose 2}} \str( \partial_{z_1} d_X \ldots \partial_{z_m} d_X )$. While there is an obvious dependence on the variable ordering, we prove that the homotopy equivalence class of this endomorphism changes only by a permutation sign if we change the ordering of the variables~$z_i$ in the outer region of the diagram.
...
\partial^{\,\sigma}_{[i]}: k[x,x'] \lra k[x,x'] \, , \qquad f \lmt \frac{{}^{t_{\sigma(1)}\ldots t_{\sigma(j-1)}}f - {}^{t_{\sigma(1)}\ldots t_{\sigma(j)}}f}{x_i-x'_i} \, . 
\ee
Together with the morphisms $\rho, \lambda$ of \eqref{lambdarho} this matrix factorisation serves as an alternative unit $1$-endomorphism of $(R,W)$. That is, $\rho$ and $\lambda$ are both isomorphisms and the coherence axioms for the unit in a bicategory holds for $\Delta^\sigma_W$. 

By uniqueness of units in a bicategory there is an isomorphism in $\hmf(\Re, \widetilde W)$
\[
...
\ee
commutes in $\HF(\Re, \widetilde W)$, i.\,e.~$\xi$ is the isomorphism connecting these two stabilisations of the diagonal.
...
\textbf{todo} Signs follow wigglies rule.

Returning to the example \eqref{eq:signdependencepermdia} we have a diagram, in which the top path computes the value of the diagram with outer region given the usual variable ordering, and the bottom path computes the value given the ordering $z_{\tau(1)}, \ldots, z_{\tau(m)}$
\[
\xymatrix@C+2pc@R-1pc{
...
In this section we clarify the relation between the adjunctions given by $\eval_X, \coev_X$ and $\widetilde\eval_X, \widetilde\coev_X$, and we list some of their properties. Of particular importance is the identity~\eqref{eq:monoidalproperty} which we will later need for applications in Sections~\ref{sec:defectaction} and~\ref{sec:ocTFT}, e.\,g.~to prove the Cardy condition. 
...
(TODO: either make everything wiggly above, or repeat the diagrams of Proposition~\ref{prop.morphismmigration} in wiggly notation here. I'd prefer the second option and start using wigglies exclusively only in Proposition~\ref{prop:monoidalproperty}, one reason being that Definition~\ref{def.pivotal} does not want wigglies.)
...
For~$\delta$ to describe a pivotal structure it must be an isomorphism between~$X$ and $X^{\dual\dual} = X^\vee[n]^\dual \cong X^{\vee\vee}[m+n]$. This is indeed true ``half of the time'', to wit whenever $m+n$ is even, and below we will establish that then it also satisfies the conditions in Definition~\ref{def.pivotal}. In particular this means that the rigid monoidal categories $\hmf(\Re,\widetilde W)^\omega = \LG(W,W)$ are pivotal for any potential $W\in R$. However, for $m+n$ odd~$X$ and $X^{\dual\dual}$ are not isomorphic in general (a counterexample being $W=0$, $V=z^d$, $X=S^{\oplus 2}$, $d_X = (\begin{smallmatrix}0&z^n\\z^{d-n}&0\end{smallmatrix})$ for $n\notin d/2$). Thus $\LG$ cannot be pivotal in the above sense. 

What is important for applications though is that identities like those in Definition~\ref{def.pivotal} hold. The precise statement is the next proposition, which can also be read as ``$\LG$ is pivotal up to shifts''. To formulate this result let $W\in k[x_1,\ldots,x_n]$, $V\in k[z_1,\ldots,z_m]$, $U\in k[y_1,\ldots,y_p]$ be potentials and $Y\in \hmf(k[y,z],U-V)$, $X,Z\in \hmf(k[x,z],V-W)$. Then as explained in Appendices~\ref{section:symidem} and~\ref{app:adjoints}, the infinite-rank matrix factorisation $Y\otimes X$ is homotopic to a direct summand of the finite-rank matrix factorisation
$$
\bar Y \otimes \bar X := (Y \otimes_{k[z]} X) \otimes_{k[z]} k[z]/(\partial_{z_1}V, \ldots, \partial_{z_m}V) \, . 
$$
This direct summand is the splitting of an idempotent~$e$ on $Y\otimes X$, i.\,e.~there are maps $\kappa: Y \otimes X \lra \bar Y \otimes \bar X$ and $\xi: \bar Y \otimes \bar X \lra Y \otimes X$ such that $\kappa\xi = 1$ and $\xi\kappa=e$. The adjunction maps for $Y\otimes X$ are appropriate compositions of those for $\bar Y \otimes \bar X$ with the maps~$\kappa$ and~$\xi$, see Appendix~\ref{app:adjoints}. 

\begin{proposition}\label{prop:monoidalproperty}
In the above setting the following identities hold true: 
\begin{enumerate}
\item 
For any map~$\varphi:Z\lra X$ in $\hmf(k[x,z], V-W)$ we have 
$$
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=0cm]
\draw[line width=0] 
...
(3,-1) node[line width=0pt] (X) {{\small $X^\vee$}}
(-1,2) node[line width=0pt] (XY) {{\small $(\bar Y\otimes \bar X)^\vee$}}; 
...
\draw (-1,0) -- (XY);
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-0.35,-0.76) .. controls +(-0.7,0.5) and +(0.1,-1) .. (-0.85,1.6); 
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (2.15,-0.65) .. controls +(0,1) and +(0.7,-0.15) .. (1.5,0.75); 
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (3.15,-0.65) .. controls +(0,1.1) and +(1.8,0) .. (1.5,1.5); 
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
=
%%%%%%%%%%%%%%%%%%%%%%%
...
%(-1,-1) node[line width=0pt] (Y) {{\small $X^\vee$}}
%(2,2) node[line width=0pt] (XY) {{\small $(\bar Y\otimes \bar X)^\vee$}}; 
%\draw[redirected] (0,0) .. controls +(0,1) and +(0,1) .. (-1,0);
...
%\draw (2,0) -- (XY);
%%
%\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (1.85,1.59) .. controls +(-0.01,-1.6) and +(0.7,0.1) .. (1.25,-0.7); 
%%
%\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-1.15,-0.65) .. controls +(0,1) and +(-0.7,0) .. (-0.4,0.77); 
%%
%\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-2.15,-0.65) .. controls +(0,1.1) and +(-1.8,0) .. (-0.4,1.5); 
%\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=0.8,color=blue!50!black, baseline, rotate=180]
...
\draw (2,0) -- (XY);
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (2.15,1.59) .. controls +(-0.01,-1.0) and +(0.7,0.1) .. (1.25,-0.7); 
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-0.85,-0.65) .. controls +(-0.3,0.9) and +(-0.7,0) .. (-0.4,0.77); 
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-1.85,-0.65) .. controls +(-0.3,1.0) and +(-1.8,0) .. (-0.4,1.5); 
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
\ee
\end{enumerate}
...
To establish part~(ii), let $\{e_i\}_i$ be a $k[x,z]$-basis of~$X$, $\{f_j\}_j$ a $k[y,z]$-basis of~$Y$, and $\{g_\alpha\}_\alpha$ a $k$-basis of $k[z]/(\partial V)$. We want to compute the left-hand side of~\eqref{eq:monoidalproperty} which we write in more detail as
\be\label{eq:leftmonoidaldetail}
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline, rotate=180]
\draw[line width=1pt] 
(2,-2) node[line width=0pt] (Y) {{\small $Y^\vee$}}
(3,-2) node[line width=0pt] (X) {{\small $X^\vee$}}
(-1,3) node[line width=0pt] (XY) {{\small $(\bar Y\otimes \bar X)^\vee$}}; 
\draw[redirected] (1,0) .. controls +(0,1) and +(0,1) .. (2,0);
\draw[redirected] (0,0) .. controls +(0,2) and +(0,2) .. (3,0);
\draw[redirected] (-1,0) .. controls +(0,-1) and +(0,-1) .. (0.5,0);
\draw (2,0) -- (Y);
...
\draw (-1,0) -- (XY);
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-0.35,-0.76) .. controls +(-0.7,0.5) and +(0.1,-1) .. (-0.85,2.67); 
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (2.15,-1.735) .. controls +(0,2) and +(0.7,-0.15) .. (1.5,0.75); 
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (3.15,-1.735) .. controls +(0,2) and +(1.8,0) .. (1.5,1.5); 
%
%
...
\, .
\ee
Using the expressions~\eqref{eq:formulalambdainv} and~\eqref{eq:alt_formula_coev} together with Lemma~\ref{lemma:atshufat} we find that $\lambda^{-1}$ and $\widetilde\coev_X$ send $(f_a \otimes g_\alpha e_b)^* \in (\bar Y \otimes \bar X)^\vee$ to
$$
\sum_i (-1)^{n + |e_i| + n(|f_a| + |e_b| + |e_i|)} \, e_i^* \otimes \varepsilon \Psi \At_1 \big(X\otimes (\bar Y \otimes \bar X)^\vee \big)^n \big(e_i\otimes (f_a \otimes g_\alpha e_b)^*\big) \, . 
$$
Similarly with~\eqref{eq:formula_rhoinverse} and~\eqref{eq:coev10} we compute that $\rho^{-1}$ and $\widetilde\coev_Y$ map this to
\begin{align*}
&\sum_{i,j} 
(-1)^{ n + (m+n+1)|e_i| + (m+1)|f_j| + n( |f_a| + |e_b| ) } \, \\
& \qquad\; \cdot \varepsilon \Psi \At_2(X^\vee \otimes Y^\vee)^m (e_i^* \otimes f_j^*)
\otimes \varepsilon \Psi \At_1\big((Y\otimes X)\otimes( \bar Y \otimes \bar X)^\vee\big)^n \big( f_j \otimes e_i \otimes (f_a \otimes g_\alpha e_b)^*\big) \, . 
\end{align*}
Next we apply the map~$\kappa$ which is simply the projection. Thus by the naturality of the Atiyah class the sole effect of~$\kappa$ on the above is to replace $\At_1((Y\otimes X)\otimes( \bar Y \otimes \bar X)^\vee)$ by $\At_1((\bar Y\otimes \bar X)\otimes( \bar Y \otimes \bar X)^\vee)$. (TODO: $g_\alpha$...) This means we are precisely in the same situation as in the proof of the Zorro move for $\bar Y \otimes \bar X$, i.\,e.~after applying $\widetilde\eval_{\bar Y \otimes \bar X}$ and~$\rho$ we are left with
$$
(-1)^{ n + m|e_b| + (m+n)|f_a| } \, 
g_\alpha^* \circ \varepsilon \Psi \At_2(X^\vee \otimes Y^\vee)^m (e_b^* \otimes f_a^*) \, . 
$$
(TODO: explain notation in rest of argument) Similarly the right-hand side of~\eqref{eq:leftmonoidaldetail} maps $(f_a \otimes g_\alpha e_b)^*$ to (TODO: that's before wiggly-line-crossing!)
$$
(-1)^{ p + mp + m|e_b| + (m+p) |f_a| } \, 
g_\alpha^* \circ \varepsilon \Psi \At_1 (X^\vee \otimes Y^\vee)^m (e_b^* \otimes f_a^*) \, . 
$$
TODO
...
$$
in terms of the morphisms encoding the monoidal and adjunction structures as follows. For $\phi\in \End(\Delta_V)$ and $\psi\in \End(\Delta_W)$ we set (TODO: how to write this without shifts and without string diagrams if we don't have extra symbols for wiggly eval and coev?)
\begin{align}
...
\item $\mathcal D_l(X) \circ \mathcal D_l(Y) = \mathcal D_l(Y \otimes X)$ and $\mathcal D_r(Y) \circ \mathcal D_r(X) = \mathcal D_r(Y \otimes X)$. (TODO: maybe have to decorate with signs)
\end{enumerate}
\end{proposition}

\begin{proof}
(i) This is a direct consequence of Proposition~\ref{prop:defectaction} together with the results of~\cite[Section\,5.1, Eq.\,(21)]{kr0405232}. 

(ii): By the argument of Remark~\ref{remark:indeptlambda} reordering the partial derivatives of $d_{X^\dual}$ leaves $\mathcal D_r(X^\dual)$ invariant up to a sign. Together with $(d_{X^\vee})_{ij} = (-1)^{|e_j|}(d_X)_{ji}$ one checks that the claim then follows from Proposition~\ref{prop:defectaction}. 

(iii): We prove the identity for $\mathcal D_l$ under the assumption that the variable numbers $n,m,p$ all have the same parity (note that $\mathcal D_l(X) \circ \mathcal D_l(Y)$ would be zero otherwise). The other cases work analogously. 

The basic argument is a standard calculation with string diagrams. There are however slight modifications needed due to the fact that $Y\otimes X$ is not equal but only homotopic to a finite-rank matrix factorisation. Recall from Section~\ref{sec:pivotality} that the latter is the splitting of an idempotent $e=\xi\kappa$ on $\bar Y \otimes \bar X = (Y \otimes_{k[z]} X) \otimes_{k[z]} k[z]/(\partial_{z_1}V, \ldots, \partial_{z_m}V)$. 

In this setting we compute $\mathcal D_l(X) \circ \mathcal D_l(Y) = \mathcal D_l(Y \otimes X)$ as
$$
\!\!\!\!\!\!\!\!\!\!\!\!
%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%
$$
where here and below we do not display the argument of the defect operators. In the first step we used the Zorro moves to rewrite the identity on $Y\otimes X$, and in the second step we used $1_{Y\otimes X} = \kappa\xi$ together with the ``morphism migration properties'' of Proposition~\ref{prop.morphismmigration}. Next we apply Proposition~\ref{prop:monoidalproperty} to find that up to a factor of $(-1)^{TODO}$, the above equals
$$
\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=0.7,color=blue!50!black, baseline=-0.75cm,>=stealth]
...
\draw (0.7,0.23) node {{\scriptsize $Y$}};
\draw (1.7,0.23) node {{\scriptsize $X$}};
\draw (4.7,0.5) node {{\scriptsize $\bar Y \otimes \bar X$}};
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
\,= \!\!\!
%%%%%%%%%%%%%%%%%%%%%%
...
% 2-morphisms: 
\coordinate (e) at ($ (1,-1) $);
\fill (e) circle (2.5pt) node[right] {{\small$e$}};
% line labels: 
\draw (1.6,0.5) node {{\scriptsize $\bar Y \otimes \bar X$}};
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
\,= \!\!\!
%%%%%%%%%%%%%%%%%%%%%%
...
\fill (one) circle (2.5pt) node[right] {{\small$\kappa\xi$}};
% line labels: 
\draw (1.6,0.5) node {{\scriptsize $Y \otimes X$}};
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
\, . 
$$
Here we used the Zorro moves for $Y, X, \bar Y \otimes \bar X$ as well as $e^2=e=\xi\kappa$ and another application of Proposition~\ref{prop.morphismmigration}. 
\end{proof}
...
\eval_Y (f_i^* \otimes f_j) = (-1)^{n + n |f_j|} \Res_{k[x]/k} \left[ \frac{ (\Lambda_Y)_{ij} \,\underline{\operatorname{d}\! x}}{\partial_1 W, \ldots, \partial_n W} \right] , \nonumber \\
...
$$
Using the expressions $\beta^X(\varphi) = (-1)^{n+1\choose 2} \str(\varphi \Lambda_X)$ and $\beta^Y(\psi) = (-1)^{n+1\choose 2} \str(\psi \Lambda_Y)$ for the boundary-bulk maps we thus find from the above that
\be\label{C1}
C = (-1)^{n+1\choose 2} \,  
\Res_{k[x]/k} \left[ \frac{\beta^X(\varphi) \, \beta^Y (\psi) \, \underline{\operatorname{d}\! x}}{\partial_1 W, \ldots, \partial_n W} \right] .
\ee

On the other hand, we may compute~$C$ by first fusing the two loops in~\eqref{annulus} together to obtain a single loop labelled by ${}^\dual X\otimes Y = \Hom(X,Y)[n] \in \End_{\LG}( (k,0) )$. Then we use
$$
...
Under the conditions (H1)--(H3) of Theorem \ref{theorem:generalkl} the pairing $\langle -, - \rangle$ is adjoint to a homotopy equivalence of $\nZ  \times \nZ _2$-graded complexes over $k$
\[
...
\begin{lemma} For any $\sigma \in S_n$ the map
\[
