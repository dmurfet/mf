...
\begin{proposition} The element $d = \det( \partial_{[i]}(f_j) )$ in $k[x,x']$ has the property that for any $g \in k[x]$
\be\label{eq:integrateagainstd}
\Res_{k[x,x']/k[x']} \left[ \frac{g \cdot d \cdot \underline{\operatorname{d}\! x}}{f_1, \ldots, f_n} \right] = g(x')
\ee
...
Let $k$ be a ring, $R$ a $k$-algebra, and set $\Re = R_1 \otimes_k R_2$ where $R_i = R$ for $i \in \{1,2\}$. We define  the operators $s,d$ on the bar complex $\Bar = \Bar_{R/k}$ as in Section \ref{subsec:Bar} and recall that $\Bar$ is equipped with an $\Re $-bimodule structure by left and right multiplication. Let $(X,D)$ be a pair consisting of a $\nZ _2$-graded free $\Re $-module $X$ and an odd $\Re $-linear operator $D$ on $X$ (for the moment we do not require any condition on $D^2$). By default $\otimes$ means $\otimes_{\Re }$ in this section.
...
\ee
and the bulk and boundary pairings BLA
\begin{align}
