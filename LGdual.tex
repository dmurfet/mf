...
In the bicategory $\LG$ a $1$-morphism $W \lto V$ is represented by a finite rank matrix factorisation $X$ of $V - W$ over $k[x,z]$, where $W\in k[x] = k[x_1,\ldots,x_n]$ and $V\in k[z] = k[z_1,\ldots,z_m]$ are potentials. The \textsl{right} and \emph{left adjoint} of this $1$-morphism are defined respectively to be
\[
X^\dual = X^\vee[n], \qquad {}^\dual X = X^\vee[m]\,.
\]
These are objects of $\hmf(k[x,z], W-V)$, with the differentials as spelled out in \eqref{eq:differentials_adjoints}.
...
\, , \qquad
\eval_X: {}^\dual X \otimes_{k[z]} X \lto \Delta_W\\
...
To simplify the notation we set $R = k[x]$ and $S = k[z]$ so that $\Re  = R_1 \otimes_k R_2$ with $R_i = R$ and $\Se  = S_1 \otimes_k S_2$ with $S_i = S$. Then $\Delta_W$ and $X^\dual \otimes X$ are $\Re $-linear factorisations of $\widetilde{W} = W \otimes 1 - 1 \otimes W$ and $\eval_X, \widetilde\coev_X$ are $\Re $-linear, while $\Delta_V$ and $X \otimes X^\dual$ are $\Se$-linear factorisations of $\widetilde{V} = V \otimes 1 - 1 \otimes V$ and $\coev_X, \widetilde\eval_X$ are $\Se$-linear. The differentials on $\Delta_V, \Delta_W$ are given by \eqref{DeltaW}. 


We fix a homogeneous basis $\{ e_i \}_{i \in I}$ for $X$ with dual basis $\{ e_i^* \}_{i \in I}$.

A representative for the coevaluation morphism is the chain map toad
\begin{gather*}
\widetilde\coev_X(\gamma) = \varepsilon \left( \gamma \wedge \sum_{m \ge 0} (-1)^m \Psi \Atlarrow_2(X^\vee)^m( \iota_X ) \right)\,, \label{eq:coev10}\\
\iota_X = \sum_{j} (-1)^{|e_j|} e_j^* \otimes e_j \in X^\dual \otimes_S X\,.
\end{gather*}
then
\begin{gather*}
\coev_X( \gamma ) = \sum_{i \ge 0} (-1)^i \varepsilon \left( \gamma \wedge \Psi \Atlarrow_2(X)^i( \iota_X ) \right)\,,\\
\iota'_X = \sum_j (-1)^{m|e_j|} e_j \otimes e_j^* \in X \otimes_R {}^\dual X\,.
\end{gather*}
