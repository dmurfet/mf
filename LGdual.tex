...
\subsection{Compatibility with adjoint morphisms}

We end this section with a brief discussion of the basic compatibility properties of the adjunction maps with arbitrary 2-morphisms. For a map $\varphi: X \lra Y$ between matrix factorisations in $\hmf(k[x_1,\ldots,x_n,z_1,\ldots,z_m], V-W)$ 
...
\section{Relations between adjoints}\label{sec:wiggliesandsigns}

With the proof that the bicategory $\LG$ has adjoints behind us we now want to reap some further rewards of our efforts by turning to applications. These will be discussed in the next sections, while in the present one we shall pave the way by clarifying two technical though practically relevant issues: the choice of representative of the isomorphism class of adjoints, and the nature of the double-adjoint. 

%and the independence of variable orderings up to signs for all string diagrams in $\LG$. 
...
\end{align*}
In this notation the Zorro moves for the left and right adjoints read
$$
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=0cm]
\draw[directed] (0,0) .. controls +(0,-1) and +(0,-1) .. (-1,0);
...
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (0.5,0.75) .. controls +(0.75,-0.05) and +(0,1) .. (1.1,-1.25); 
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
\;=\;
%%%%%%%%%%%%%%%%%%%%%%
...
As mentioned in the Introduction, the adjoint $X^\dagger$ of a 1-morphism~$X$ is interpreted as describing the same defect condition as~$X$ buth with the opposite orientation. Since reversing the orientation twice is doing nothing, a natural expectation is thus that $X^{\dagger\dagger}$ should be identified with~$X$ in a suitable sense. In the categorical approach to rational conformal field theory (conjecturally related to Landau-Ginzburg models via the CFT/LG correspondence) this is made precise by the results that there is a \textsl{pivotal structure}, i.\,e.~a natural monoidal transformation~$\delta$ between the identity and the functor $(-)^{\dagger\dagger}$. 
...
For~$\delta$ to describe a pivotal structure it must be an isomorphism between~$X$ and $X^{\dual\dual} = X^\vee[n]^\dual \cong X^{\vee\vee}[m+n]$. This is indeed true ``half of the time'', to wit whenever $m+n$ is even, and below in Proposition~\ref{prop:monoidalproperty} we will establish properties of the adjunction maps that are equivalent to pivotality in this case. In particular this means that the rigid monoidal categories $\hmf(\Re,\widetilde W)^\omega = \LG(W,W)$ are pivotal for any potential $W\in R$. However, for $m+n$ odd~$X$ and $X^{\dual\dual}$ are not isomorphic in general (a counterexample being $W=0$, $V=z^d$, $X=S^{\oplus 2}$, $d_X = (\begin{smallmatrix}0&z^n\\z^{d-n}&0\end{smallmatrix})$ for $n\notin d/2$). Thus $\LG$ cannot be pivotal in the strict above sense. 

What what is important for applications thought, and what we deem the appropriate notion of pivotality in our setting, is that identities of Proposition~\ref{prop:monoidalproperty} hold, which can also be read as ``$\LG$ is pivotal up to shifts''. To formulate this result let $W\in k[x_1,\ldots,x_n]$, $V\in k[z_1,\ldots,z_m]$, $U\in k[y_1,\ldots,y_p]$ be potentials and $Y\in \hmf(k[y,z],U-V)$, $X,Z\in \hmf(k[x,z],V-W)$. Then as explained in Appendices~\ref{section:symidem} and~\ref{app:adjoints}, the infinite-rank matrix factorisation $Y\otimes X$ is homotopic to a direct summand of the finite-rank matrix factorisation
$$
...
\draw[line width=1pt] 
(2,-1) node[line width=0pt, right] (Y) {{\small $Y$}}
(3,-1) node[line width=0pt, right] (X) {{\small $X$}}
(-1,2) node[line width=0pt, right] (XY) {{\small $\bar Y\otimes \bar X$}}; 
...
\draw (-1,0) -- (-1,2);
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-0.35,-0.76) .. controls +(-0.7,0.5) and +(0.1,-1) .. (-0.85,2); 
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (2.15,-1) .. controls +(0,1) and +(0.7,-0.15) .. (1.5,0.75); 
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (3.15,-1) .. controls +(0,1.1) and +(1.8,0) .. (1.5,1.5); 
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
=
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{tikzpicture}[very thick,scale=0.8,color=blue!50!black, baseline, rotate=180]
\draw[line width=1pt] 
(-2,-1) node[line width=0pt, right] (X) {{\small $Y$}}
(-1,-1) node[line width=0pt, right] (Y) {{\small $X$}}
(2,2) node[line width=0pt, right] (XY) {{\small $\bar Y\otimes \bar X$}}; 
\draw[redirected] (0,0) .. controls +(0,1) and +(0,1) .. (-1,0);
\draw[redirected] (1,0) .. controls +(0,2) and +(0,2) .. (-2,0);
\draw[redirected] (2,0) .. controls +(0,-1) and +(0,-1) .. (0.5,0);
\draw (-1,0) -- (-1,-1);
...
\draw (2,0) -- (2,2);
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (2.15,2) .. controls +(-0.01,-1.0) and +(0.7,0.1) .. (1.25,-0.7); 
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-0.85,-1) .. controls +(-0.3,0.9) and +(-0.7,0) .. (-0.4,0.77); 
%
\draw[very thin, color=blue!55!white, decorate, decoration={snake, amplitude=0.2mm, segment length=1.0mm}] (-1.85,-1) .. controls +(-0.3,1.0) and +(-1.8,0) .. (-0.4,1.5); 
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%
\ee
\end{enumerate}
