...
The general rule is that permuting the variables in a region by $\tau$ changes the value of a diagram by a factor of $(-1)^{M|\tau|}$ where $M$ is the number of wiggly lines (see Section \ref{??}) entering or departing an evaluation or coevaluation within that region. The simplest example is \eqref{eq:signdependencepermdia} where the value changes by the sign $(-1)^{|\tau|}$.
