...
\xymatrix@C+2pc{
X \ar[r]^-{\rho^{-1}} & X \otimes \Delta \ar[r]^-{1 \otimes\,\widetilde{\coev}} & X \otimes X^{\dagger} \otimes X \ar[r]^-{\widetilde{\eval}\, \otimes 1} & \Delta \otimes X \ar[r]^-{\lambda} & X\,,
}
\ee
...
\end{align*}
Note that tensoring with the diagonal defines a map $(-)|_{z=z'}: X \otimes X^{\dagger} \lto \Hom_{k[x,z]}(X,X)$ (\textbf{more} there is no longer a $z'$?) and $\mathcal{O}(\theta)$ denotes higher order terms which do not survive $\lambda$ and therefore may be ignored for present purposes. Also define $\nabla$ and Atiyah classes (\textbf{todo}).
...
\end{align} 
In the first step we use that $\Psi$ intertwines the shuffle product on $\Bar$ with the exterior product on $\Delta$, and in the last step we use Lemma \ref{lemma:shuffleatiyah} below, whose proof is delayed to the end of the section. Applying $\eval$ in the first two components of $X \otimes X^{\dagger} \otimes X$ to the expression in (\ref{firstzorromaps}) has the effect of identifying $z$ with $z'$. This changes the associative Atiyah class of $X \otimes X^{\dagger}$ to the associative Atiyah class of $\End(X)$ and so
\begin{align}
...
\begin{lemma}\label{lemma:shuffleatiyah} $\At \times \At = \At$.
\end{lemma}

The form of self-adjointness of the Atiyah class that we will use is contained in the next result. The $p = 0$ case is the statement that for $\varphi \in \End(X) \otimes_{k[x]} \Bar$
\[
...
\begin{lemma}\label{lemma:langlecalc1} For $\varphi \in \End(X)$
\[
\big\langle\!\big\langle \str \big( \underline\lambda \At^n (\varphi)\big) \big\rangle\!\big\rangle = \str(\varphi)|_{x \mapsto y} + \sum_{p=0}^{n-1} h_i( D \At^{n-p-1}(\varphi) )\,.
\]
\end{lemma}
\begin{proof}
...
But by a small argument involving transition determinants (\textbf{todo}) this shows that
\[
= \str( \varphi )|_{x \mapsto y} + \sum_{p=0}^{n-1} h_i( D \At^{n-p-1}(\varphi) ).
\]
\end{proof}
