...
In this section we recall the definition of various canonical maps. Throughout $R$ is a ring and all factorisations are over $R$. Let $X,Y$ denote linear factorisations of $W,W'$ respectively.

\begin{lemma} There is a natural morphism of linear factorisations of $W' - W$
\begin{gather*}
\xi: X^{\vee} \otimes_R Y \lto \Hom_R(X,Y),\,\\
\xi( \nu \otimes y )(x) = (-1)^{|\nu||y|} \nu(x) \cdot y
\end{gather*}
which is an isomorphism if $X$ is a finitely generated projective $R$-module.
\end{lemma}

\begin{lemma} There is a natural isomorphism of linear factorisations of $W + W'$
\begin{gather*}
\textup{swap}: X \otimes_R Y \lto Y \otimes_R X\,,\\
...
\begin{gather*}
Y \otimes_R X^{\vee} \lto \Hom_R(X,Y)\,,\\
y \otimes \nu \mapsto \big\{ x \mapsto \nu(y) \cdot x \big\}\,.
\end{gather*}
Since it is unlikely to cause confusion, we also denote this map by $\xi$.

\begin{lemma} There are natural isomorphisms of linear factorisations of $W + W'$
\begin{gather*}
X[i] \otimes_R Y \lto (X \otimes_R Y)[i]\,,\\
x \otimes y \mapsto x \otimes y\,,
\end{gather*}
and
\begin{gather*}
X \otimes_R Y[i] \lto (X \otimes _R Y)[i]\,,\\
x \otimes y \mapsto (-1)^{i|x|} x \otimes y\,.
\end{gather*}
\end{lemma}
...
\At_{2}(X)^m( e_i \otimes \omega ) = (-1)^{m|e_i| + \binom{m+1}{2}} \sum_{j_1,\ldots,j_m} e_{j_m} \otimes d( D_{j_m, j_{m-1}} ) \cdots d(D_{j_2,j_1})d( D_{j_1 i} ) \omega\,.
\end{equation}
...
\lAt_{2}(X) = [d, D], \qquad \lAt_{1}(X) = [s,D]\,.
\end{equation}
The difference from the other Atiyah classes appears when we exponentiate: 
\begin{equation}\label{eq:leftatiyahexp}
\lAt_{2}(X)^m( \omega \otimes e_i ) = \sum_{j_1,\ldots,j_m} \omega d( D_{j_1,i} ) d(D_{j_2,j_1}) \cdots d(D_{j_m,j_{m-1}}) \otimes e_{j_m}\,.
\end{equation}
...
\[
\Atlarrow_{2}(X) = [ d, D ] \, , \qquad \Atlarrow_{1}(X) = [s, D]\,.
\]
...
\be
\Atlarrow_{2}(X)^m( \omega \otimes e_i ) = (-1)^{m|\omega| + \binom{m}{2}} \sum_{j_1,\ldots,j_m} d(D_{j_m,j_{m-1}}) \cdots d(D_{j_2,j_1}) d(D_{j_1,i}) \omega \otimes e_{j_m}\,.
\ee
We adopt similar notational conventions for the other Atiyah classes.

\subsection{Properties}

With the above notation, let $(X,d_X)$ and $(Y,d_Y)$ be $\mathbb{Z}_2$-graded free $\Re $-modules equipped with odd $\Re $-linear differentials (again, no condition on $d_X^2$ or $d_Y^2$) and respective homogeneous bases $\{ e_i \}_{i \in I}$ and $\{ f_j \}_{j \in J}$. Throughout all Atiyah classes are of the same ``type'' and we write $\At$ for $\At_{2}$. 
...
\begin{remark} To relate this to the earlier construction, observe that if $X$ is a free $\Re $-module with basis $\{ e_i \}_{i \in I}$ then $\Omega R$ can be identified with $\Bar$ and the operator $d: X \lto X \otimes \Bar_1 = X \otimes_R \Omega^1 R$ of \eqref{eq:extendd} is a connection.
\end{remark}
...
% see p.21 of (onct2)
\begin{proof}
With $\alpha = d( W \otimes 1 - 1 \otimes W ) = dW \otimes 1$ and $\omega = a_0 da_1 \cdots da_n \otimes a_{n+1}$ we have
\begin{align*}
\alpha \times \omega &= a_0 dW da_1 \cdots da_n \otimes a_{n+1} - a_0 da_1 dW da_2 \cdots da_n \otimes a_{n+1}\\
&+ \cdots + (-1)^n a_0 da_1 \cdots da_n dW \otimes a_{n+1}\,.
\end{align*}
On the other hand
\begin{align*}
\alpha \times d\omega &= dW da_0 da_1 \cdots da_n \otimes a_{n+1} - da_0 dW da_1 \cdots da_n \otimes a_{n+1}\\
&+ \cdots + (-1)^{n+1} da_0 da_1 \cdots da_n dW \otimes a_{n+1}\,,
\end{align*}
...
&= \sum_{m \ge 0} (-1)^{m+n+n|e_i|} \Psi (1 \otimes \eval''_0)\left( g \sum_{k, k_1, \ldots, k_m} \Lambda_{ki} \cdot d( D_{k_1k} ) \cdots d( D_{k_m, k_{m-1}} ) \otimes e_{k_m} \otimes e_j^* \right)
\end{align*}
Applying $\eval''_0$ involves taking the supertrace $\str( e_{k_m} \circ e_j^* ) = (-1)^{|e_j|}\delta_{j,k_m}$ so that the above equals
\[
\sum_{m \ge 0} \sum_{k,k_1,\ldots,k_{m-1}} (-1)^{m+n+n|e_i|+|e_j|} \Lambda_{ki} \Psi( d(D_{k_1,k}) \cdots d(D_{j,k_{m-1}}) ) \Res_{S \otimes_k R/S} \left[ \frac{ g \,  \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]
\]
...
Let $\{ e_i \}_{i \in I}$ be a homogeneous $(S \otimes_k R)$-basis of $X$, $\{ e_i^* \}_{i \in I}$ the dual basis and $\{ e_{ij} = e_i \circ e_j^* \}_{i \in I, j \in J}$ the determined basis of $\End(X) = \Hom_{S \otimes_k R}(X,X)$. By the Atiyah class $\At = \At( \End(X) )$ of this complex we mean the operator on $\End(X) \otimes_{R_1} \Bar$ which would more properly be called the Atiyah class of $\End(X) \otimes_{R_1} \Re $ determined by the basis $e_{ij}$ using the convention of Remark \ref{remark:linearatother}.
...
\begin{align*}
\rho^{-1}: X \lto X \otimes \Delta \, , \qquad \rho^{-1} &= \sum_{m \ge 0}(-1)^m \Psi \At_2(X)^m \sigma_2\,,\\
\widetilde{\coev}: \Delta \lto X^{\dagger} \otimes X \, , \qquad \widetilde{\coev}(\gamma) &= \varepsilon\Big( \gamma \wedge \sum_{m \ge 0}(-1)^m \Psi \Atlarrow_2(X^{\vee})^m(\iota_X) \Big)\,,\\
&= \sum_{m,j} (-1)^{m+|e_j|} \varepsilon \Big( \gamma \wedge \Psi\!\At_2(X^{\vee})^m( e_j^* ) \otimes e_j \Big)\\
...
\begin{align}
(1 \otimes \widetilde{\coev}) \circ \rho^{-1}( e_q ) &= (1 \otimes \widetilde{\coev}) \sum_{m \ge 0} (-1)^m \Psi \At(X)^m (e_q)\nonumber\\
&= \sum_{j} \sum_{m+m'=n} (-1)^{n+|e_j|} \varepsilon\Big( \Psi \At(X)^m( e_q ) \wedge \Psi \Atlarrow(X^{\vee})^{m'}( e_j^* ) \otimes e_j \Big) \nonumber\\
&= \sum_{j} \sum_{m+m' = n} (-1)^{n+|e_j|} \varepsilon\Psi\Big( \At(X)^m( e_q ) \times \Atlarrow(X^{\vee})^{m'}( e_j^* ) \otimes e_j \Big) \label{eq:zorro1a12}
\end{align} 
In the last step we use that $\Psi$ intertwines the shuffle product on $\Bar$ with the exterior product on~$\Delta$. Next we apply Lemma \ref{lemma:atshufat} to write this shuffle product of Atiyah classes as a single Atiyah class. As it stands in \eqref{eq:zorro1a12} the term inside the $\varepsilon \Psi ( - )$ belongs to $X \otimes \gamma_*(\Bar) \otimes X^\vee \otimes X$. Moving the $\Bar$ to the right of the $X^\vee$ introduces a sign $(-1)^{n( |e_j| + m' )}$. 

We account for this sign by using $\Atlarrow(X^{\vee}[n])^{m'} = (-1)^{nm'} \Atlarrow(X^{\vee})^{m'}$ to rewrite \eqref{eq:zorro1a12} as
\begin{align*}
= \sum_{j} \sum_{m+m' = n} (-1)^{n+|e_j|+n(|e_j| + m') + n|e_j|} \varepsilon\Psi\Big( \At(X)^m( e_q ) \times \Atlarrow(X^{\vee}[n])^{m'}( e_j^* ) \otimes e_j \Big)\,.
\end{align*}
...
\]
The empty sequence $\emptyset$ is the unique sequence of length $\ell(\emptyset) = 0$. We write $\Lambda_{i_1,\ldots,i_p}$ [TODO: why commas between indices=] for the product $\Lambda = \lambda_1 \cdots \lambda_n$ with the $\lambda_{i_1},\ldots,\lambda_{i_p}$ omitted.
...
\begin{align*}
\sum_{\ell(\bs{i}) = p} (-1)^{\gamma(\bs{i}) + p} & \Big\langle\!\Big\langle d \circ f_{i_1} \circ \cdots \circ d \circ f_{i_p} \str\big( \Lambda_{i_1,\ldots,i_p} \circ \At(\varphi) \big) \Big\rangle\!\Big\rangle\\
&= \sum_{\ell(\bs{i}) = p+1} (-1)^{\gamma(\bs{i}) + p+1} \Big\langle\!\Big\langle d \circ f_{i_1} \circ \cdots \circ d \circ f_{i_{p+1}} \str\big( \Lambda_{i_1,\ldots,i_p,i_{p+1}} \circ \varphi \big) \Big\rangle\!\Big\rangle + h_p( D( \varphi ) )
\end{align*}
...
\[
h_p( \psi ) = \sum_{\ell(\bs{i}) = p} (-1)^{\gamma(\bs{i}) +p} \Big\langle\!\Big\langle d \circ f_{i_1} \circ \cdots \circ d \circ f_{i_p} \str\big( [ \Lambda_{i_1,\ldots,i_p}, d](\psi) \big) \Big\rangle\!\Big\rangle\,.
\]
\end{lemma}
\begin{proof}
Fix a sequence $\bs{i}$ of length $p$ and let $\psi = \Lambda_{i_1,\ldots,i_p}$ and let $\eta$ be the operator $\eta = d \circ f_{i_1} \circ \cdots \circ d \circ f_{i_p}$ on $\Bar$, where the $f_i$ stand for multiplication (on the left) by $f_i$. Then by Lemma \ref{lemma:lameadjoints}, or rather the proof with $\dlangle - \drangle$ replaced by $\dlangle \eta( - ) \drangle$ we have
\begin{align}
...
\[
\sum_{a \notin \{ i_1,\ldots, i_p \}} (-1)^{a-1 + \#\{ b \l i_b < a \}} \bigdlangle d \circ f_{i_1} \circ \cdots \circ d \circ f_{i_p} f_a \str\big( \big[ D, \Lambda_{i_1,\ldots,i_p,a} \big]( d \varphi ) \big) \bigdrangle \, .
\]
...
\begin{align*}
\big\langle\!\big\langle \str \big( \Lambda \At^n (\varphi)\big) \big\rangle\!\big\rangle = (-1)^n \sum_{\sigma \in S_n} (-1)^{|\sigma|} \big\langle\!\big\langle d \circ f_{\sigma(1)} \circ \cdots \circ d \circ f_{\sigma(n)} \str(\varphi) \big\rangle\!\big\rangle + \sum_{p=0}^{n-1} h_i( D \At^{n-p-1}(\varphi) )\,.
\end{align*}
%where 
%$$
%\delta = 
...
For a map $\varphi: X \lra Y$ between matrix factorisations in $\hmf(k[x_1,\ldots,x_n,z_1,\ldots,z_n], V-W)$ 
%\begin{align*}
...
\end{tikzpicture} 
%%%%%%%%%%%%%%%%%%%%%%
: X\stackrel{\cong}{\lra} {}^\dual X^\dual \cong X^{\vee\vee}[m+m] = X^{\vee\vee}
\, , \qquad 
\delta_X^{-1} = 
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=.4cm]
...
\begin{enumerate}
\item For any map~$\varphi:Z\lra Y$ in $\hmf(k[x_1,\ldots,x_n,z_1,\ldots,z_n], V-W)$ we have 
$$
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=0cm]
\draw[line width=0] 
...
\mathcal D_r(X)(\psi) & = \widetilde\eval_X[m+n] \circ (1_{X}\otimes (\lambda_{{}^\dual X} \circ (\phi\otimes 1_{{}^\dual X})\circ \lambda_{{}^\dual X}^{-1})) \circ \coev_X \, . 
\end{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
\ee
which is a map $k\lra k$ that we call~$C$. In the case of odd~$n$ we have to suspend $\widetilde\eval_X$ (with the dotted line in~\eqref{annulus} representing $\Delta_W[n]$) and use the isomorphisms $\tau: X\otimes X^\vee[n] \lra X[n] \otimes X^\vee$ and $\tau': Y^\vee \otimes \Delta_W[n] \lra Y^\vee[n] \lra \Delta_W$; for~$n$ even this additional notational clutter disappears. 
...
$$
\eval_{{}^\dual X \otimes Y} = \str(-) \, , \qquad \widetilde\coev_{{}^\dual X \otimes Y} : 1 \lmt 1_{X^\dual \otimes Y}
$$
to arrive at
\be\label{C2}
C 
...
$$
with ${}_\psi m_\varphi(\alpha) = \psi\circ\alpha\circ\varphi$ for $\alpha\in \End_k(\Hom_{\hmf(k[x],W)}(X,Y))$. 
\end{theorem}
