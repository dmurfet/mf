...
Our main reference for bicategories is \cite{bor94}. 
...
\]
Given $1$-morphisms $f: A \lto B$ and $g: B \lto C$ we write $g \circ f$ for their \textsl{composite} $c_{ABC}( f, g )$. Given $2$-morphisms $\gamma: f \lto f', \delta: g \lto g'$ we write $\delta \star \gamma$ for $c_{ABC}(\gamma, \delta)$, sometimes called the \textsl{horizontal composite}.
...
In general discussions of bicategories we will follow the notational convention of \cite{bor94}, which is to denote the identity $2$-endomorphism of a $1$-morphism $f: A \lto B$ by $i_f$, and the identity on $1_A$ by $i_A$.
