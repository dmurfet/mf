...
In the notation of Section \ref{section:atiyahclasses} we can describe the homotopy inverses of $\pi$ using Atiyah classes. Note that $X$ is a free $(S \otimes_k \Re)$-module so we are using the convention of Remark \ref{remark:linearatother}.

\begin{proposition}\label{prop:finalpertdefo} The morphism $\pi$ is an $R_2$-linear homotopy equivalence with inverse
\[
\sigma_\infty = \sum_{m \ge 0} (-1)^m \At_{2}(X)^m \sigma_d\,.
\]
...
\]
Expanding this yields $\sum_{m \ge 0} (-1)^m [d, D]^m \sigma$ plus terms involving factors of the form
...
\[
\sigma_\infty = \sum_{m \ge 0} (-1)^m \At_{1}(X)^m \sigma_s\,.
\]
...
\begin{equation}
\pi^{-1}_d = \sum_{m \ge 0} (-1)^m \Psi \At_{2}(X)^m \sigma_d\,,
\end{equation}
while a left $R_1$-linear homotopy inverse is given by
\begin{equation}
\pi^{-1}_s = \sum_{m \ge 0} (-1)^m \Psi \At_{1}(X)^m \sigma_s\,.
\end{equation}
...
\begin{remark} As the proof makes clear, we could have made use of $\pi^{-1}_s$ instead of $\pi^{-1}_d$ in the construction of $\varphi_{\textup{lift}}$, so that an alternative chain level representative of this homotopy class could be given using $\lAt_{1}$ instead of $\lAt_{2}$.
\end{remark}
...
\[
\lambda_X^{-1} = \sum_{m \ge 0} (-1)^m \Psi \At_{1}(X)^m \sigma_s\,, \qquad \rho_X^{-1} = \sum_{m \ge 0} (-1)^m \Psi \At_{2}(X)^m \sigma_d\,.
\]
\end{proposition}
...
In this section $\Delta$ denotes $\Delta_W$. The coevaluation is produced by computing a chain representative for the inverse image of the identity under the canonical quasi-isomorphism of the next result.
...
\Delta^\vee \otimes_{\Re} \left( X^\vee[n] \otimes_{S} X \right) \ar[u]^-{\cong}_-{\xi}\\
\left( X^\vee[n] \otimes_{S} X \right) \otimes_{\Re} \Delta^\vee \ar[u]^-{\cong}_-{\textup{swap}}\\
\left( X^\vee \otimes_{S} X \right) \otimes_{\Re} \Delta^\vee[n] \ar[u]^-{\cong}\\
\left( X^\vee \otimes_{S} X \right) \otimes_{\Re} \Delta' \ar[u]^-{\cong}_-{\zeta} \ar[d]^-{\pi}\\
\left( X^\vee \otimes_{S} X \right) \otimes_{\Re} R \ar[d]_-{\cong}\ar[d]^-{\kappa}\\
\Hom_{R \otimes_k S}(X,X)
}\label{eq:isogivescoev2}
\end{equation}
where for $\eta \in \Delta^\vee, x \in X^\vee \otimes_{S} X$ and $\tau \in \Delta$ we have
\[
\xi( \eta \otimes x )( \tau ) = (-1)^{|\eta||x|} \eta( \tau ) \cdot x\,,
\]
and for $f \in X^\vee$ and $g,g' \in X$: 
\[
\kappa( f \otimes g )(g') = (-1)^{|f||g|} f(g') \cdot g\,.
\]
So to complete the proof we need only to show that the step marked $\pi$ is a right $R$-linear homotopy equivalence. But this is a consequence of Lemma \ref{lemma:koszulstab1} with $A = k[x]$ and $R = \Ae$.
\end{proof}
...
\widetilde\coev^L_X(\gamma) = \varepsilon \left( \gamma \wedge \sum_{m \ge 0} (-1)^m \Psi \Atlarrow_2(X^\dual)^m( \iota_X ) \right)\
\end{equation}
where the Atiyah class sends forms to the left, as explained in Section~\ref{section:atiyahclasses}.
\end{theorem}
\begin{proof}
Let us chase $1_X$ upwards through (\ref{eq:isogivescoev2}), noting that $\iota_X = \kappa^{-1}(1_X)$. With $A = k[x], R = \Ae$ we may apply Corollary \ref{corollary:koszulstab2} to describe a homotopy inverse to $\pi$ in terms of the Atiyah class $[d, d_{X^\dual \otimes X}]$. In light of the observations preceeding the statement of the theorem we have
...
\[
f \otimes \zeta( \theta_{\bs{i}} ) = f \otimes \varepsilon\big( \theta_{\bs{i}} \wedge - \big)
\]
and then to the function $\Delta \lto X^\dagger \otimes X$ defined by $\gamma \lmt (-1)^{|\omega||f|} \varepsilon\big( \gamma \wedge \theta_{\bs{i}} \otimes f \big)$, and so (\ref{eq:coev10}) follows from (\ref{eq:coev11}).
\end{proof}

There is an alternative presentation which we will also need. On the tensor product
\[
( X^\dual \otimes X) \otimes_{\Re } \Bar
\]
...
\[
\widetilde\coev^R_X(\gamma) = \varepsilon \left( \sum_{m \ge 0} (-1)^m \Psi \At_{1}(X)^m( \iota_X ) \wedge \gamma \right)\,.
\]
\end{theorem}
...
$$ 
For example the operators $\lambda_i = \partial_{x_i} d_X$ would do, but it will be important to allow other choices as well. 
...
\begin{remark} For the moment in this section there is a universal sign factor $\chi$, yet to be determined. We should also address why the map is independent up to homotopy of the choice of $\lambda_i$, by just citing the pushforward paper with Toby perhaps.
\end{remark}
...
\begin{align}
\eval_0( \eta \otimes \nu ) &= (-1)^{\chi + n|\eta| +n} \Res_{\Se  \otimes_k R/\Se } \left[ \frac{ \str( \Lambda \circ \eta \circ \nu) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]\\
...
\widetilde\eval_X( \eta \otimes \nu ) = \sum_{m \ge 0} (-1)^{m+n+n|\eta| + |\nu| + \chi} \Res_{\Se  \otimes_k R/\Se } \left[ \frac{ \Psi \nu \big( \At_2(X)^m \Lambda ( \eta ) \big) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]
\ee
\end{proposition}

Before giving the proof, let us be clear that the term under the residue is the composite
\[
\xymatrix@C+1pc{
...
\begin{proof}[Proof of Proposition \ref{prop:constructevalmap}] Consider the commutative diagram
...
\[
\sum_{m \ge 0} \sum_{k,k_1,\ldots,k_{m-1}} (-1)^{m+n+n|e_i|+|e_j|+\chi} \Lambda_{ki} \Psi( d(D_{k_1,k}) \cdots d(D_{j,k_{m-1}}) ) \Res_{S \otimes_k R/S} \left[ \frac{ g \,  \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right]
\]
...
\, ,\qquad
\big\langle\!\big\langle \alpha \big\rangle\!\big\rangle = (-1)^{\chi} \Res_{\Re /R_2} \left[ \frac{ \varepsilon\Psi( \alpha ) \, \underline{\operatorname{d}\!x}}{\partial_{x_1}W \ldots \partial_{x_n} W} \right] .
$$
...
\mathcal Z = \sum_j (-1)^{(n+1)|e_j|} \big\langle\!\big\langle \str \big( \Lambda \circ \At^n (-\circ e_j^*) \big) \big\rangle\!\big\rangle \cdot e_j
\ee
...
\begin{align*}
\rho^{-1}: X \lto X \otimes \Delta \, , \qquad \rho^{-1} &= \sum_{m \ge 0}(-1)^m \Psi \At_2(X)^m \sigma_d\,,\\
\widetilde{\coev}: \Delta \lto X^{\dagger} \otimes X \, , \qquad \widetilde{\coev}(\gamma) &= \varepsilon\Big( \gamma \wedge \sum_{m \ge 0}(-1)^m \Psi \Atlarrow_2(X^{\dagger})^m(\iota_X) \Big)\,,\\
&= \sum_{m,j} (-1)^{m+|e_j|} \varepsilon \Big( \gamma \wedge \Psi\!\At_2(X^{\dagger})^m( e_j^* ) \otimes e_j \Big)\\
...
Consider the image of a basis element $e_q$ under the first two maps of \eqref{eq:zorro1a1}: 
\begin{align}
(1 \otimes \widetilde{\coev}) \circ \rho^{-1}( e_q ) &= (1 \otimes \widetilde{\coev}) \sum_{m \ge 0} (-1)^m \Psi \At_2(X)^m (e_q)\nonumber\\
&= \sum_{j} \sum_{m+m'=n} (-1)^{n+|e_j|} \varepsilon\Big( \Psi \At(X)^m( e_q ) \wedge \Psi \Atlarrow(X^{\dagger})^{m'}( e_j^* ) \otimes e_j \Big) \nonumber\\
&= \sum_{j} \sum_{m+m' = n} (-1)^{n+|e_j|} \varepsilon\Psi\Big( \At(X)^m( e_q ) \times \Atlarrow(X^{\dagger})^{m'}( e_j^* ) \otimes e_j \Big) \nonumber
\end{align} 
In the last step we use that $\Psi$ intertwines the shuffle product on $\Bar$ with the exterior product on~$\Delta$. Next we apply Lemma \ref{lemma:atshufat}, or more precisely Remark \ref{remark:twistshufat}, to rewrite this shuffle product of Atiyah classes as the action of a single Atiyah class
\[
= \sum_j (-1)^{n+|e_j|} \varepsilon \Psi \Big( \Atlarrow(X \otimes X^\dagger)^n( e_q \otimes e_j^* ) \otimes e_j \Big)
\]
This is an element of $X \otimes_R X^\dual \otimes_S X$. Applying the last two maps of \eqref{eq:zorro1a1} is, by definition of $\widetilde\eval$, the same as applying the morphism $\eval_0$ of Lemma \ref{lemma:morphismevalzero}. Recall that this map was pieced together step by step at the beginning of Section \ref{subsec:eval}. Since there are some subtleties with signs, let us address the first few steps explicitly: applying the first isomorphism of \eqref{eq:firstisoevalsec} yields
\[
\sum_j (-1)^{n+|e_j|} \varepsilon \Psi \Big( \Atlarrow((X \otimes X^\vee)[n])^n( (-1)^{n|e_q|} e_q \otimes e_j^* ) \otimes e_j \Big)\,.
\]
Since the Atiyah class of $Y[n]$ differs from that of $Y$ by a sign $(-1)^n$, this is the same as
\[
\sum_j (-1)^{|e_j|+n|e_q|} \varepsilon \Psi \Big( \Atlarrow(X \otimes X^\vee)^n( e_q \otimes e_j^* ) \otimes e_j \Big)\,.
\]
The next step of \eqref{eq:firstisoevalsec} is identify the left and right actions of $S$, under which operation  $X \otimes X^\vee$ becomes identified $\End(X)$, so we are left with
\[
\sum_j (-1)^{|e_j|+n|e_q|} \varepsilon \Psi \Big( \Atlarrow( \End(X) )^n( e_q \circ e_j^* ) \otimes e_j \Big)\,.
\]
Switching from the Atiyah class $\Atlarrow$ to one of type $\At$ with noncommutative forms on the right comes at the expense of a sign $(-1)^{n(|e_q|+|e_j|_n}$. The final result of applying \eqref{eq:firstisoevalsec} to $(1 \otimes \widetilde{\coev}) \circ \rho^{-1}( e_q )$ is therefore
\[
\sum_j (-1)^{(n+1)|e_j|+n} \varepsilon \Psi \Big( \At^n( e_q \circ e_j^* ) \otimes e_j \Big)\,.
\]
To finish applying $\eval_0$ it remains to postcompose with $(-1)^n \Lambda$, take the supertrace and then integrate:
\begin{align*}
\mathcal{Z}(e_q) &= \lambda \circ (\widetilde\eval \otimes 1) \circ (1 \otimes \widetilde\coev) \circ \rho^{-1}( e_q )\\
&= \sum_j (-1)^{(n+1)|e_j|+\chi}\Res_{\Re /R_2}\! \left[ \frac{ \varepsilon\Psi \str\Big( \Lambda \circ \At^n( e_q \circ e_j^* ) \Big) \underline{\operatorname{d}\! x} }{\partial_{x_1}W \ldots \partial_{x_n} W} \right] \cdot e_j\\
&= \sum_j (-1)^{(n+1)|e_j|} \big\langle\!\big\langle \str \big( \Lambda \circ \At^n (e_q\circ e_j^*) \big) \big\rangle\!\big\rangle \cdot e_j\,.
\end{align*}
