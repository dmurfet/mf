...
& \widetilde\eval_X = \str_k(-) + \mathcal O(\theta) \, , \qquad \eval_Y = (-1)^{n+1\choose 2} \Res_{k[x]/k} \left[ \frac{\str_k( \, - \circ \Lambda_Y)}{\partial_1 W\ldots \partial_n W} \right] \, , \nonumber \\
...
$$
\eval_{X^\dual \otimes Y} = \str_k(-) \, , \qquad \widetilde\coev_{X^\dual \otimes Y} : 1 \lmt 1_{X^\dual \otimes Y}
$$
to arrive at
\be\label{C2}
C 
...
\ee
Here the second step follows from the general calculus of bicategories with duals; see e.\,g.~\cite[Lemma~3.1(iii)]{cr1006.5609} for a detailed proof [TODO: only if we have pivotality! Need to prove that!]. Comparing~\eqref{C1} and~\eqref{C2} we thus observe: 
