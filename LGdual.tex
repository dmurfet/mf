...
In general we do not phrase the discussion in terms of an isomorphism between $X$ and its double dual, but in terms of the relationship between the left and right adjoints. It is clear from Theorem \ref{theorem:adjointbicategoryyeah} that the left adjoint of any $1$-morphism is isomorphic to a suspension of the right adjoint, and we show in Proposition~\ref{prop:monoidalproperty} below that these isomorphisms possess nontrivial properties that we might call ``pivotality up to shifts''. These properties are what will be important in applications. 

Let $W\in k[x_1,\ldots,x_n]$, $V\in k[z_1,\ldots,z_m]$ be potentials and $X \in \hmf(k[x,z],V-W)$. Then
\[
...
diagram
\]
For the rest of this section, in addition to $W, V$ and $X$ above, let $U\in k[y_1,\ldots,y_p]$ be a potential and let $Y\in \hmf(k[y,z],U-V)$ be given. The property of the isomorphisms $q$ that we wish to prove is that they are, in an appropriate sense, monoidal: the isomorphism
\[
q_{Y \otimes X}: R[p] \otimes_R R[n] \otimes_R (Y \otimes X)^\dual \lto {}^\dual(Y \otimes X)
\]
can be constructed out of $q_X$ and $q_Y$.

Then as explained in Appendices~\ref{section:symidem} and~\ref{app:adjoints}, the infinite-rank matrix factorisation $Y\otimes X$ of $U - W$ over $k[x,y]$ is homotopy equivalent to a direct summand of a finite-rank matrix factorisation. More precisely, we show that the quotient map
$$
...
(TODO: remark that part (ii) isn't so bad if $m=p=0$ mod 2. Also explain why we talk about $\bar{Y} \otimes \bar{X}$ and not $Y \otimes X$)
...
To establish part~(ii), let $\{e_i\}_i$ be a $k[x,z]$-basis of~$X$, $\{f_j\}_j$ a $k[y,z]$-basis of~$Y$, and $\{g_\alpha\}_\alpha$ a $k$-basis of $k[z]/(\partial V)$. Then $\{ f_j \otimes g_\alpha e_i \}_{i,j,\alpha}$ is a $k[x,y]$-basis of $\bar{Y} \otimes \bar{X}$. 

We want to compute the left-hand side of~\eqref{eq:monoidalproperty} which we write in more detail as
...
\, .
\ee
This diagram depicts a morphism $\mathscr{L}: (\bar{Y} \otimes \bar{X})^\dual \lto X^\dual \otimes Y^\dual$. Using the expressions~\eqref{eq:formulalambdainv} and~\eqref{eq:alt_formula_coev} together with Lemma~\ref{lemma:atshufat} we find that $\lambda^{-1}$ and $\widetilde\coev_X$ send $(f_q \otimes g_\alpha e_p)^* \in (\bar Y \otimes \bar X)^\vee$ to
$$
...
\end{align*}
Here there is a recurrence of the subtlety mentioned in Remark \ref{??} and which also appeared in the proof of the Zorro move. Namely that the output of the $\varepsilon \Psi \At_2$ term is an element of $\Se = S_1 \otimes_k S_2$, with the $S_2$ component going to act on the $f_k$ which is subsequently introduced as an argument of the $\varepsilon \Psi \At_1$ term, so we keep in mind that this input is actually a sum of terms of the form $g_\beta f_k$ for some $\beta$.
...
(1 \otimes g_\alpha^*) \circ \varepsilon \Psi \At_2(X^\dual \otimes Y^\dual)^m (e_p^* \otimes f_q^*) \, . 
\ee
...
Similarly the right-hand side of~\eqref{eq:leftmonoidaldetail}, before the engineering of wiggly lines, is
\[
diagram
\]
This depicts a morphism $\mathscr{R}: {}^\dual( \bar{Y} \otimes \bar{X} ) \lto {}^\dual X \otimes {}^\dual Y$. One computes that $\mathscr{R}$ is given by
\be\label{eq:valueofRpivot}
\mathscr{R}( (f_q \otimes g_\alpha e_p)^* ) = (-1)^{m|f_q| + mp} \, 
(g_\alpha^* \otimes 1) \circ \varepsilon \Psi \At_1 ({}^\dual X \otimes {}^\dual Y)^m (e_p^* \otimes f_q^*) \, . 
\ee
Here the output of $\varepsilon$ is an element of $({}^\dual X \otimes {}^\dual Y) \otimes_{S_2} \Se$ and $g_\alpha^*$ now acts on the first factor of $\Se$. 

To relate $\mathscr{L}$ and $\mathscr{R}$ we evidently 
