...
We call these \textsl{associative} Atiyah classes to distinguish them from the more standard Atiyah classes defined using commutative differential forms, see \cite{illusie,markarian,buchweitz03,buchweitz08,huybrechts10,roberts}. Since the latter do not appear in this paper except in Section~\ref{section:dualityadjointop}, which is self-contained, we will usually omit the qualifier ``associative''. 
...
\end{equation}
and the Zorro move is proved. Another example of symmetries of Atiyah classes playing a role in algebraic geometry is appearance of the Bianchi identity in Kapranov's study of Rozansky-Witten invariants \cite{kapranov}.
\end{remark}
...
With the proof that the bicategory $\LG$ has adjoints behind us, we turn in the next several sections to applications. But first we need to introduce a refined diagrammatic calculus more appropriate for working with adjoints in this bicategory; in particular, with this in Section~\ref{sec:ocTFT} we will realise the goal stated in the Introduction of translating arbitrary TFT correlators directly into diagrams in $\LG$. The new diagrams are also the appropriate language for clarifying the nature of double-adjoints. 
...
The new convention is as follows: a wiggly line with label $a \in \nZ$ in a domain associated to an object $(R = k[x_1,\ldots,x_n],W)\in\LG$ represents $R[a]$, that is, the ring~$R$ shifted by $a$. A wiggly line in this domain with no label represents $R[n]$, where $n$ is the number of variables in $R$.
