...
Let $W\in R = k[x_1,\ldots,x_n]$, $V\in S = k[z_1,\ldots,z_m]$ be potentials and $X \in \hmf(k[x,z],V-W)$. Then
\[
X^\dual[m+n] \cong X^{\vee}[n][m+n] \cong X^{\vee}[m] \cong {}^\dual X\,.
\]

\begin{definition} We define the isomorphism
\[
q_X: R[m] \otimes_R R[n] \otimes_R X^\dual \lto {}^\dual X, \qquad q(\nu) = (-1)^{m|\nu|} \nu\,.
\]
\end{definition}

We write $X^\dual[m+n]$ in this unusual way so as to have a simple diagrammatic interpretation:
\[
diagram
\]
...
As mentioned in the Introduction, the adjoint $X^\dagger$ of a 1-morphism~$X$ is interpreted as describing the same defect condition as~$X$ but with the opposite orientation. Since reversing the orientation twice is doing nothing, it is natural to expect that $X^{\dagger\dagger}$ should be identified with~$X$, in a suitable sense. In the categorical approach to rational conformal field theory (conjecturally related to Landau-Ginzburg models via the CFT/LG correspondence) this is made precise by the results which state that there is a \textsl{pivotal structure}, i.\,e.~a natural monoidal transformation~$\delta$ between the identity and the functor $(-)^{\dagger\dagger}$. This implies in particular that the left and right adjoints are isomorphic.

The story for the bicategory $\LG$ is subtler, as we only expect the left and right adjoints of $X$ to be isomorphic when the number of variables in the regions on either side of $X$ agree mod $2$. This is the case, for example, when $X$ is an endomorphism of an object of $\LG$. Indeed, we prove that the rigid monoidal categories $\hmf(\Re,\widetilde W)^\omega = \LG(W,W)$ are pivotal for any potential $W\in R$. See Remark \ref{remark:notpivotal} for the construction of an isomorphism between $X$ and $X^{\dual \dual}$.

In general we do not phrase the discussion in terms of an isomorphism between $X$ and its double dual, but in terms of the relationship between the left and right adjoints. We have specified in \eqref{??} above a choice of isomorphism $q$ between the left adjoint and a suspension of the right adjoint of any $1$-morphism. We show in Proposition~\ref{prop:monoidalproperty} below that these isomorphisms possess nontrivial properties that we might call ``pivotality up to shifts''. These properties are what will be important in applications. 

To explain what we mean, first we recall a formal result of category theory. If $X: W \lto V$ and $Y: V \lto U$ are $1$-morphisms in $\LG$ then the composite $X^{\dual} \otimes Y^{\dual}$ is right adjoint to $Y \otimes X$ and so by uniqueness of adjoints there is a canonical isomorphism
\[
\mathscr{R}: (Y \otimes X)^{\dual} \cong X^{\dual} \otimes Y^{\dual}\,.
\]
Similarly, there is an isomorphism
\[
\mathscr{L}: {}^\dual (Y \otimes X) \cong {}^\dual X \otimes {}^\dual Y\,.
\]
Concretely, we define these morphisms by the diagrams
\[
diagrams
\]
One can ask whether or not these isomorphisms $\mathscr{R}, \mathscr{L}$ are compatible in a sutiable sense with the isomorphism $q$ which exchanges left and right adjoints, and this is precisely the content of the next result. For the statement, we let
\begin{align*}
W &\in R = k[x_1,\ldots,x_n],\\
V &\in S = k[z_1,\ldots,z_m],\\
U &\in k[y_1,\ldots,y_p],
\end{align*}
be potentials and $X \in \hmf(k[x,z],V-W)$ and $Y\in \hmf(k[y,z],U-V)$.

\begin{proposition}\label{prop:monoidalproperty} The diagram
...
\ee
commutes up to homotopy (beware the sign). Equivalently, the following identity holds true: 
...
\end{proposition}
\begin{proof}
The equivalence of commutativity of \eqref{eq:monoidalpropertycomd} and the identity \eqref{eq:monoidalproperty} is an exercise in the diagrammatic calculus for wiggly lines, which we leave to the reader. 
...
\, .
\ee
We denote this morphism by $\mathscr{R}: (\bar{Y} \otimes \bar{X})^\dual \lto X^\dual \otimes Y^\dual$. 

Similarly, after adding $\kappa^{\vee}$ and migrating, the right hand side becomes the morphism $\mathscr{L}$:
\[
diagram (including the wiggly rearrangement on top and bottom)
\]
It remains to compute $\mathscr{R}$ and $\mathscr{L}$ and then prove that $\mathscr{R} = (-1)^m \mathscr{L}$.

Let $\{e_i\}_i$ be a $k[x,z]$-basis of~$X$, $\{f_j\}_j$ a $k[y,z]$-basis of~$Y$, and $\{g_\alpha\}_\alpha$ a $k$-basis of $k[z]/(\partial V)$. Then $\{ f_j \otimes g_\alpha e_i \}_{i,j,\alpha}$ is a $k[x,y]$-basis of $\bar{Y} \otimes \bar{X}$. Using the expressions~\eqref{eq:formulalambdainv} and~\eqref{eq:alt_formula_coev} together with Lemma~\ref{lemma:atshufat} we find that $\lambda^{-1}$ and $\widetilde\coev_X$ send $(f_q \otimes g_\alpha e_p)^* \in (\bar Y \otimes \bar X)^\vee$ to
$$
...
\end{align*}
Here there is a recurrence of the subtlety mentioned in Remark \ref{??} and which also appeared in the proof of the Zorro move. Namely that the output of the $\varepsilon \Psi \At_2$ term is an element of $\Se = S_1 \otimes_k S_2$, with the $S_2$ component going to act on the $f_k$ which is subsequently introduced as an argument of the $\varepsilon \Psi \At_1$ term, so we keep in mind that this input is actually a sum of terms of the form $g_\beta f_k$.
...
\mathscr{R}( (f_q \otimes g_\alpha e_p )^* ) = (-1)^{m|f_q|} \, 
...
Similarly one computes that $\mathscr{L}$ is given by
\be\label{eq:valueofRpivot}
\mathscr{L}( (f_q \otimes g_\alpha e_p)^* ) = (-1)^{m|f_q|} \, 
(g_\alpha^* \otimes 1) \varepsilon \Psi \At_1 (X^\dual \otimes Y^\dual)^m (e_p^* \otimes f_q^*) \, . 
\ee
We prove now that $\mathscr{R}$ is homotopic to $(-1)^m \mathscr{L}$.

The main discrepancy is that $\mathscr{R}$ involves $\At_2$ while $\mathscr{L}$ involves $\At_1$. In order to match these let $\mathscr{R}'$ be the morphism computed by the diagram \eqref{eq:leftmonoidaldetail} but with the variable ordering $z_{m}, z_{m-1}, \ldots, z_1$ on the ring $k[z]$. By Appendix \ref{app:variableordering}, $\mathscr{R}'$ is homotopic to $(-1)^{\binom{m}{2}} \mathscr{R}$ so we must prove that
\be\label{eq:lversusrbeforeh}
\mathscr{R}' = (-1)^{\binom{m+1}{2}} \mathscr{L}.
\ee
Again we need to reduce to finite rank, using the split monomorphism in $\HMF(k[x,y], U - W)$
\[
...
Hence
\begin{align*}
\varepsilon \Psi_\sigma( da_1 \cdots da_m ) &= \partial^{\,\sigma}_{[1]}(a_1) \cdots \partial^{\,\sigma}_{[m]}(a_m)\\
&= \tau\left( \partial_{[m]}(a_1) \cdots \partial_{[1]}(a_m) \right)\,.
\end{align*}
...
\textbf{todo} Here go the diagrams from (cardc3).

\begin{corollary} Give the better versions of the pivotality diagram.
\end{corollary}
...
For~$\delta$ to describe a pivotal structure it must be an isomorphism between~$X$ and $X^{\dual\dual} = X^\vee[n]^\dual \cong X^{\vee\vee}[m+n]$. This is indeed true ``half of the time'', to wit whenever $m+n$ is even, and below in Proposition~\ref{prop:monoidalproperty} we will establish properties of the adjunction maps that are equivalent to pivotality in this case. However, when $m+n$ is odd~$X$ and $X^{\dual\dual}$ are not isomorphic in general (a counterexample being $W=0$, $V=z^d$, $X=S^{\oplus 2}$, $d_X = (\begin{smallmatrix}0&z^n\\z^{d-n}&0\end{smallmatrix})$ for $n\notin d/2$).
\end{remark}

\begin{proposition} For any map~$\varphi:Z\lra X$ in $\hmf(k[x,z], V-W)$ we have 
$$
%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[very thick,scale=1.0,color=blue!50!black, baseline=0cm]
\draw[line width=0] 
...
\end{proposition}
\begin{proof}
Part (i) follows in a standard way by applying Zorro moves together with the ``morphism migration properties'' of Proposition~\ref{prop.morphismmigration}. \textbf{todo} Where does this go?
\end{proof}
