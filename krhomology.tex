...
\begin{example}
\label{MOYremark}
(TODO: call this a \emph{state web}. Make sure that when we refer to this, we call it Example, not Remark) 
...
\]
and make the following assignments of matrix factorisations to vertices (TODO: either always display variable dependence of $\Xcirc, \Xbul$, or never)
\begin{equation}
\label{MFcrossings}
\begin{tikzpicture}[scale=0.7,baseline, inner sep=1mm, >=stealth]
\node (1) at (-1.1,1.0) [circle,inner sep=1.5pt, draw=white,fill= white] {$x^{N+1}_{i}$};
...
\begin{example}\label{example:defectonbrane} (TODO: refer here in Simple Examples and do adjustments there)
Consider the web
$$
...
\[
\idem = \frac{1}{n!}(-1)^{\binom{n+1}{2}} \sum_{\tau \in S_n} \textup{sgn}(\tau)\left\{ \prod_{i=1}^n \lambda_{x_i} \right\}[\partial_{\tau(1)}, d_X] \ldots [\partial_{\tau(n)}, d_X]
\]
...
$$
and we see that the \emph{compiled map} $g'\circ(\Phi\otimes 1)\circ f$ is what~$\Phi$ induces between the compilations~$C$ and~$C'$. Note that we need no knowledge of the maps $\vartheta,\psi$ (TODO: of Theorem~\ref{theorem:compilation_main}) to compute the compiled map. 
