...
TODO: try to improve intro (will do that on Wednesday morning): 
...
\]
Let us fix for example the polynomial $W = x_1^{N+1} + x_2^{N+1}$. These triangulated categories are algebraic, and the general theory of Rouquier \cite{??} applies (\textbf{todo}) to produce an interesting autoequivalence of $\K^b( \hmf(S, W) )$. Namely, tensoring with the complexes of graded bimodules
