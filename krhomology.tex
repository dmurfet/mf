...
TODO: We thank N.~Behr, S.~Gukov, M.~Khovanov, J.~Rasmussen, R.~Rouquier, I.~Runkel, and G.~Watts. 
...
\begin{example} \textbf{todo} Put defect on boundary condition, defect on defect here, refer back to from example section. This helps motivate why we want to ignore the internal variables, or at least gives two motivations (computing cohomology, from KR example, and unphysical $y$-variable from defect example).
\end{example}
...
\end{equation}
determine endofunctors on $\K^b( \hmf(S, W) )$ which are mutually inverse equivalences. In fact these equivalences are the ones associated by Khovanov and Rozansky to over- and under-crossings, so the whole theory is built out of (\ref{eq:bimod_cpx1}) and (\ref{eq:bimod_cpx2}). But there is an additional step before we can phrase the construction in the original terms: writing $R = S/(W)$ there is an equivalence
\[
