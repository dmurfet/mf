...
\[
\xymatrix@C+3pc{
\Modd(A) \ar@/^1pc/[r]^-{\varphi^*} \ar@/_1pc/[r]_-{\varphi^!} & \Modd(S) \ar[l]^-{\varphi_*}
}, \qquad
\xymatrix{
\varphi^* \ar@{-|}[r] & \varphi_* \ar@{-|}[r] & \varphi^!
}
\]
between restriction of scalars $\varphi_*$, extension of scalars $\varphi^* = - \otimes_A S$ and $\varphi^! = \Hom_A(S, -)$. Now any polynomial $f \in S$ be written uniquely as $f_1 + f_2(x_1-x_2)$ for symmetric polynomials $f_i$, so $S$ is a free $A$-module on the basis $\{1, x_1 - x_2\}$ and we write $1^*, (x_1-x_2)^*$ for the corresponding dual basis of $\Hom_A(S,A)$ as an $A$-module. The important point is that there is an isomorphism of $S$-modules
\[
\psi: S \lto \Hom_A(S,A), \quad \psi(1) = (x_1-x_2)^*, \; \psi(x_1-x_2) = 1^*.
\]
From this we deduce a natural isomorphism for any $A$-module $N$
\[
\varphi^!(N) = \Hom_A(S,N) \cong \Hom_A(S,A) \otimes_A N \stackrel{\psi}{\cong} S \otimes_A N = \varphi^*(N),
\]
...
These morphisms will come from the units and counits of the adjunction $\xymatrix@1{\varphi^* \ar@{-|}[r] & \varphi_*}$, which we denote $\eta, \varepsilon$, and the units and counits of the new adjunction $\xymatrix@1{\varphi_* \ar@{-|}[r] & \varphi^*}$, which we denote $\eta', \varepsilon'$. For an $A$-module $N$ and $S$-module $M$ these maps are given by
\begin{align*}
\eta: N \lto \varphi_* \varphi^*(N) = \varphi_*( S \otimes_A N ), \qquad &n \mapsto 1 \otimes n,\\
\eta': M \lto \varphi^* \varphi_*(M) = S \otimes_A \varphi_*(M), \qquad &m \mapsto (x_1-x_2) \otimes m + 1 \otimes (x_1 - x_2) \cdot m
\end{align*}
and
\begin{align*}
\varepsilon: S \otimes_A \varphi_*(M) = \varphi^* \varphi_*(M) \to M, \qquad &b \otimes m \mapsto b \cdot m,\\
\varepsilon': \varphi_*(S \otimes_A N) = \varphi_*\varphi^*(N) \lto N, \qquad &b \otimes n \mapsto \psi(b)(1) \cdot n.
\end{align*}
To make the transition from modules to matrix factorisations, we present the biadjunction between $\varphi^*$ and $\varphi_*$ in terms of bimodules and bimodule morphisms. It is clear that $\varphi^*$ is represented by tensoring with the $S$-$A$-bimodule ${}_S S_A$, and likewise $\varphi_*$ is represented by the $A$-$S$-bimodule ${}_A S_S$. Hence the composite $\varphi^* \varphi_*$ is represented by ${}_S S \otimes_A S {}_S$ and $\varphi_* \varphi^*$ by ${}_A S {}_A$. The units and counits of adjunction are represented by morphisms between these bimodules and the rings $A,S$ as bimodules over themselves, representing the identity functors:
\[
\eta: S \lto S \otimes_A S, \qquad b \mapsto (x_1 - x_2) \otimes b + 1 \otimes (x_1 - x_2)b
\]
and
\[
\varepsilon: S = S \otimes_A A \lto A, \qquad b \mapsto \psi(b)(1).
\]
\textbf{todo} present units and counits of the right-way adjunction. The bimodule $S \otimes_A S$ is of particular interest:

\begin{lemma}\label{lemma:bimodules_as_algebras} Viewing a $\QQ[x_1,x_2,y_1,y_2]$-module as a $S$-bimodule by identifying the action of $y_1, y_2$ with the right action of $x_1, x_2$ respectively, there are isomorphisms of $S$-bimodules
\begin{gather*}
...
Moreover it is clear that these isomorphisms make the following diagrams commute:
\begin{equation}
\label{eq:bimodalg1}
\xymatrix{
S \otimes_A S \ar[d]_{??} \ar[r]^-{\cong} & \QQ[x_1,x_2,y_1,y_2]/(y_1 + y_2 - x_1 - x_2, y_1 y_2 - x_1 x_2) \ar[d]^{\can}\\
S \ar[r]_-{\cong} & \QQ[x_1,x_2,y_1,y_2]/(y_1 - x_1, y_2 - x_2)\,,
}
\end{equation}
where the vertical map on the right is a quotient, and
\begin{equation}
\label{eq:bimodalg2}
\xymatrix{
S \ar[r]_-{\cong}\ar[d]_{??} & \QQ[x_1,x_2,y_1,y_2]/(y_1 - x_1, y_2 - x_2) \ar[d]^{??}\\
S \otimes_A S \ar[r]^-{\cong} & \QQ[x_1,x_2,y_1,y_2]/(y_1 + y_2 - x_1 - x_2, y_1 y_2 - x_1 x_2)\,.
}
\end{equation}
If $W \in A$ is any symmetric polynomial then, since $S$ is free as an $A$-module with basis $\{1, x_1 - x_2\}$, these functors lift to an adjoint pair of functors between the triangulated categories of matrix factorisations
\[
\xymatrix@C+2pc{
\hmf(A, W) \ar@<1ex>[r]^{\varphi^*} & \hmf(S, W) \ar@<1ex>[l]^{\varphi_*}.
}
\]
Let us fix the polynomial $W = x_1^{N+1} + x_2^{N+1}$. The general theory of \cite{??} applies (\emph{details}) to produce an interesting autoequivalence of $\mathbb{K}^b( \hmf(S, W) )$, given by tensoring with the short complex (or something like this)
\[
0 \lto S \otimes_A S \lto S \lto 0.
\]
...
\[
\hmf(S, W) \cong \qderu{b}{\modd S}/\K^b(\modd S)
\]
which says that a matrix factorisation of $W$ remembers the asymptotic part of a free resolution of a $S/W$-module. In this sense $S \otimes_A S, S$, which are both clearly $S/W$-modules, contain far too much information: the action of the corresponding functors on the homotopy category of matrix factorisations will only depend on the asymptotic part of the resolution of $S \otimes_A S, S$ as $S/W$-modules. Extracting this asymptotic part is the process known as \emph{stabilisation}, and will take place in the next section. We will stabilise $S \otimes_A S$ to a matrix factorisation $\Xbul$ and $S$ to $\Xcirc$. The morphisms $??$ lift to the free resolutions of these modules and thus to the asymptotic part, which means that these maps stabilise to morphisms between $\Xcirc$ and $\Xbul$, which are the $\chi$-morphisms of \cite{kr0401268}.
...
We want to stabilise the bimodules $S \otimes_A S$ and $S$ of the previous section to get graded matrix factorisations $\Xbul$ and $\Xcirc$, together with morphisms $\chi_0$ and $\chi_1$ stabilising the units and counits of adjunction in (\ref{eq:bimodalg1}) and (\ref{eq:bimodalg2}). Take $S = \QQ[x_1,x_2]$ as before and set $R = \QQ[x_1,x_2,y_1,y_2]$. The reader wanting to compare with \cite[Section 6]{kr0401268} should make the change of variables $x_1,x_2,y_1,y_2 \leftrightarrow x_4,x_3,x_1,x_2$. Fix an integer $N > 0$ and consider the potential
\[
...
\end{align*}
Of course there is some indeterminacy in the choice of the $u_i$'s, but up to homotopy equivalence the factorisation $\Xbul$ does not depend on the choice of $u_i$'s (this is a standard fact, see for example \cite{??}, and will also be clear from our description of $\Xbul$ as the solution of a universal problem). Moreover, we can and do assume that the polynomials $u_i$ are invariant under interchanging the $x$'s with the $y$'s, that is, we arrange that $u_i(y_1,y_2,x_1,x_2) = u_i(x_1,x_2,y_1,y_2)$ for $i = 1,2$ (see Appendix \ref{??}). We will manage to avoid discussing the explicit form of these polynomials, making use only of (\ref{eq:cons2}) and the symmetry just mentioned.

Recall from Section \ref{prelim:cyclic_koszul} that the model of the Koszul complex that we use is the following: set $F = R \theta_1 \oplus R \theta_2$, where the $\theta$'s are formal symbols of degree $-1$. Then both $\Xbul$ and $\Xcirc$ have underlying $\Ztwo$-graded module $\bigwedge F$ and the differentials are given by
\[
...
\end{align*}
The module $R/\bold{t} R = R/(t_1,t_2)R$ is the diagonal, and we identify it with $S = \QQ[x_1,x_2]$ in the usual way. If we make the identification $R \cong S \otimes_k S$ with $x_1,x_2$ in the second tensor factor being identified with $y_1,y_2$ in $R$ then the $R$-module $R/\bold{s} R$ corresponds to a certain $S$-$S$-bimodule, which happens to be the most basic kind of \emph{Soergel bimodule}.
...
The upshot is that the maps
\begin{align*}
\Hom(1,\pi_{\circ})&: \Hom_R(\Xbul, \Xcirc) \lto \Hom_R(\Xbul, R/(\bold{t}))\\
\Hom(1,\pi_{\bullet})&: \Hom_R(\Xcirc, \Xbul) \lto \Hom_R(\Xcirc, R/(\bold{s}))
\end{align*}
are isomorphisms in the homotopy category of $\Ztwo$-graded complexes over $S$. We are now going to produce the morphism $\chi_1: \Xbul \lto \Xcirc$ as the image under the inverse of $\Hom(1,\pi_{\circ})$ of a certain natural cocycle on the right hand side, namely, the composite
...
Next we discuss the morphism $\chi_0$ stabilising \textbf{todo}. Rather than use perturbation, which is much more complicated in this case, we obtain $\chi_0$ by dualising $\chi_1$ to get a morphism $(\chi_1)\mdual: \Xcirc\mdual \lto \Xbul\mdual$. Using the standard properties of cyclic Koszul complexes collected in Appendix \ref{??}, we can rewrite the dual of a cyclic Koszul complex as a cyclic Koszul complex; up to signs and a change of variables $\Xcirc$ and $\Xbul$ are self-dual, so we get a morphism $\Xcirc \lto \Xbul$. This is our $\chi_0$.
...
\]
where each map is an isomorphism labelled by the corresponding Lemma in Appendix \ref{appendix:graded_mfs} (\textbf{todo} what to say about gradings?). The matrix factorisations $\{ \bs{a}, -\bs{t} \}$ and $\{ \bs{u}, -\bs{s} \}$ are what we would assign to the diagrams (\ref{delta2}) with the orientations of the arrows reversed. Switching the role of the $x$'s and $y$'s will therefore give us a map between $\Xbul$ and $\Xcirc$.

Let $\phi: R \lto R$ be the automorphism $\phi(x_i) = y_i$ and $\phi(y_i) = x_i$. It is clear that $\phi$ sends $t_i$ to $-t_i$, $s_i$ to $-s_i$ and $W$ to $-W$, so that by pulling back along $\phi$ (equivalently, applying $\phi$ to the entries of all matrices) we get a morphism of matrix factorisations of $W$
\[
\xymatrix{
...
\begin{lemma} The map $\chi_0: \Xcirc \lto \Xbul$ defined by (\ref{eq:chi0defn}) is the unique morphism of graded matrix factorisations of degree $2$, making the diagram
\begin{equation}
\xymatrix@C+2pc{
\Xcirc \ar[d]_{\chi_0} \ar[r]^{\pi_{\bullet}} & R/(\bold{t}) \ar[d]^-{blah}\\
\Xbul \ar[r]_{\pi_{\circ}} & R/(\bold{s})
}
\end{equation}
commute up to homotopy.
\end{lemma}
\begin{proof}
\textbf{todo} $\chi_0$ does the right thing on $1$, and uniqueness is for free.
\end{proof}
...
\xymatrix@C+2pc{
\Hom_R(X, \Xcirc) \ar[r]^{\Hom(1,\pi_{\circ})} & \Hom_R(X, R/\bold{t} R)\\
X^{\lor} \otimes \Xcirc \ar[u]^{\cong} \ar[r]_{1 \otimes \pi} & X^{\lor} \otimes R/\bold{t} R \ar@/^2pc/[l]^{\sigma_\infty} \ar[u]_{\cong}
}
\end{equation}
where $(-)^{\lor} = \Hom_R(-,R)$, tensor products are $R$-linear, and the columns are the canonical maps.

There is an $S$-linear homotopy inverse $\sigma_\infty$ of the map $1 \otimes \pi$, which comes about as follows: consider the $\Ztwo$-graded module $X^{\lor} \otimes \Xcirc$ with only the differential $1 \otimes \delta_{+}$, for which we write $(X^{\lor} \otimes \Xcirc, 1 \otimes \delta_{+})$. This is clearly quasi-isomorphic to $(X^{\lor} \otimes R/\bold{t} R, 0)$. In fact the map $\pi_{\circ}: (\Xcirc, \delta_+) \lto (R/\bold{t} R,0)$ is stronger than a quasi-isomorphism: it is an $S$-linear homotopy equivalence, and we can produce an inverse using a choice of connection. Since we can write $R$ as $S[t_1,t_2]$ there is an $S$-linear standard (\emph{explain}) flat connection
\begin{gather*}
