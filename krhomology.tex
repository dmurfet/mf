...
We thus arrive at the result that one can algorithmically compile any decorated web, i.\,e.~for arbitrary tensor products of matrix factoriations one can explicitly compute a homotopy equivalent finite-rank matrix factorisation. Note that in practice it is more efficient not to compile a web by computing and splitting the idempotent associated to the total matrix factorisation, but rather to repeatedly apply the procedure locally, in order to minimise the size of the involved ``inflated'' matrices. For this we choose a path through the underlying graph and begin by reducing the tensor product of the first two matrix factorisations to finite rank (where we can apply Theorem~\ref{theorem:compilation_main} to reduce the pushforward one variable at a time); next we compute the tensor product of the result with the next matrix factorisation in the path along the same lines, and we iterate the procedure until the whole web is compiled. 
...
% In this setting we will identify the two-term complexes $\Xcirc \lto \Xbul$ and $\Xbul \lto \Xcirc$ as representations of the autoequivalences that give rise to categorified link invariants. 
...
\end{align*}
In the notation of Section~\ref{compilewebs} we have idempotents $\idem=\vartheta\circ\psi$ and $\idem'=\vartheta'\circ\psi'$ which we can split in $\hmf(R,W)$ by finding maps $f,g$ and a matrix factorisation~$Y$ such that $g\circ f= 1_{Y}$ and $f\circ g=\idem$, and analogously for~$\idem'$. Then there is a commutative diagram
$$
\xymatrix@C+2pc{%
