...
In this section we give a description of Khovanov and Rozansky's homological link invariants. This invariant categorifies a specialisation of the Homfly polynomial, and as outlined in the previous section the categorification happens in two steps, following the two-step approach to the Homfly polynomial of \cite{moy1998}. The first step is to categorify the polynomial invariant $p_N(\Gamma)$ of state graphs $\Gamma$, by constructing a graded $\QQ$-vector space $H(\Gamma)$ whose graded Euler characteristic is $p_N(\Gamma)$, and which satisfies certain relations which lift the MOY relations~\eqref{MOYdecomp1}.
...
\[
\psi: S \lto \Hom_A(S,A) \, , \qquad \psi(1) = (x_1-x_2)^* \, , \qquad \psi(x_1-x_2) = 1^* \, .
\]
From this we deduce for any $A$-module $N$ a natural isomorphism 
\[
...
\eta': M  \lto \varphi^* \varphi_*(M) = S \otimes_A \varphi_*(M) \, , \qquad m & \longmapsto (x_1-x_2) \otimes m + 1 \otimes (x_1 - x_2) \cdot m
\end{align*}
and
\begin{align*}
\varepsilon: S \otimes_A \varphi_*(M) = \varphi^* \varphi_*(M) \lto M \, , \qquad b \otimes m & \longmapsto b \cdot m \, ,\\
\varepsilon': \varphi_*(S \otimes_A N) = \varphi_*\varphi^*(N) \lto N \, , \qquad b \otimes n & \longmapsto \psi(b)(1) \cdot n \, .
\end{align*}
...
\[
\eta: S \lto S \otimes_A S \, , \qquad b \longmapsto (x_1 - x_2) \otimes b + 1 \otimes (x_1 - x_2)b
\]
and
\[
\varepsilon: S = S \otimes_A A \lto A \, , \qquad b \longmapsto \psi(b)(1) \, .
\]
\textbf{todo} present units and counits of the right-way adjunction. The bimodule $S \otimes_A S$ is of particular interest:

\begin{lemma}\label{lemma:bimodules_as_algebras} Viewing a $\QQ[x_1,x_2,y_1,y_2]$-module as an $S$-bimodule by identifying the action of $y_1, y_2$ with the right action of $x_1, x_2$, respectively, there are isomorphisms of $S$-bimodules
\begin{align*}
...
\[
\xymatrix{%
0 \ar[r] & S \otimes_A S \ar[r] & S \ar[r] & 0 \, .
}
\]
...
\begin{align*}
& \pi_{\circ}: \Xbul = ( \bigwedge F, \beta_+ + \beta_{-}) \lto {\bigwedge}^0 F/\Im(\beta_+^0) = R/(\boldsymbol{s}) \, ,\\
& \pi_{\bullet}: \Xcirc = ( \bigwedge F, \delta_+ + \delta_{-}) \lto {\bigwedge}^0 F/\Im(\delta_+^0) = R/(\boldsymbol{t}) \, .
\end{align*}
The module $R/(\boldsymbol{t}) = R/(t_1,t_2)$ is the diagonal, and we identify it with $S = \QQ[x_1,x_2]$ in the usual way. If we make the identification $R \cong S \otimes_k S$ with $x_1,x_2$ in the second tensor factor being identified with $y_1,y_2$ in $R$ then the $R$-module $R/(\boldsymbol{s})$ corresponds to a certain $S$-$S$-bimodule, which happens to be the most basic kind of \emph{Soergel bimodule}.
...
\xymatrix{
\Xbul \ar[r]^-{\pi_{\bullet}} & R/(\boldsymbol{s}) \ar[r]^-{\can} & R/\boldsymbol{t}R
}
\end{equation}
where the second map comes about because of the inclusion of ideals $\boldsymbol{s} R \subseteq \boldsymbol{t} R$. That is, $\chi_1$ will be the unique (up to homotopy) morphism making the diagram
...
\begin{equation}
\xymatrix@C+2pc{
\Xcirc \ar[d]_{\chi_0} \ar[r]^{\pi_{\bullet}} & R/(\boldsymbol{t}) \ar[d]^-{blah}\\
\Xbul \ar[r]_{\pi_{\circ}} & R/(\boldsymbol{s})
}
\end{equation}
...
\end{align*}
where $C$ denotes a component of $L$, and $i$ is a label assigned in the planar diagram $D$ to an edge lying on this component. In this appendix we make some remarks about the relationship between reduced and unreduced homology. Let us fix the label $i$ in the following.
...
On the other hand $\redh(\Gamma)$ has nonzero contributions in both $\Ztwo$-degrees, so the reduced link homology $\overline{H}(L,C)$, as defined in \cite{kr0401268}, is honestly $(\ZZ \times \ZZ \times \Ztwo)$-graded. We write $\redh^j(\Gamma)$, respectively $\redh^j(L,C)$, for the $\Ztwo$-component in degree $j \in \Ztwo$. The point of this appendix is to check that $\redh^0(L,C)$ and $\redh^1(L,C)$ only differ by a grading shift.
...
\begin{lemma}\label{lemma:directsumalgebras} $H(\Gamma)$ is concentrated in degree $p(\Gamma)$, and as a graded $\QQ[x_i]$-module it is isomorphic to a direct sum of copies of $\QQ[x_i]/(x_i^N)$ shifted in the $\mathds{Z}$-grading, that is, for some integers $b_j$
\[
...
(TODO: refer to \eqref{MOYcatDecomp1}--\eqref{MOYcatDecomp4}, define various $\Gamma$'s) 
...
\item Let $F = R \theta_1 \oplus \ldots \oplus R \theta_n$ with $|\theta_i| = -1$ and define $\{ \bs{a}, \bs{b} \}$ in the usual way, let
\[
...
\end{gather*}
This induces an $S$-linear map, also denoted $\nabla$, on $R \otimes_{S[\boldsymbol{t}]} \Omega_{S[\boldsymbol{t}]/S}$ where $\Omega = \bigwedge^\bullet \Omega^1$. We identify this free $\mathds{Z}$-graded $R$-module with $\bigwedge F$ (and hence also with $\Xcirc$) via $\ud t_i = \theta_i$. It is easily checked that for $\theta = \theta_{i_1} \ldots \theta_{i_p}$ with distinct indices $i_j$ and $p > 0, f \in R$,
\[
...
H = (\nabla \delta_+ + \delta_+ \nabla)^{-1} \nabla
\]
on the module $\bigwedge F$. Each application of $H$ involves differentiation with respect to the $y_i$, and then solving a differential equation. Let $\sigma: S = R/(\boldsymbol{t}) \lto \bigwedge F$ be the inclusion of $k[x_1,x_2]$ into $R$. One checks that $H \delta_+ + \delta_+ H = 1 - \sigma \pi_{\Delta}$, so that $\sigma$ is exhibited by $H$ as the $S$-linear homotopy inverse to the canonical morphism $\pi_{\Delta}: (\Delta, \delta_+) \lto (R/(\boldsymbol{t}),0)$ from the Koszul complex to its cohomology.

It remains to tensor with $X^{\lor}$ and perturb in the other differentials. With only the differential $\delta_+$ turned on it is clear that $X^{\lor} \otimes \Delta$ is $S$-homotopy equivalent to $X^{\lor} \otimes R/(\boldsymbol{t})$. The actual equivalence is exhibited by extending $H$ and $\sigma$ to $S$-linear maps
\[
\xymatrix@C+2pc{
X^{\lor} \otimes \Delta \ar@(dl,dr)[]_{H} & X^{\lor} \otimes R/(\boldsymbol{t}) \ar[l]_{\sigma}
}
\]
...
\]
is an $S$-homotopy inverse to $1 \otimes \pi$. Composing with the vertical isomorphisms in (\ref{eq:cons3}) we have the desired inverse of $\Hom(1,\pi_{\Delta})$. The morphism (\ref{eq:cons4}) corresponds to the tensor $1^* \otimes 1 \in X^{\lor} \otimes R/(\boldsymbol{t})$, so to find $\chi_1$ we are left with the task of evaluating
\[
