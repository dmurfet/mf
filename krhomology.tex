...
Let $\phi: R \lto R$ be the $k$-automorphism with $\phi(x_i) = y_i$ and $\phi(y_i) = x_i$. It is clear that $\phi$ sends $t_i$ to $-t_i$, $s_i$ to $-s_i$ and $W$ to $-W$, so that by pulling back along $\phi$ (equivalently, applying $\phi$ to the entries of all matrices) we get a morphism of matrix factorisations of $W$: 
\[
\xymatrix{
...
\end{tikzpicture}
\end{minipage} 
does the job. That is, if we promote every resolution $\Gamma$ to a state web using these local assignments, and write $C(\Gamma)$ for the total matrix factorisation of the state web~$\Gamma$, then there are a series of homotopy equivalences of matrix factorisations which together give a categorified version of~\eqref{MOYdecomp1}: 
\begin{align}
\label{MOYcatDecomp1}&
C \bigg( 
\begin{tikzpicture}[scale=0.5,baseline=-2pt, inner sep=1mm, >=stealth]
