...
TODO: We thank N.~Behr, S.~Gukov, M.~Khovanov, J.~Rasmussen, R.~Rouquier, I.~Runkel, and G.~Watts. also T.~Licata, C.~Stroppel, S.~Cautis,
...
A \emph{web} $\cat{F}$ is an oriented graph with a nonempty set of vertices $V$ and a nonempty set of oriented edges $E$ where we allow edges to begin or end (but not both) on an auxiliary ``vertex'' which we call the \emph{boundary}. By convention the boundary is \emph{not} a vertex. We allow loops and multiple edges. An edge is \emph{internal} if it does not begin or end on the boundary, and \emph{external} otherwise. 
...
\[
X := \bigotimes_v \varphi_v^*(X_v) = X_{v_1} \otimes \cdots \otimes X_{v_s}\,.
\]
...
\draw[->] (0) -- (tr); 
\end{tikzpicture}
\end{minipage}. We define a decoration of this web by fixing an integer $N$ and assigning to each edge $i$ a single variable $x_{i}$ and the potential $x_{i}^{N+1}$. Next we choose two finite rank graded matrix factorisations
\[
\Xcirc, \Xbul \in \hmf( k[x,y,z,u], x^{N+1} + y^{N+1} - z^{N+1} - u^{N+1} )
\]
and make the following assignments of matrix factorisations to vertices
\begin{equation}
\label{MFcrossings}
\begin{tikzpicture}[scale=0.7,baseline, inner sep=1mm, >=stealth]
\node (1) at (-1.1,1.0) [circle,inner sep=1.5pt, draw=white,fill= white] {$x^{N+1}_{i}$};
...
\begin{example}\label{example:defectonbrane} Consider the web
$$
...
\draw[->,  thick] (D) -- (b) node[midway,above] {$f$}; 
\end{tikzpicture} 
$$
with vertices $u,v$ and edges $e,f$ which are internal and external respectively. A decoration would consist of $f$-variables $\bs{x}$, $e$-variables $\bs{y}$ and homogeneous potentials $W \in k[\bs{y}], W' \in k[\bs{x}]$ with isolated singularities together with a matrix factorisation $D$ of $W' - W$ and $Q$ of $W$. 

The total matrix factorisation $D \otimes Q$ is an object of $\hmf(k[\bs{x},\bs{y}], W')$ which can be viewed as an infinite rank matrix factorisation over $k[\bs{x}]$ of $W'$. In this way, if we were to fix $D$ and let $Q$ vary, we can view $D$ as a functor
\[
\HMF(k[\bs{y}], W) \lto \HMF(k[\bs{x}], W')\,.
\]
Alternatively, the tensor product $D \otimes Q$ can be viewed as the action of the defect $D$ (between two Landau-Ginzburg models with potentials $W, W'$) on the brane $Q$ (in the theory with potential $W$) to produce a new brane $D \otimes Q$ (in the theory with potential $W'$).
\end{example}
...
\]
such that $f \circ g = 1_Y$ and $g \circ f = e$ in $\hmf(k[\extvar], W)$. Then $f \circ \vartheta$ is a homotopy equivalence. 
\end{remark}
...
\]
That is, the polynomial $\partial_x(W_e)$ acts null-homotopically on $X_v$ with null-homotopy $\partial_x(d_v)$. But then the same is true of $X$, of which $X_v$ is a tensor factor, provided we interpret $\partial_x(d_v)$ as the map $1 \otimes \cdots \otimes \partial_x(d_v) \otimes \cdots \otimes 1$ on $X$ with suitable factors of the identity inserted, and Koszul signs.

Let $I$ denote the ideal in $k[\intvar]$ generated by the set of polynomials $\bs{t} = \{\partial_x(W_e)\}_{x,e}$ where $e$ ranges over all \emph{internal} edges in the web and $x$ over each variable lying on $e$. We refer to $J = k[\intvar]/I$ as the \emph{internal Jacobi algebra} of the decorated web. It is obviously the tensor product over $k$ of the edge Jacobi algebras
\[
...
\]
is a graded free $k[\extvar]$-module of finite rank, and consequently the quotient $X/\bs{t} X = X \otimes_{k[\bs{x}]} k[\bs{x}]/I$ is a finite rank graded matrix factorisation of $W$ over $k[\extvar]$. At this point we can apply the main result of \cite{dm1102.2957} to give an explicit idempotent
\[
\idem = \frac{1}{n!}(-1)^{\binom{n+1}{2}} \prod_x \partial_x(d_v) \circ \At(X)^n\,,
\]
such that the pair $(X/\bs{t} X\langle n \rangle, \idem)$ compiles the web\footnote{Strictly speaking this is not correct, as the pair $(X/\bs{t} X\langle n \rangle, \idem)$ will split in the category of graded matrix factorisations not to $X$ but to $X\{ p \}$ for some nonzero integer $p$. We choose not to address this point since this is not the version of the idempotent that we actually compute with. In the second approach such subtleties will be taken into account.}. Here $n$ is the total number of internal variables, the product is over all internal variables $x$ lying on an edge with origin $v$, and $\At(X)$ is the Atiyah class. For theoretical purposes (for example if one wanted to pursue a residue formula for the Euler characteristic of $X$, or address the correlators of \cite{kr0404189}) this compilation is the right one. But for implementation on a computer it is better to choose the sequence $\bs{t}$ differently, so that the connections in the construction (see below) become very simple operations on polynomials.
\\
...
\]
We also write $\lambda_x$ for the map $1 \otimes \cdots \otimes \lambda_x \otimes \cdots \otimes 1$ on $X$, which gives a null-homotopy for $x^a \cdot 1_X$. We can now apply the main theorem of \cite{dm1102.2957} to the sequence of polynomials $\intvar^a = \{ x^a \}_{x \in \intvar}$ and null-homotopies $\{ \lambda_x \}_{x \in \intvar}$. Again the formula for the idempotent will involve an Atiyah class, and to make this explicit we choose a connection on $X$. The algebra $k[\intvar]$ is a free module over subring $k[\intvar^a] = k[x_1^a, x_2^a, \ldots]$ with a basis given by monomials in the internal variables $\bs{x}^\beta$ where the degree of each variable $< a$, that is, $\beta_i < a$ for each $i$. From now on, let $\beta$ stand for such tuples of integers.
...
\]
where we write $f = \sum_\beta f_\beta \bs{x}^\beta$ for polynomials $f_\beta = f_\beta(\bs{x}^a)$ in $k[\intvar^a]$ and monomials $\bs{x}^\beta$ as above, restricted to degree $< a$ in each variable, and the sum is over all internal variables $x$. For example, if $x$ is an internal variable then
\begin{align*}
...
\begin{lemma}\label{lemma:division_woremainder} Given $f \in k[\bs{x}]$ write $f = x^a g + h$ for a polynomial $h$ of $x$-degree $< a$. Then
\[
...
\]
The \emph{Atiyah class} $\At(X) := [d_X, \nabla]$ is a $k[\extvar,\intvar^a]$-linear morphism of matrix factorisations, which up to homotopy does not depend on the choice of connection. Composing the Atiyah class with itself $n$-times, where $n$ is the number of internal variables, gives a map $X \lto X \otimes \Omega^n \cong X$ where we implicitly choose an ordering $x_1, x_2, \ldots$ of the internal variables and identify $\ud(x_1^a) \wedge \ud(x_2^a) \wedge \cdots$ with $1$. This map $\At(X)^n$ induces a $k[\extvar]$-linear map on the quotient
\[
X/\intvar^a X = X \otimes_{k[\intvar]} k[\intvar]/(\intvar^a)\,
\]
which is a finite rank graded matrix factorisation of $W$ over $k[\extvar]$. The homotopies $\lambda_x$ also give $k[\extvar]$-linear maps on this quotient, and we define the $k[\extvar]$-linear endomorphism of $X/\intvar^a X$
\begin{equation}
\idem = \frac{1}{n!}(-1)^{\binom{n+1}{2}} \prod_x \lambda_x \circ \At(X)^n\,.
\end{equation}
With this notation:
...
\xymatrix@C+3pc{
\left(X/\intvar^a X\right)\{ n(c - 2a) \}\langle n \rangle \ar@<-1ex>[r]_-{\psi} & \varphi_*(X) \ar@<-1ex>[l]_-{\vartheta}
}
\end{equation}
with $\vartheta \circ \psi = \idem$ and $\psi \circ \vartheta = 1_{\varphi_*(X)}$. That is, the pair
\[
\left(\left(X/\intvar^a X\right)\{ n(c - 2a) \}\langle n \rangle, \idem\right)
\]
compiles the decorated web $\cat{F}$.
\end{theorem}
\begin{proof}
This follows from main theorem of \cite{dm1102.2957}, with the caveat that the theorem just cited only discusses ungraded matrix factorisations. Let us justify why $\idem$ has degree zero; the same argument will show that specific maps $\vartheta$ and $\psi$ given in \emph{ibid.} both have degree zero.
...
\[
\idem = \frac{1}{n!}(-1)^{\binom{n+1}{2}} \sum_{\tau \in S_n} \textup{sgn}(\tau)\left\{ \prod_{i=1}^n \lambda_{x_i} \right\}[\partial_{\tau(1)}, d_X] \cdots [\partial_{\tau(n)}, d_X]
\]
...
Moreover it is clear that these isomorphisms can be chosen to make the diagrams of graded bimodules
\begin{equation}
\label{eq:bimodalg1}
\xymatrix{
...
\end{equation}
determines endofunctors on $\K^b( \hmf(S, W) )$ which are mutually inverse equivalences. In fact these equivalences are the ones associated by Khovanov and Rozansky to over- and under-crossings, so the whole theory is built out of (\ref{eq:bimod_cpx1}) and (\ref{eq:bimod_cpx2}). But there is an additional step before we can phrase the construction in the original terms: writing $R = S/(W)$ there is an equivalence
\[
...
We noted in Example~\ref{MOYremark} that only special choices of matrix factorisations will lead to a categorification of the MOY relations~\eqref{MOYdecomp1}. As suggested by our notation and as proved in~\cite{kr0401268}, assigning~$\Xcirc$ to
\begin{minipage}{0.48cm}
\begin{tikzpicture}[scale=0.23,baseline, inner sep=0.1mm, >=stealth]
\node (bl) at (-1,-1) [circle] {};
\node (br) at (1,-1) [circle] {};
...
In order to illustrate how our code can be used to compute Khovanov-Rozansky homology and the fusion of defects, we provide the following simple examples. (\textbf{todo} refer back to Example \ref{example:defectonbrane}).
