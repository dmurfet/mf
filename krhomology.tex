...
\[
\xymatrix{
X^0 \ar[r]^-{d_X^0} & X^1 \ar[r]^-{d_X^1} & X^1\,,
}
\]
...
Let $k$ be a field of characteristic zero and $S = k[x_1,x_2]$ and $R = k[x_1,x_2,y_1,y_2]$. The reader wanting to compare with \cite[Section 6]{kr0401268} should make the change of variables $x_1,x_2,y_1,y_2 \leftrightarrow x_4,x_3,x_1,x_2$. We consider the potential
\[
...
\[
0 \lto X \xlto{\chi_1} \Delta \lto 0.
\]
To keep the notation to a minimum, we write $X$ instead of $C(\Gamma^1)$ (the $X$ is meant to remind the reader of the wide-edge) and $\Delta$ for $C(\Gamma^0)$. Both $X, \Delta$ are Koszul factorisations associated to certain linear factorisations of $W$. Namely, if we write $t_i = y_i - x_i$ then
...
\end{equation}
Specific polynomials $u_i$ are given in \cite[p.48]{kr0401268}, and these will of course become important when we want to implement homology computations on the computer, but for the theoretical development that follows we are able to avoid these complicated polynomials: the identity (\ref{eq:cons2}) will be enough. 

In the notation of Section \ref{prelim:cyclic_koszul}, we define
\begin{align*}
X &= \{ (u_1, u_2)\, , (y_1 + y_2 - x_1 - x_2, y_1 y_2 - x_1 x_2) \}\\
\Delta &= \{ (\pi_1, \pi_2) \, , (t_1, t_2) \}\,.
\end{align*}
The model of the Koszul complex that we use is the following: set $F = R \theta_1 \oplus R \theta_2$, where the $\theta$'s are formal symbols of degree $-1$. Then both $X$ and $\Delta$ have underlying $\Ztwo$-graded module $\bigwedge F$ and the differentials are given by
\[
X = ( \bigwedge F, \beta_+ + \beta_{-}), \quad \beta_+ = \Big(\sum_i s_i \theta_i^*\Big) \neg\, (-), \quad \beta_{-} = \Big(\sum_i u_i \theta_i\Big) \wedge (-).
\]
where $s_1 = y_1 + y_2 - x_1 - x_2$ and $s_2 = y_1 y_2 - x_1 x_2$, and
\[
\Delta = ( \bigwedge F, \delta_+ + \delta_{-}), \quad \delta_+ = \Big(\sum_i t_i \theta_i^*\Big) \neg\, (-), \quad \delta_{-} = \Big(\sum_i \pi_i \theta_i\Big) \wedge (-).
\]
...
\begin{align*}
\pi_X: X = ( \bigwedge F, \beta_+ + \beta_{-}) \lto \bigwedge^0 F/\Im(\beta_+^0) = R/\bold{s} R,\\
\pi_\Delta: \Delta = ( \bigwedge F, \delta_+ + \delta_{-}) \lto \bigwedge^0 F/\Im(\delta_+^0) = R/\bold{t} R.
\end{align*}
...
The $\pi$ morphisms are, as we will soon see, universal in a certain sense: they exhibit $X, \Delta$ as the \emph{stabilisations} of the Soergel bimodule and the diagonal, respectively. To prove this we make use of the perturbation lemma (\emph{details}, in appendoix).
\\

The upshot is that the map
\[
\Hom(1,\pi_\Delta): \Hom_R(X, \Delta) \lto \Hom_R(X, R/\bold{t} R)
\]
is an isomorphism in the homotopy category of $\Ztwo$-graded complexes over $S$. We are now going to produce the morphism $\chi_1: X \lto \Delta$ as the image under the inverse of $\Hom(1,\pi_{\Delta})$ of a certain natural cocycle on the right hand side, namely, the composite
\begin{equation}\label{eq:cons4}
X \xlto{\pi_X} R/\bold{s} R \xlto{\can} R/\bold{t}R
\end{equation}
...
\xymatrix@C+2pc{
X \ar[d]_{\chi_1} \ar[r]^{\pi_X} & R/\bold{s} R \ar[d]^{\can}\\
\Delta \ar[r]_{\pi_{\Delta}} & R/\bold{t} R
}
\end{equation}
...
\end{equation}
has a unique polynomial solution $\Omega_2(\gamma)$, and the map $\chi_1: X \lto \Delta$ defined by
\begin{align*}
...
\[
\chi^{even}_1 = \begin{pmatrix} 1 & 0 \\ -a_2 & \frac{1}{2}(x_1+y_1-x_2-y_2)\end{pmatrix}, \qquad \chi^{odd}_1 = \begin{pmatrix} 1 & \frac{1}{2}(x_2+y_2) \\ 1 & \frac{1}{2}(x_1+y_1) \end{pmatrix}.
\]
...
\begin{remark} If $a,b \in R$ are homogeneous with $\deg(a) + \deg(b) = 2c$ then it is clear that there is an isomorphism $\{ b, a \}\mdual \cong \{-a, b\}$, and if we think of these cyclic Koszul factorisations as exterior algebras on a symbol $\theta$, this isomorphism sends the basis element $1^*$ to $1$ and $\theta^*$ to $\theta$.
\end{remark}
...
For the next pair of lemmas let $R = \oplus_{i \ge 0} R_i$ be a graded ring, and $\bs{a}, \bs{b}$ sequences of $n$ homogeneous elements in $R$ with $\deg(a_i) + \deg(b_i) = 2c$. If $\bs{a} = (a_1,\ldots,a_n)$ then $- \bs{a}$ denotes $(-a_1, \ldots, -a_n)$.

\begin{lemma} There is a canonical isomorphism of graded matrix factorisations
\[
\{ \bs{b}, \bs{a} \}\mdual \cong \{ -\bs{a}, \bs{b} \}.
\]
\end{lemma}
\begin{proof}
\textbf{todo}.
...
Finally, let us discuss the cohomology of cyclic Koszul complexes (\textbf{todo, fix}). We begin more generally: let $C = \oplus_{i \in \mathbb{Z}} C^i$ be a $\mathbb{Z}$-graded $R$-module together with $R$-linear maps $d_{\pm}: C \lto C$ of degree $\pm 1$, satisfying
\[
