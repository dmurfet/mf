...
\usepackage[all]{xy}
\usepackage{tikz}
\def\nicecolourscheme{\shadedraw[top color=blue!0, bottom color=blue!20]}
\def\nicecolourschemedash{\shadedraw[top color=blue!0, bottom color=blue!20, draw=blue!50!black, dashed]}
...
\def\sln{\operatorname{\mathfrak{sl}}(N)}
\def\nN{\mathds{N}}
\def\nZ{\mathds{Z}}
\def\nQ{\mathds{Q}}
\def\nR{\mathds{R}}
...
\newcommand{\homn}[2]{[#1,#2]_{\mathbb{N}}}
...
We are now in a position to conclude our reminder of the construction of~\cite{kr0401268}. To each crossing in a link diagram of a link~$L$ one associates the appropriate complex in~\eqref{resolutions}, and then one takes the tensor product of all these complexes to produce the total complex $C^{\bullet,\bullet}_N(L)$. The bigrading is induced by the cohomological grading of the two-term complexes~\eqref{resolutions} and the internal grading of the matrix factorisations involved. The main result of~\cite{kr0401268} can now be summarised by the fact that the cohomology $H^{\bullet,\bullet}_N(L)$ of $C^{\bullet,\bullet}_N(L)$ is an invariant of~$L$ which is conveniently encoded in the Poincar\'e polynomial
$$
\KR_{N}(L) = \sum_{i,j\in\nZ} t^{i} q^j \dim_{\nQ}(H^{i,j}(L)) \, . 
$$
...
The remaining part of the present paper is organised as follows. TODO


\begin{acknowledgements}
We thank M.~Aganagic, N.~Behr, S.~Gukov, M.~Khovanov, J.~Rasmussen, and G.~Watts. 
TODO: add more
...
\[
\Hom_{gr}(X,Y) = \bigoplus_{i \in \mathbb{Z}} \Hom_R(X,Y)_i
\]
where $\Hom_R(X,Y)_i$ denotes the group of all $R$-linear maps $X \lto Y$ of degree $i$. This is a submodule of $\Hom_R(X,Y)$, and if $X$ is finitely generated then $\Hom_{gr}(X,Y) = \Hom_R(X,Y)$, in which case we use the latter notation. This graded module has a natural $\Ztwo$-decomposition into even and odd maps
\begin{align*}
\Hom_{gr}(X,Y) &= \left( \Hom_{gr}(X^0, Y^0) \oplus \Hom_{gr}(X^1, Y^1) \right)\\
&\qquad \oplus \left( \Hom_{gr}(X^0, Y^1) \oplus \Hom_{gr}(X^1, Y^0) \right)\,.
\end{align*}
Together with the differential $d$ defined for an odd or even map $\alpha$ (with $|\alpha|$ denoting this $\Ztwo$-degree)
\[
d(\alpha) = d_Y \circ \alpha - (-1)^{|\alpha|} \alpha \circ d_X,
\]
the graded module $\Hom_{gr}(X,Y)$ is a graded linear factorisation of $W' - W$. In particular if $W = W'$ we have a $(\ZZ \times \Ztwo)$-graded complex. This construction is functorial, in the sense that if $\varphi: Y \lto Y'$ is a morphism of graded linear factorisations of degree $e$, then $\Hom_{gr}(X,\varphi)$ is another morphism of degree $e$, and similarly in the first variable.
...
Let us recall the definition of the cyclic Koszul complex from \cite[\S 2]{kr0401268}. Let $R = \oplus_{i \ge 0} R_i$ be a graded ring. If $a,b$ are homogeneous with $\deg(a) + \deg(b) = 2c$ then $\{ a, b \}$ denotes the graded matrix factorisation of $ab$ given by
\[
R \xlto{a} R\{ c - \deg(a) \} \xlto{b} R.
\]
For two sequences $\bs{a} = (a_1,\ldots,a_n)$ and $\bs{b} = (b_1, \ldots, b_n)$ of homogeneous elements in $R$ satisfying $\deg(a_i) + \deg(b_i) = 2c$ for each $i$, we introduce the following notation for the tensor product
\[
\{ \bs{a}, \bs{b} \} := \{a_1,b_1\} \otimes \cdots \otimes \{a_m, b_m\}
\]
which is a graded matrix factorisation of $\sum_i a_i b_i \in R_{2c}$. A better way to present the differential on this tensor product is to introduce formal symbols $\theta_i$ of bidegree $(-1, \deg(a_i) - s)$, so that when we write $R \theta_i$ we mean the $R\{ \deg(a_i) - s \}$ placed in cohomological degree $-1$. The graded free module $F = \oplus_i R\theta_i$ and its exterior algebra $\bigwedge F$ acquire both a $\ZZ$-grading from these cohomological degrees, and a grading in the usual sense. We define differentials $\delta_{\pm}$ on $\bigwedge F$ of bidegree $(\pm 1,c)$ by
\[
\delta_+ = \Big(\sum_i b_i \theta_i^*\Big) \neg\, (-) \quad \delta_{-} = \Big(\sum_i a_i \theta_i\Big) \wedge (-)\,.
\]
...
\end{tikzpicture}
\end{minipage}.} 
From a link diagram with~$m$ crossings we thus produce $2^m$ oriented planar graphs~$\Gamma$ whose vertices are either bivalent with one incoming and one outgoing edge, or four-valent of type 
 \begin{minipage}{0.48cm}
\begin{tikzpicture}[scale=0.23,baseline, inner sep=0.1mm, >=stealth]
\node (0) at (0,0) [circle,inner sep=0.7pt, thin,draw=black,fill= black] {};
\node (bl) at (-1,-1) [circle] {};
...
\draw[->] (0) -- (tr); 
\end{tikzpicture}
\end{minipage}. If~$\Gamma$ has~$m$ edges and we label each edge by distinct integers $1,\ldots,m$, then we call it a \emph{state graph}. To each state graph~$\Gamma$ the construction of~\cite{moy1998} associates a Laurent polynomial $p_{N}(\Gamma)\in\nZ[q^{\pm 1}]$ that is uniquely determined by its value $[N]=(q^N-q^{-N})(q-q^{-1})^{-1}$ for an oriented circle, and the skein relations
\begin{align}
& 
p_{N} \Bigg( 
...
In the second step, the link invariant $P_{N}(L)$ is obtained as follows. To produce the $2^m$ graphs $\Gamma_{j}$, each crossing in the link diagram is resolved according to
\begin{equation}
...
TODO: add example calculation here? 
...
\begin{remark}
\label{MOYremark}
Note that we do not specify the matrix factorisations $\Xcirc$ and $\Xbul$ in the definition of a state web. However, in order for a state web to categorify the MOY relations, i.\,e.~for identities analogous to~\eqref{MOYdecomp1} to hold between compilations of state webs, $\Xcirc$ and $\Xbul$ need to be carefully chosen. This was done in~\cite{kr0401268} and we will give the expressions for $\Xcirc$ and $\Xbul$ in TODO below; see also~\eqref{MOYcatDecomp1}--\eqref{MOYcatDecomp4}. 
\end{remark}
...
\]
where we order the variables according to the edge ordering. Given a tuple $\alpha \in \mathbb{N}^\intvar$ we write $\phi^\alpha$ for the corresponding monomial $\phi_1^{\alpha_1} \ldots \phi_\intvar^{\alpha_\intvar}$. In (\ref{eq:defnqsweb}) we produced a collection of integers $\{q_e\}_{e \in E_{\text{int}}}$, and a $k$-basis for~$J$ is formed by the monomials with sufficiently small degrees in each variable. Precisely, for $\alpha \in \mathbb{N}^\intvar$ we write $\alpha < q$ if for each internal edge $e$ and each $i\in\{1,\ldots,n_{e}\}$ the integer associated to the variable $x_{e,i}$ by $\alpha$ is smaller than $ q_e$. Then it is clear that $\{ x^\alpha \}_{\alpha < q}$ forms a $k$-basis for~$J$, and we let $\pi: R_{\text{int}} \lto J$ be the projection and denote by
\begin{equation}
...
\begin{equation}
P = \sum_{\alpha \in \mathbb{N}^\intvar} P_\alpha \cdot \phi^\alpha
\end{equation}
...
$$
\delta_{\phi_i}(P) = \sum_{\alpha \in \mathbb{N}^\intvar} P_\alpha \otimes \pi [g_i, \phi^\alpha] \sigma \, ,
$$
...
In this section we complete the description of Khovanov and Rozansky's homological link invariant, begun in Section \ref{??}. This invariant categorifies a specialisation of the HOMFLY polynomial, and as outlined earlier the categorification happens in two steps, following the two-step approach to the HOMFLY polynomial of \cite{moy1998}. The first step is to categorify the polynomial invariant $p_N(\Gamma)$ of state graphs $\Gamma$, by constructing a graded $\QQ$-vector space $H(\Gamma)$ whose graded Euler characteristic is $p_N(\Gamma)$, and which satisfies certain relations which lift the MOY relations (\ref{??}).

This is done by making good on the promise made in Remark \ref{??}. A state graph makes a choice, for each crossing in our link, of one of two types of resolution, and we have to explain how to choose for each type of resolution a corresponding matrix factorisation (denoted here $X_{\circ}, X_{\bullet}$). Tensoring these together over all crossings will produce a $\ZZ \times \Ztwo$-graded complex $C(\Gamma)$ whose homology $H(\Gamma)$ happens to be concentrated in one $\Ztwo$-degree, so that we have the desired graded $\QQ$-vector space.

The second step is to produce maps $X_{\circ} \lto X_{\bullet}$ and $X_{\bullet} \lto X_{\circ}$. These morphisms are viewed as two-term complexes of matrix factorisations, categorifying the differences on the right hand side of the identities in (\ref{??}), and we thereby define the homological invariant $\textup{KR}_N(L)$ which categorifies $P_N(L)$. Our point of view is that these two-term complexes represent autoequivalences of a certain triangulated category, corresponding to the over and under crossings.
...
Let $k$ be a field of characteristic zero and consider the inclusion
\[
\varphi: A = k[x_1 + x_2, x_1 x_2] \lto k[x_1,x_2] = B
\]
of symmetric polynomials in all polynomials. As with any ring morphism, there are adjunctions
\[
\xymatrix@C+3pc{
\Modd(A) \ar@/^1pc/[r]^-{\varphi^*} \ar@/_1pc/[r]_-{\varphi^!} & \Modd(B) \ar[l]^-{\varphi_*}
}, \qquad
\xymatrix{
\varphi^* \ar@{-|}[r] & \varphi_* \ar@{-|}[r] & \varphi^!
}
\]
between restriction of scalars $\varphi_*$, extension of scalars $\varphi^* = - \otimes_A B$ and $\varphi^! = \Hom_A(B, -)$. Now any polynomial $f \in B$ be written uniquely as $f_1 + f_2(x_1-x_2)$ for symmetric polynomials $f_i$, so $B$ is a free $A$-module on the basis $\{1, x_1 - x_2\}$ and we write $1^*, (x_1-x_2)^*$ for the corresponding dual basis of $\Hom_A(B,A)$ as an $A$-module. The important point is that there is an isomorphism of $B$-modules
\[
\psi: B \lto \Hom_A(B,A), \quad \psi(1) = (x_1-x_2)^*, \; \psi(x_1-x_2) = 1^*.
\]
From this we deduce a natural isomorphism for any $A$-module $N$
\[
\varphi^!(N) = \Hom_A(B,N) \cong \Hom_A(B,A) \otimes_A N \stackrel{\psi}{\cong} B \otimes_A N = \varphi^*(N),
\]
so that the left and right adjoints of $\varphi_*$ are naturally isomorphic. One way to look at this is that by allowing us to identify $\varphi^*$ with $\varphi^!$, the map $\psi$ has provided us with an adjunction $\xymatrix@1{\varphi_* \ar@{-|}[r] & \varphi^*}$, so that these functors are biadjoint. By a process of stabilisation this biadjunction will give rise to a biadjunction on the level of matrix factorisations, which is the starting point for the Khovanov-Rozansky construction.

These morphisms will come from the units and counits of the adjunction $\xymatrix@1{\varphi^* \ar@{-|}[r] & \varphi_*}$, which we denote $\eta, \varepsilon$, and the units and counits of the new adjunction $\xymatrix@1{\varphi_* \ar@{-|}[r] & \varphi^*}$, which we denote $\eta', \varepsilon'$. For an $A$-module $N$ and $B$-module $M$ these maps are given by
\begin{align*}
\eta: N \lto \varphi_* \varphi^*(N) = \varphi_*( B \otimes_A N ), \qquad &n \mapsto 1 \otimes n,\\
\eta': M \lto \varphi^* \varphi_*(M) = B \otimes_A \varphi_*(M), \qquad &m \mapsto (x_1-x_2) \otimes m + 1 \otimes (x_1 - x_2) \cdot m
\end{align*}
and
\begin{align*}
\varepsilon: B \otimes_A \varphi_*(M) = \varphi^* \varphi_*(M) \to M, \qquad &b \otimes m \mapsto b \cdot m,\\
\varepsilon': \varphi_*(B \otimes_A N) = \varphi_*\varphi^*(N) \lto N, \qquad &b \otimes n \mapsto \psi(b)(1) \cdot n.
\end{align*}
To make the transition from modules to matrix factorisations, we present the biadjunction between $\varphi^*$ and $\varphi_*$ in terms of bimodules and bimodule morphisms. It is clear that $\varphi^*$ is represented by tensoring with the $B$-$A$-bimodule ${}_B B_A$, and likewise $\varphi_*$ is represented by the $A$-$B$-bimodule ${}_A B_B$. Hence the composite $\varphi^* \varphi_*$ is represented by ${}_B B \otimes_A B {}_B$ and $\varphi_* \varphi^*$ by ${}_A B {}_A$. The units and counits of adjunction are represented by morphisms between these bimodules and the rings $A,B$ as bimodules over themselves, representing the identity functors:
\[
\eta: B \lto B \otimes_A B, \qquad b \mapsto (x_1 - x_2) \otimes b + 1 \otimes (x_1 - x_2)b
\]
and
\[
\varepsilon: B = B \otimes_A A \lto A, \qquad b \mapsto \psi(b)(1).
\]
\textbf{todo} present units and counits of the right-way adjunction. The bimodule $B \otimes_A B$ is of particular interest: 

\begin{lemma} Viewing a $k[x_1,x_2,x_3,x_4]$-module as a $B$-bimodule by identifying the action of $x_3, x_4$ with the right action of $x_1, x_2$ respectively, there are isomorphisms of $B$-bimodules
\[
B \otimes_A B \cong k[x_1,x_2,x_3,x_4]/(x_1 + x_2 - x_3 - x_4, x_1 x_2 - x_3 x_4)
\]
and
\[
B \cong k[x_1,x_2,x_3,x_4]/(x_1 - x_3, x_2 - x_4).
\]
\end{lemma}

If $W \in A$ is any symmetric polynomial then, since $B$ is free as an $A$-module with basis $\{1, x_1 - x_2\}$, these functors lift to an adjoint pair of functors between the triangulated categories of matrix factorisations
\[
\xymatrix@C+2pc{
\hmf(A, W) \ar@<1ex>[r]^{\varphi^*} & \hmf(B, W) \ar@<1ex>[l]^{\varphi_*}.
}
\]
Let us fix the polynomial $W = x_1^{n+1} + x_2^{n+1}$. The general theory of \cite{??} applies (\emph{details}) to produce an interesting autoequivalence of $\mathbb{K}^b( \hmf(B, W) )$, given by tensoring with the short complex (or something like this)
\[
0 \lto B \otimes_A B \lto B \lto 0.
\]
This is the autoequivalence we want to associate to an undercrossing of our link. Next we are going to replace this short complex of bimodules by a short complex of matrix factorisations. The intuition is the following: there is an equivalence
\[
\hmf(B, W) \cong \qderu{b}{\modd B}/\K^b(\modd B)
\]
which says that a matrix factorisation of $W$ remembers the asymptotic part of a free resolution of a $B/W$-module. In this sense $B \otimes_A B, B$, which are both clearly $B/W$-modules, contain far too much information: the action of the corresponding functors on the homotopy category of matrix factorisations will only depend on the asymptotic part of the resolution of $B \otimes_A B, B$ as $B/W$-modules. Extracting this asymptotic part is the process known as \emph{stabilisation}, and will take place in the next section. We will stabilise $B \otimes_A B$ to a matrix factorisation $X_{\bullet}$ and $B$ to $X_{\circ}$. The morphisms $??$ lift to the free resolutions of these modules and thus to the asymptotic part, which means that these maps stabilise to morphisms between $X_{\circ}$ and $X_{\bullet}$, which are the $\chi$-morphisms of \cite{kr0401268}.

We can view the stabilisation as a kind of truncation of the bimodule picture elaborated above, and in fact these truncations together form a kind of filtration as we vary the integer $N$. Indeed if one proceeds in the above line, one will end up constructing the triply graded Khovanov-Rozansky homology of \cite{??}, and the relationship between this homology and the truncations at level $N$ (the $sl(N)$-homologies) is encoded by the spectral sequence of Rasmussen \cite{??}.

\subsection{Stabilisation}

Earlier we explained how one constructs the Khovanov-Rozansky complex by stabilising an adjunction on the level of module categories. In this section we present the explicit matrix factorisations and morphisms which stabilise this adjunction, following \cite{kr0401268}. The presentation of the matrix factorisations is standard, but we give a new approach to stabilisation of the morphisms by inverting a certain canonical quasi-isomorphism. Our construction is no better for computations: we simply recover the explicit maps of \cite{kr0401268}, but we feel it is worth including here since the \emph{ad hoc} nature of the maps in the originl paper is a major turn off for newbies (\emph{rephrase}).

Let us begin with a general definition.

\begin{definition} Let $R$ be a ring, $W \in R$ and $M$ a finitely generated $R/(W)$-module. We define the \emph{stabilisation} of $M$ with respect to $W$ to be a finite rank matrix factorisation $X_M \in \hmf(R,W)$ together with a morphism of linear factorisations $\pi: X_M \lto M$ with the property that the map
\[
\Hom_R(Y, X_M) \lto \Hom_R(Y, M)
\]
is a quasi-isomorphism for every finite rank matrix factorisation $Y$ of $W$.
\end{definition}

\textbf{todo}: mention the equivalence of triangulated categories, equivalent to the above definition of stabilisation.

\begin{proposition} Under some conditions $\{ \bs{a}, \bs{b} \}$ stabilises $R/(\bs{b})$.
\end{proposition}
\begin{proof}
See appendix.
\end{proof}

\textbf{todo} The example we are most interested in: stabilising the bimodules of the previous section.
\\

Let $S = \QQ[x_1,x_2]$ and $R = \QQ[x_1,x_2,y_1,y_2]$. The reader wanting to compare with \cite[Section 6]{kr0401268} should make the change of variables $x_1,x_2,y_1,y_2 \leftrightarrow x_4,x_3,x_1,x_2$. We consider the potential
\[
W = y_1^{n+1} + y_2^{n+1} - x_1^{n+1} - x_2^{n+1}
\]

Both $\Xbul, \Xcirc$ are cyclic Koszul complexes. Namely, if we write $t_i = y_i - x_i$ then
...
\Xcirc &= \{ (\pi_1, \pi_2) \, , (t_1, t_2) \}\,.
\end{align*}
Of course there is some indeterminacy in the choice of the $u_i$'s, but up to homotopy equivalence the factorisation $\Xbul$ does not depend on the choice of $u_i$'s (this is a standard fact, see for example \cite{??}, and will also be clear from our description of $\Xbul$ as the solution of a universal problem). Moreover, we can and do assume that the polynomials $u_i$ are invariant under interchanging the $x$'s with the $y$'s, that is, we arrange that $u_i(y_1,y_2,x_1,x_2) = u_i(x_1,x_2,y_1,y_2)$ for $i = 1,2$ (see Appendix \ref{??}). We will manage to avoid discussing the explicit form of these polynomials, making use only of (\ref{eq:cons2}) and the symmetry just discussed.


The model of the Koszul complex that we use is the following: set $F = R \theta_1 \oplus R \theta_2$, where the $\theta$'s are formal symbols of degree $-1$. Then both $\Xbul$ and $\Xcirc$ have underlying $\Ztwo$-graded module $\bigwedge F$ and the differentials are given by
\[
\Xbul = ( \bigwedge F, \beta_+ + \beta_{-}), \quad \beta_+ = \Big(\sum_i s_i \theta_i^*\Big) \neg\, (-), \quad \beta_{-} = \Big(\sum_i u_i \theta_i\Big) \wedge (-).
\]
where $s_1 = y_1 + y_2 - x_1 - x_2$ and $s_2 = y_1 y_2 - x_1 x_2$, and
\[
\Xcirc = ( \bigwedge F, \delta_+ + \delta_{-}), \quad \delta_+ = \Big(\sum_i t_i \theta_i^*\Big) \neg\, (-), \quad \delta_{-} = \Big(\sum_i \pi_i \theta_i\Big) \wedge (-).
\]
By construction these are both matrix factorisations of $W$ over $R$. In the $\mathbb{Z}$-grading on $\bigwedge F$ the differentials with subscripts $\pm$ increase/decrease degree, so it is clear that there are morphisms of linear factorisations (viewing modules as factorisations of zero)
\begin{align*}
\pi_{\circ}: \Xbul = ( \bigwedge F, \beta_+ + \beta_{-}) \lto \bigwedge^0 F/\Im(\beta_+^0) = R/\bold{s} R,\\
\pi_{\bullet}: \Xcirc = ( \bigwedge F, \delta_+ + \delta_{-}) \lto \bigwedge^0 F/\Im(\delta_+^0) = R/\bold{t} R.
\end{align*}
The module $R/\bold{t} R = R/(t_1,t_2)R$ is the diagonal, and we identify it with $S = k[x_1,x_2]$ in the usual way. If we make the identification $R \cong S \otimes_k S$ with $x_1,x_2$ in the second tensor factor being identified with $y_1,y_2$ in $R$ then the $R$-module $R/\bold{s} R$ corresponds to a certain $S$-$S$-bimodule, which happens to be the most basic kind of \emph{Soergel bimodule}.

The $\pi$ morphisms are, as we will soon see, universal in a certain sense: they exhibit $\Xbul, \Xcirc$ as the \emph{stabilisations} of the Soergel bimodule and the diagonal, respectively. To prove this we make use of the perturbation lemma (\emph{details}, in appendoix).
\\

which Khovanov and Rozansky associate to a crossing labeled as in the diagram
\[
\begin{tikzpicture}[scale=0.7,baseline, inner sep=1mm, >=stealth]
\node (0) at (0,0) [circle,inner sep=1.5pt, draw=white,fill= white] {};
\node (bl) at (-1,-1) [circle,draw=white,fill= white] {$x_1$};
\node (br) at (1,-1) [circle,draw=white,fill= white] {$x_2$};
\node (tl) at (-1,1) [circle,draw=white,fill= white] {$y_1$};
\node (tr) at (1,1) [circle,draw=white,fill= white] {$y_2$};
\draw[->, thick] (bl) -- (tr); 
\draw[-, thick] (br) -- (0); 
\draw[->, thick] (0) -- (tl); 
\end{tikzpicture}
\]
In the convention of \cite{kr0401268} this is a negative crossing, and we are going to attach to this crossing a complex of matrix factorisations
\[
\xymatrix{
0 \ar[r] & \Xbul \ar[r]^{\chi_1} & \Xcirc \ar[r] & 0\,.
}
\]

The upshot is that the map
\[
\Hom(1,\pi_{\circ}): \Hom_R(\Xbul, \Xcirc) \lto \Hom_R(\Xbul, R/\bold{t} R)
\]
is an isomorphism in the homotopy category of $\Ztwo$-graded complexes over $S$. We are now going to produce the morphism $\chi_1: \Xbul \lto \Xcirc$ as the image under the inverse of $\Hom(1,\pi_{\circ})$ of a certain natural cocycle on the right hand side, namely, the composite
\begin{equation}\label{eq:cons4}
\xymatrix{
\Xbul \ar[r]^-{\pi_{\bullet}} & R/\bold{s} R \ar[r]^-{\can} & R/\bold{t}R
}
\end{equation}
where the second map comes about because of the inclusion of ideals $\bold{s} R \subseteq \bold{t} R$. That is, $\chi_1$ will be the unique (up to homotopy) morphism making the diagram
\begin{equation}\label{eq:cons5}
\xymatrix@C+2pc{
\Xbul \ar[d]_{\chi_1} \ar[r]^{\pi_{\bullet}} & R/\bold{s} R \ar[d]^{\can}\\
\Xcirc \ar[r]_{\pi_{\circ}} & R/\bold{t} R
}
\end{equation}
commute (we might say $\chi_1$ is the stabilisation of the canonical map between the quotient algebras).
...
We give a more conceptual derivation by finding an explicit inverse of $\Hom(1,\pi_{\Delta})$ and evaluating it on (\ref{eq:cons4}). Besides avoiding the need for explicit polynomials $u_i$ in the foundations, our approach has the advantage that it can be adapted to stabilise other morphisms into the diagonal.
...
\]
\begin{lemma}\label{lemma:consfirst} The partial differential equation in an unknown $z$
\begin{equation}\label{eq:consfirst1}
2 z + \frac{\partial z}{\partial y_1} (y_1 - x_1) + \frac{\partial z}{\partial y_2}(y_2 - x_2) = \gamma
\end{equation}
has a unique polynomial solution $\Omega_2(\gamma)$, and the map $\chi_1: \Xbul \lto \Xcirc$ defined by
\begin{align*}
...
\end{align*}
is a morphism of matrix factorisations, making the diagram (\ref{eq:cons5}) commute up to homotopy.
\end{lemma}
...
\]
These agree with the matrices $V_0,V_1$ of \cite[p.51]{kr0401268} for $\lambda = \frac{1}{2}$, up to a sign which arises because they write their differentials and maps in the basis $1, \theta_2 \theta_1 = - \theta_1 \theta_2, \theta_1, \theta_2$.
\\

Next to the positive crossing
\[
\begin{tikzpicture}[scale=0.7,baseline, inner sep=1mm, >=stealth]
\node (0) at (0,0) [circle,inner sep=1.5pt, draw=white,fill= white] {};
\node (bl) at (-1,-1) [circle,draw=white,fill= white] {$x_1$};
...
\end{tikzpicture}
\]
we are going to attach to the complex of matrix factorisations
\[
\xymatrix{
0 \ar[r] & \Xcirc \ar[r]^{\chi_0} & \Xbul \ar[r] & 0\,,
}
\]
where $\chi_0$ is the morphism stabilising \textbf{todo}. Rather than use perturbation, which is much more complicated in this case, we obtain $\chi_0$ by dualising $\chi_1$ to get a morphism $(\chi_1)\mdual: \Xcirc\mdual \lto \Xbul\mdual$. Using the standard properties of cyclic Koszul complexes collected in Appendix \ref{??}, we can rewrite the dual of a cyclic Koszul complex as a cyclic Koszul complex; up to signs and a change of variables $\Xcirc$ and $\Xbul$ are self-dual, so we get a morphism $\Xcirc \lto \Xbul$. This is our $\chi_0$.

More carefully, we define $\chi_0'$ to be the morphism making the diagram below commute:
\[
\xymatrix@C+2pc{
\Xcirc\mdual \ar[r]^-{\chi_1\mdual} & \Xbul\mdual\\
\{ \bs{a}, \bs{t} \}\mdual & \{ \bs{u}, \bs{s} \}\mdual\\
\{ -\bs{t}, \bs{a} \} & \{ -\bs{s}, \bs{u} \}\\
\{ -\bs{a}, \bs{t} \} & \{ -\bs{u}, \bs{s} \}\\
\{ \bs{a}, -\bs{t} \} & \{ \bs{u}, -\bs{s} \}
}
...
We will now present some results for $\sln$ Khovanov-Rozansky homology of all prime links with up to~6 crossings. The explicit Poicar\'e polynomials are listed in Tables~\ref{unreducedKR} and~\ref{reducedKR}. 
...
Let $R = \oplus_{i \ge 0} R_i$ be a graded ring. If $X$ is a finite rank graded matrix factorisation of $W \in R_{2c}$ over $R$ then the \emph{dual factorisation} is $X\mdual = \Hom_R(X,R)$, which factorises $-W$. More explicitly, the dual factorisation is the pair
\[
\xymatrix@C+3pc{
...
\begin{lemma} Given $W, W' \in R_{2c}$ and a finite rank graded matrix factorisation $X$ of $W$ and a graded linear factorisation $Y$ of $W'$, there is a natural isomorphism
\[
...
\[
\Hom_{gr}(X \otimes Y, Z) \lto \Hom_{gr}(X, \Hom_{gr}(Y,Z)).
\]
\end{lemma}
% see NGMF
...
\[
\{ - \bs{a}, \bs{b} \} \cong \{ -\bs{b}, \bs{a} \}\langle n \rangle \left\{ \sum_i \deg(a_i) - nc \right\}.
\]
\end{lemma}
\begin{proof}
...
\]
defined by $\theta_i \mapsto - \theta_i$.
\end{lemma}

Finally, let us discuss the cohomology of cyclic Koszul complexes (\textbf{todo, fix}). Let $R$ be any ring. We begin more generally: let $C = \oplus_{i \in \mathbb{Z}} C^i$ be a $\mathbb{Z}$-graded $R$-module together with $R$-linear maps $d_{\pm}: C \lto C$ of degree $\pm 1$, satisfying
\[
(d_+)^2 = (d_{-})^2 = 0, \qquad \text{ and } \qquad d_{+}d_{-} + d_{-}d_{+} = 0.
\]
Let $C_{\Ztwo}$ denote the $\Ztwo$-folding, which is a $\Ztwo$-graded complex with different $d_{tot} = d_+ + d_{-}$. If we replace $d_{+}$ by $-d_{+}$ then we have a second complex $(C_{\Ztwo}, -d_{+} + d_{-})$. We claim that these two complexes have the same $\Ztwo$-graded cohomology: this is the cyclic version of the obvious fact that multiplying the differentials on the ordinary Koszul complex by $-1$ does not change the cohomology. Recall that given a $\Ztwo$-graded complex $(X,d)$ we denote by $(X,d)_{-}$ the complex
\[
X^0 \xlto{-d^0} X^1 \xlto{d^1} X^0.
\]
...
In particular for sequences $\bs{a}, \bs{b}$ we have $H(\{ \bs{a}, \bs{b} \}) \cong H(\{\bs{a}, -\bs{b}\})$.

\section{The relation between reduced and unreduced homology}

As was discussed in Section \ref{??}, to a state graph $\Gamma$ with $m$ edges Khovanov and Rozansky assign a $\Ztwo$-graded complex $\krc(\Gamma)$ over the polynomial ring $R = \QQ[\boldsymbol{x}]$ in the edge variables $\boldsymbol{x} = \{x_1,\ldots,x_m\}$, defined by tensoring together local matrix factorisations assigned to the singular crossings and marks of the state graph. The definition depends on a choice of integer $N > 1$, which we fix throughout and omit from the notation, so that all our homologies are $\sln$-homologies.
...
\overline{H}(L, C) &= H\big( H( \krc(D) \otimes_{R} R/(x_i), \diffm), \diffh \big),
\end{align*}
where $C$ denotes a component of $L$, and $i$ is a label assigned in the planar diagram $D$ to an edge lying on this component. In this section we make some remarks about the relationship between reduced and unreduced homology. Let us fix the label $i$ in the following.

After taking the $\diffm$-cohomology everything becomes finite-dimensional, and taking the cohomology with respect to the differential $\diffh$ is trivial; the whole problem with computing these invariants is computing $H(\krc(D), \diffm)$. In this article the emphasis is on computations, and in this context there is a big difference between taking cohomology before and after setting $x_i = 0$. Since $H(\krc(D), \diffm)$ is a direct sum of modules $H(\krc(\Gamma), \diffm)$ for various state graphs $\Gamma$ let us speak only of state graphs, in which case we are making the distinction between
\[
...
\end{align*}
These are both finite-dimensional $\mathbb{Z} \times \Ztwo$-graded vector spaces. It is shown in \cite{kr0401268} that $H(\Gamma)$ is concentrated in only one $\Ztwo$-degree, namely the degree of the \emph{parity} $p = p(\Gamma)$ defined by erasing all wide edges in $\Gamma$ and counting the number of circles in the resulting link. It follows that the link homology $H(L)$, which is \emph{a priori} $\ZZ \times \ZZ \times \Ztwo$-graded, is actually only $\ZZ \times \ZZ$-graded.

On the other hand $\redh(\Gamma)$ has nonzero contributions in both $\Ztwo$-degrees, so the reduced link homology $\overline{H}(L,C)$, as defined in \cite{kr0401268}, is honestly $\ZZ \times \ZZ \times \Ztwo$-graded. We write $\redh^j(\Gamma)$, respectively $\redh^j(L,C)$, for the $\Ztwo$-component in degree $j \in \Ztwo$. The point of this section is to check that $\redh^0(L,C)$ and $\redh^1(L,C)$ only differ by a grading shift.

\begin{lemma}\label{lemma:redvsunred} There is an isomorphism of $\ZZ \times \ZZ$-graded $\mathbb{Q}$-vector spaces 
\[
\redh^{p+1}(L,C) \cong \redh^{p}(L,C)\{N-1\}.
\]
\end{lemma}
...
\[
0 \lto C(\Gamma) \xlto{x_i} C(\Gamma)\{-2\} \lto C(\Gamma) \otimes_R R/(x)\{-2\} \lto 0
\]
we deduce a long exact sequence in cohomology, of graded $R$-modules
\begin{equation}\label{eq:redvsunred_seq}
0 \lto \redh^{p+1}(\Gamma)\{N-1\} \lto H(\Gamma) \xlto{x_i} H(\Gamma)\{-2\} \lto \redh^p(\Gamma)\{-2\} \lto 0.
\end{equation}
...
\[
\redh^p(L, C) \cong H(H(C(D),d) \otimes_R R/(x_i), d_{\chi})\,.
\]
As has already been mentioned, this $\ZZ \times \ZZ$-graded $\QQ$-vector space is adopted as the \emph{definition} of the reduced Khovanov-Rozansky homology in \cite{r0607544,websterblah, rasmussenblah}. In light of Lemma \ref{lemma:redvsunred} this is reasonable, as the other $\Ztwo$-degree of $\redh(L,C)$ contains no new information, but as far as we know the lemma has not appeared before in the literature (although it is no doubt known to the experts). To relate the $\ZZ$-gradings of $\redh^p(\Gamma)$ and $\redh^{p+1}(\Gamma)$ we make use of the following basic property of the space $H(\Gamma)$.

\textbf{Convention.} Since we ultimately only care about the homologies $H(L)$ and $\overline{H}(L,C)$, we are free to choose $D$ to be the closure of a braid with $i$ the label on one of the edges in the closure, and for the rest of this section we make this assumption. In particular, $\Gamma$ is a braid graph.

\begin{lemma}\label{lemma:directsumalgebras} $H(\Gamma)$ is concentrated in degree $p(\Gamma)$, and as a graded $\QQ[x_i]$-module it is isomorphic to a direct sum of copies of $\QQ[x_i]/(x_i^N)$ shifted in the $\mathbb{Z}$-grading, that is, for some integers $b_j$
\[
...
\]
commutes. Decompose $H(\Gamma), H(\Gamma')$ as in Lemma \ref{lemma:directsumalgebras} and let $\chi': \QQ[x_i]/(x_i^N) \lto \QQ[x_i]/(x_i^N)$ be one of the components of $\chi$. The corresponding component of $\redh^{p+1} \chi$ is the restriction of $\chi'$ to the ideal generated by $x_i^{N-1}$, and the component of $\redh^p \chi$ is the map induced by $\chi$ on the quotients by the ideal $(x_i)$. But since $\chi'$ is homogeneous it sends $1$ to $\lambda x_i^a$ for some $\lambda \in \QQ$ and $0 \le a \le N - 1$, and it is easy to see that if $a > 0$ then these components of $\redh^{p+1} \chi$ and $\redh^p \chi$ are both zero. On the other hand if $a = 0$ then the components are both multiplication by $\lambda$, and hence the the diagram commutes. Since the differentials in $(H^p(C(D) \otimes_R R/(x_i),d), d_\chi)$ are built out of the $\chi$'s, we may conclude from this that $\redh^{p+1}(L,C) \cong \redh^{p}(L,C)\{N-1\}$, completing the proof.
\end{proof}

\begin{proof}[Proof of Lemma \ref{lemma:directsumalgebras}]
The argument is similar to that of \cite[Lemma 5.8]{r0607544}. We prove the claim by induction on the complexity of braid graphs $\Gamma$ together with a chosen edge $i$ appearing in the braid closure, using the induction scheme of Wu \cite{Wu} and the MOY relations proved in \cite[Section 6]{kr0401268}. 

By \cite{Wu}, if $\Gamma$ is the closure of an open braid graph $\Gamma_{open}$ then $\Gamma$ always contains a region of the form $\Gamma_0$ or $\Gamma_{\text{I}}$, or $\Gamma_{open}$ contains a region $\Gamma_{\text{II}}$ or $\Gamma_{\text{III}}$. This means that using only the MOY relations I-III we can go from any braid graph to a family of unlinked circles $\Gamma$, and in this base case $H(\Gamma)$ is is a tensor product over $\mathbb{Q}$ of grading shifted copies of $\QQ[x_j]/(x_j^N)\langle 1 \rangle$ for various $j$. By hypothesis $i$ is among these indices, so both claims are clear in the base case.

The only MOY relation which changes the parity is relation I, which changes the parity by one and also introduces (in the direct sum decomposition of \cite[Section 6]{kr0401268}) a suspension, so it is now clear by induction that $H(\Gamma)$ is always concentrated in degree $p(\Gamma)$, and we need to check the other claim.

Given a braid graph $\Gamma$ suppose that we can find a region of type $\Gamma_{\text{II}}$ or $\Gamma_{\text{III}}$ in $\Gamma_{open}$. Then the corresponding direct sum decomposition (\ref{??}) has $x_i$ as an external variable, so the equivalences of the decompositions are $x_i$-linear. By the inductive hypothesis the cohomology of all the other braid graphs involved in the decomposition are direct sums of grading shifted copies of $\QQ[x_i]/(x_i^N)$ so the same is true of $C(\Gamma)$.
...
In this situation we consider the dual braid graph $\Gamma^{\lor}$ where we reverse the orientation of all the edges. Note that for some sequences of polynomials $\boldsymbol{a},\boldsymbol{b}$ and an integer $c$ we have $C(\Gamma) = \{ \boldsymbol{a}, \boldsymbol{b} \}\{c\}$, and using (\ref{eq:cyclickos1}) it follows that
\[
C(\Gamma^{\lor}) = C(\Gamma)\mdual = \{ \boldsymbol{a}, \boldsymbol{b} \}\{c\}\mdual \cong \{ \bs{a}, -\bs{b} \}\{ \sum_i \deg(a_i) - m(N+1) - c \}\,.
\]
But $\{ \bs{a}, -\bs{b} \}$ has the same cohomology as $\{ \bs{a}, \bs{b} \}$ (Appendix \ref{??}), so up to grading shifts the cohomology of $\Gamma$ and $\Gamma^{\lor}$ agree. It therefore suffices to prove the claim for $\Gamma^{\lor}$. This follows by the above argument, since $i$ is the leftmost edge and can therefore never be involved in a $\Gamma_{\text{I}}$-relation.
\end{proof}

\section{Morphisms via perturbation}\label{section:morphismpert}
% dcct, skrh, ptons

do it ungraded but mention what happens when $\bs{a}, \bs{b}$ are graded.

Let $R$ be a noetherian ring, $\varphi: S \lto R$ a ring morphism and $\bs{a} = (a_1,\ldots,a_n)$ and $\bs{b} = (b_1, \ldots, b_n)$ elements of $R$ with $\bs{b}$ a regular sequence, and set $W = \sum_i a_i b_i$. We assume
\begin{itemize}
\item $R/(\bs{b})$ is a finitely generated projective $S$-module.
\item There is a flat $S$-linear connection
\[
\nabla: R \lto R \otimes_{S[\bs{b}]} \Omega^1_{S[\bs{b}]/S}
\]
such that $\Ker(\nabla) + \bs{b}R R$, which is standard in the sense of \cite{??}.
\item Let $F = R \theta_1 \oplus \cdots \oplus R \theta_n$ with $|\theta_i| = -1$ and define $\{ \bs{a}, \bs{b} \}$ in the usual way, let
\[
\pi: ( \bigwedge F, \delta_+ ) \lto R/(\bs{b})
\]
be the usual morphism from the Koszul complex to its cohomology.
\end{itemize}

\textbf{todo} We begin with the diagram
\begin{equation}\label{eq:cons3}
\xymatrix@C+2pc{
\Hom_R(X, \Delta) \ar[r]^{\Hom(1,\pi_\Delta)} & \Hom_R(X, R/\bold{t} R)\\
X^{\lor} \otimes \Delta \ar[u]^{\cong} \ar[r]_{1 \otimes \pi} & X^{\lor} \otimes R/\bold{t} R \ar@/^2pc/[l]^{\sigma_\infty} \ar[u]_{\cong}
}
\end{equation}
where $(-)^{\lor} = \Hom_R(-,R)$, tensor products are $R$-linear, and the columns are the canonical maps.

There is an $S$-linear homotopy inverse $\sigma_\infty$ of the map $1 \otimes \pi$, which comes about as follows: consider the $\Ztwo$-graded module $X^{\lor} \otimes \Delta$ with only the differential $1 \otimes \delta_{+}$, for which we write $(X^{\lor} \otimes \Delta, 1 \otimes \delta_{+})$. This is clearly quasi-isomorphic to $(X^{\lor} \otimes R/\bold{t} R, 0)$. In fact the map $\pi_{\Delta}: (\Delta, \delta_+) \lto (R/\bold{t} R,0)$ is stronger than a quasi-isomorphism: it is an $S$-linear homotopy equivalence, and we can produce an inverse using a choice of connection. Since we can write $R$ as $S[t_1,t_2]$ there is an $S$-linear standard (\emph{explain}) flat connection
\begin{gather*}
\nabla: R \lto R \otimes_{S[\bold{t}]} \Omega^1_{S[\bold{t}]/S},\\
\nabla( f(x_1,x_2,y_1,y_2) ) = \frac{\partial f}{\partial y_1} \ud t_1 + \frac{\partial f}{\partial y_2} \ud t_2.
\end{gather*}
This induces an $S$-linear map, also denoted $\nabla$, on $R \otimes_{S[\bold{t}]} \Omega_{S[\bold{t}]/S}$ where $\Omega = \bigwedge^\bullet \Omega^1$. We identify this free $\mathbb{Z}$-graded $R$-module with $\bigwedge F$ (and hence also with $\Delta$) via $\ud t_i = \theta_i$. It is easily checked that for $\theta = \theta_{i_1} \cdots \theta_{i_p}$ with distinct indices $i_j$ and $p > 0, f \in R$,
\[
...
H = (\nabla \delta_+ + \delta_+ \nabla)^{-1} \nabla
\]
on the module $\bigwedge F$. Each application of $H$ involves differentiation with respect to the $y_i$, and then solving a differential equation. Let $\sigma: S = R/\bold{t} R \lto \bigwedge F$ be the inclusion of $k[x_1,x_2]$ into $R$. One checks that $H \delta_+ + \delta_+ H = 1 - \sigma \pi_{\Delta}$, so that $\sigma$ is exhibited by $H$ as the $S$-linear homotopy inverse to the canonical morphism $\pi_{\Delta}: (\Delta, \delta_+) \lto (R/\bold{t} R,0)$ from the Koszul complex to its cohomology.

It remains to tensor with $X^{\lor}$ and perturb in the other differentials. With only the differential $\delta_+$ turned on it is clear that $X^{\lor} \otimes \Delta$ is $S$-homotopy equivalent to $X^{\lor} \otimes R/\bold{t} R$. The actual equivalence is exhibited by extending $H$ and $\sigma$ to $S$-linear maps
\[
\xymatrix@C+2pc{
X^{\lor} \otimes \Delta \ar@(dl,dr)[]_{H} & X^{\lor} \otimes R/\bold{t} R \ar[l]_{\sigma}
}
\]
using the homogeneous basis $1^*, \theta_1^*, \theta_2^*, (\theta_1\theta_2)^*$ for $X^{\lor}$. This involves Koszul signs as we move $H$ and $\sigma$ past homogeneous elements of $X^{\lor}$, for example, $H(\theta_1^* \otimes f \theta_2) = - \theta_1^* \otimes H(f \theta_2)$. 

Next we consider the $R$-linear map
\[
\mu = d_{X^{\lor}} \otimes 1 + 1 \otimes \delta_{-}
\]
on $X^{\lor} \otimes \Delta$, viewed as a perturbation of the differential on $(X^{\lor} \otimes \Delta, 1 \otimes \delta_{+})$. The total differential $\mu + 1 \otimes \delta_{+}$ is the usual differential on the tensor product, and the perturbation lemma tells us that the $S$-linear map
\[
\sigma_\infty = \sum_{m \ge 0} (-1)^m (H \mu)^m \sigma
\]
is an $S$-homotopy inverse to $1 \otimes \pi$. Composing with the vertical isomorphisms in (\ref{eq:cons3}) we have the desired inverse of $\Hom(1,\pi_{\Delta})$. The morphism (\ref{eq:cons4}) corresponds to the tensor $1^* \otimes 1 \in X^{\lor} \otimes R/\bold{t} R$, so to find $\chi_1$ we are left with the task of evaluating
\[
...
The map corresponding to $\sigma_\infty(1^* \otimes 1)$ makes (\ref{eq:cons5}) commute, so we need to evaluate this tensor and show that it has the explicit form given in the statement of the lemma. The notation becomes a little involved: throughout recall that we are producing tensors in $X^{\lor} \otimes \Delta$, an $R$-basis of which is given by tensors $\theta_1^* \otimes 1, 1^* \otimes \theta_1\theta_2$, etc. Clearly $\sigma(1^* \otimes 1)$ is just $1^* \otimes 1$, and
...
&+  \frac{1}{2}(\theta_1 \theta_2)^* \otimes (x_1 + y_1 - x_2 - y_2) \theta_1 \theta_2\\
&- \theta_1^* \otimes (\theta_1 + \theta_2) - \frac{1}{2} \theta_2^* \otimes ( (x_2+y_2)\theta_1 + (x_1+y_1)\theta_2 ).
\end{split}
\end{equation}
The image under the canonical isomorphism $\varepsilon: X^{\lor} \otimes \Delta \lto \Hom_R(X, \Delta)$ is our morphism $\chi_1 := \varepsilon \sigma_\infty(1^* \otimes 1)$, which has the desired form (the signs get corrected by Koszul signs in $\varepsilon$).
\end{proof}
...
\bibitem[BN03]{bnKhovanov11crossings11crossings}
