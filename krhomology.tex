...
\]
The \emph{total potential} $W$ is the sum over external edges $e$ of the potentials $W_e$, which are given a positive sign if $e$ begins on a vertex and ends on the boundary, and a negative sign otherwise; hence $W=W_{b}$. If there are no external edges, then $W = 0$ and $\rext = k$. Finally, we write
\[
